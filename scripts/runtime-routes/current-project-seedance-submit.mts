...
function agentTaskPreflightFromRequest(value) {
  const preflight = isRecord(value?.preflight) ? value.preflight : {};
  return {
    projectWriteReceiptRequired: booleanFlag(preflight.projectWriteReceiptRequired),
    ruleQaRequired: booleanFlag(preflight.ruleQaRequired),
    textQaRequired: booleanFlag(preflight.textQaRequired),
    noBgmGuardRequired: booleanFlag(preflight.noBgmGuardRequired),
    providerSubmitAfterPreflightOnly: booleanFlag(preflight.providerSubmitAfterPreflightOnly),
  };
}

function agentTaskEnvelopeFromRequest(value) {
  if (!isRecord(value)) return undefined;
  const id = asString(value.id);
  const inputHash = asString(value.inputHash);
  if (!id || !inputHash) return undefined;
  return {
    id,
    inputHash,
    policyBinding: asString(value.policyBinding, "director_agent_tool_handoff"),
    actionId: asString(value.actionId),
    handoffId: asString(value.handoffId),
    handler: asString(value.handler),
    expectedReceipt: asString(value.expectedReceipt),
    preflight: agentTaskPreflightFromRequest(value),
  };
}

function agentTaskEnvelopeBlockers(envelope) {
  if (!envelope) return [];
  return uniqueStrings([
    envelope.policyBinding === "director_agent_tool_handoff" ? "" : "Agent 任务记录来源不正确。",
    envelope.handler === "seedance_video_submit" ? "" : "Agent 任务不是视频提交动作。",
    envelope.expectedReceipt === "video_submit_receipt" ? "" : "Agent 视频任务回执类型不正确。",
    envelope.preflight?.projectWriteReceiptRequired ? "" : "Agent 视频任务缺少项目写入回执要求。",
    envelope.preflight?.ruleQaRequired ? "" : "Agent 视频任务缺少规则 QA 前置检查。",
    envelope.preflight?.textQaRequired ? "" : "Agent 视频任务缺少文本 QA 前置检查。",
    envelope.preflight?.noBgmGuardRequired ? "" : "Agent 视频任务缺少 no BGM 保护。",
    envelope.preflight?.providerSubmitAfterPreflightOnly ? "" : "Agent 视频任务没有声明通过前置检查后才能提交。",
  ]);
}
...
function storyShots(projectFacts, workbenchFacts) {
  ...
  const visualAssets = Array.isArray(workbenchFacts?.visualMemory?.assets) ? workbenchFacts.visualMemory.assets : [];
  ...
  return source.filter(isRecord).map((shot, index) => {
    const shotId = asString(shot.id || shot.shotId, `shot_${index + 1}`);
    ...
    const scopedSceneAssetIds = scopedAssetReferenceIds({ shot, shotId, assets: visualAssets, type: "scene" });
    const scopedCharacterAssetIds = scopedAssetReferenceIds({ shot, shotId, assets: visualAssets, type: "character" });
    const scopedPropAssetIds = scopedAssetReferenceIds({ shot, shotId, assets: visualAssets, type: "prop" });
    return {
      id: shotId,
      ...
      sceneAssetIds: scopedSceneAssetIds,
      characterAssetIds: uniqueStrings([...scopedCharacterAssetIds, ...propIdsThatAreCharacters]),
      propAssetIds: scopedPropAssetIds.filter((id) => !characterKeys.has(identityKey(id))),
...
function shotSceneCluster(shot) {
  const sceneAssetIds = textArray(shot.sceneAssetIds);
  if (sceneAssetIds.length) {
    const stableKeys = sceneAssetIds.map(identityKey).filter(Boolean).sort();
    if (stableKeys.length) {
      return {
        key: `scene_asset_${stableKeys.join("_")}`,
        label: sceneAssetIds.slice(0, 2).join(" / "),
      };
    }
  }
...
function relayQueueForSegments(input) {
  ...
  return buildVideoRelayQueueState({
    ...
    storyboardConfirmed: input.storyboardConfirmed !== false,
    paused: false,
    items,
  });
}
...
function activeRelayQueueItem(existingQueue, relayQueueItemId) {
...
function resumeDownloadDirFromCommand(command, fallbackDir) {
  ...
  const match = value.match(/--download_dir(?:=|\s+)(?:"([^"]+)"|'([^']+)'|(\S+))/);
  return match?.[1] || match?.[2] || match?.[3] || fallbackDir;
}
...
function explicitVisibleClipCountFromText(text, durationSeconds) {
  const normalized = cleanPublicText(text);
  const clipRange = normalized.match(/(\d+)\s*[-~～到至]\s*(\d+)\s*个?\s*(?:最终)?(?:可见)?(?:剪辑|镜头|片段|clips?)/i);
  if (clipRange) {
  ...
  const clipExact = normalized.match(/(\d+)\s*个?\s*(?:最终)?(?:可见)?(?:剪辑|镜头|片段|clips?)/i);
  if (clipExact) {
    const value = Number(clipExact[1]);
    if (Number.isFinite(value)) return Math.max(1, value);
  }
  const cutRange = normalized.match(/(\d+)\s*[-~～到至]\s*(\d+)\s*个?\s*(?:可见)?切点/i);
  if (cutRange) {
    const low = Number(cutRange[1]);
    const high = Number(cutRange[2]);
    if (Number.isFinite(low) && Number.isFinite(high)) {
      return Math.max(1, (durationSeconds > 4.5 ? Math.max(low, high) : Math.min(low, high)) + 1);
    }
  }
  const cutExact = normalized.match(/(\d+)\s*个?\s*(?:可见)?切点/i);
  if (cutExact) {
...
function rapidVisibleClipCountForShot(shot, compilePlan) {
  ...
  const visibleClipBudgetCount = explicitVisibleClipCountFromText([
    shot.splitPolicy,
    shot.intent,
    shot.camera,
    shot.primaryAction,
  ].filter(Boolean).join(" "), shot.durationSeconds);
  if (visibleClipBudgetCount > 0) return Math.max(1, Math.min(12, visibleClipBudgetCount));
...
function seedanceSubmitRequestInput(body) {
  ...
  return {
    ...
    agentTaskEnvelope: agentTaskEnvelopeFromRequest(input?.agentTaskEnvelope),
    mockProviderResult: input?.mockProviderResult === true,
    cliPath: validateCliPath(input?.cliPath || process.env.VIBE_JIMENG_CLI_PATH),
  };
}

function seedanceResumeRequestInput(body) {
  const input = unwrapRequestJsonBody(body);
  return {
    relayQueueItemId: asString(input?.relayQueueItemId, undefined),
    pollSeconds: Math.max(30, Math.floor(asNumber(input?.pollSeconds, DEFAULT_POLL_SECONDS))),
    mockProviderResult: input?.mockProviderResult === true,
    cliPath: validateCliPath(input?.cliPath || process.env.VIBE_JIMENG_CLI_PATH),
  };
...
export function createRuntimeApiCurrentProjectSeedanceSubmit(deps) {
  const {
    endpoint,
    resumeEndpoint = endpoint.replace(/\/submit$/, "/resume"),
    repoRoot,
    ...
    runCommand: runCommandDep = runCommand,
    running,
    setRunning,
  } = deps;
  ...
  async function currentProjectSeedanceSubmitResponse(input, extra = {}, source) {
    ...
    const baseRelayQueue = relayQueueForSegments({
      ...
      storyboardConfirmed: confirmationOk,
    ...
    if (submitLockPromise) {
      return {
        ...
        agentTaskEnvelopeId: input.agentTaskEnvelope?.id,
        agentTaskInputHash: input.agentTaskEnvelope?.inputHash,
    ...
    try {
      ...
      if (textQaReport.status === "blocked") {
        const report = {
          ...
          ...runtimePolicy({
            ...
            liveSubmitAllowed: false,
            workerSpawnForbidden: true,
            dryRunOnly: !textQaReport.runtimeExternalNetworkCallMade,
          }),
          endpoint,
      ...
      if (!storyboardResult.ok) {
        const report = {
          ...
          ...runtimePolicy({
            ...
            liveSubmitAllowed: false,
            workerSpawnForbidden: true,
            dryRunOnly: input.mockProviderResult === true,
          }),
          endpoint,
      ...
      if (submit.exitCode !== 0 && !taskInfo.submitId && !videoFiles.length) {
        ...
        status: "submit_failed",
          ...
          providerCalled: true,
          runtimeExternalNetworkCallMade: true,
          storyboardGenerated: hasStoryboardReference,
          videoSubmitted: false,
          outputRoot,
          storyboardReferencePath: hasStoryboardReference ? storyboardRelPath : undefined,
          promptPath: promptRelPath,
  ...
  async function currentProjectSeedanceResumeResponse(input, extra = {}, source) {
    ...
    const referenceSegments = contiguousReferenceSegments(allShots);
    const relayQueueRelPath = `${source.runRootRelativePath}/reports/video_relay_queue.json`;
    const reportRelPath = `${source.runRootRelativePath}/reports/seedance_resume_report.json`;
    const previewPlanRelPath = source.previewPlanRelativePath;
    let relayQueuePath;
    let previewPlanPath;
    try {
    ...
    if (blockers.length) {
      const report = {
        ok: false,
        ...runtimePolicy({
          runMode: "current_project_seedance_resume",
          providerCalled: false,
          liveSubmitAllowed: false,
          workerSpawnForbidden: true,
          dryRunOnly: true,
    ...
    const taskInfo = extractDreaminaTaskInfo(query.stdout, query.stderr);
    let videoFiles = findVideoFiles(videoDir);
    if (!videoFiles.length && taskInfo.videoUrls?.length) {
      await downloadDreaminaVideoUrls(taskInfo.videoUrls, videoDir);
      videoFiles = findVideoFiles(videoDir);
    ...
    if (outputVideoPath) {
      const videoItemId = `seedance_storyboard_video_${activeSegmentId || safePathSegment(activeItem.id)}`;
      const belongsToActiveSegment = (item) => item?.segmentId === activeSegmentId || item?.relayQueueItemId === relayQueueItemId;
      const previousClips = Array.isArray(existingPreviewPlan?.clips)
        ? existingPreviewPlan.clips.filter((item) => !belongsToActiveSegment(item))
        : [];
      const previousPreviewItems = Array.isArray(existingPreviewPlan?.previewItems)
      ...
      const currentClip = {
        ...
        segmentShotIds: activeShots.map((shot) => shot.id),
        localMediaPaths: [outputVideoPath],
      };
      const currentPreviewItem = {
        ...currentClip,
        reviewRequired: true,
      };
      writeCurrentProjectRuntimeJson(previewPlanRelPath, {
        ...(existingPreviewPlan || {}),
        schemaVersion: "current_project_seedance_preview_plan_v1",
        generatedAt: checkedAt,
        status: relayQueue.autoSubmitAllowed ? "ready_for_next_submit" : "needs_review",
  ...
  async function handleCurrentProjectSeedanceResumeRoute(req, res, url) {
    if (req.method !== "POST" || url.pathname !== resumeEndpoint) return false;
    const routeContext = await currentProjectRouteContext(req, res, url, resumeEndpoint);
    if (!routeContext) return true;
    const input = seedanceResumeRequestInput(routeContext.body);
    const payload = await currentProjectSeedanceResumeResponse(input, {
      running: runtimeState(),
      ignoredRequestContext: requestOverrideDiagnostics(routeContext.requestContext),
    }, routeContext.source);
    writeJson(res, payload.ok === false ? 409 : 200, payload);
  ...
  return {
    ...
    seedanceResumeRequestInput,
    currentProjectSeedanceSubmitResponse,
    currentProjectSeedanceResumeResponse,
