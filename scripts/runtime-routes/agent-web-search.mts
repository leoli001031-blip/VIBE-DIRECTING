...
export function createRuntimeApiAgentWebSearchRoute({
  ...
  async function handleRuntimeApiAgentWebSearchRoute(req, res, url) {
    ...
    try {
      ...
      const agentTaskEnvelope = agentTaskEnvelopeFromRequest(body.agentTaskEnvelope);
      ...
      const resultWithTrace = agentTaskEnvelope
        ? {
            ...result,
            agentTaskEnvelope,
            agentTaskEnvelopeId: agentTaskEnvelope.id,
            agentTaskInputHash: agentTaskEnvelope.inputHash,
          }
        : result;
      writeJson(res, 200, {
        ok: true,
        ...runtimePolicy(),
        status: resultWithTrace.status,
        providerCalled: resultWithTrace.networkCalled,
