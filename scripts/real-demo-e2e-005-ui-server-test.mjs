  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", (chunk) => {
      ...
      for (const line of stdout.split(/\r?\n/)) {
        if (!line.includes("real-demo-e2e-005-ui-server-listening")) continue;
        try {
...
const child = spawn(process.execPath, ["scripts/real-demo-e2e-005-ui-server.mjs"], {
  cwd: process.cwd(),
  env: { ...process.env, REAL_DEMO_E2E_005_UI_SERVER_PORT: "0" },
  stdio: ["ignore", "pipe", "pipe"],
});

try {
  const { baseUrl } = await waitForServer(child);
  const status = await fetchJson(`${baseUrl}/api/real-demo-e2e/005/status`);
  assert(status.response.status === 200, "GET status should return 200");
  assert005Payload(status.payload, "GET status");

  const run = await fetchJson(`${baseUrl}/api/real-demo-e2e/005/run`, { method: "POST" });
  ...
  console.log("Real Demo E2E 005 UI server test passed. No provider was called.");
} finally {
  child.kill("SIGTERM");
}
