...
function assertCreatorPanelContract() {
  ...
  assert(/function initialBrowserProjectDraftStorageKey\(\)[\s\S]*const explicitSession[\s\S]*if \(!explicitSession\)[\s\S]*readActiveBrowserProjectVibeDraftStorageKey\(\)[\s\S]*if \(activeStorageKey\) return activeStorageKey/.test(appSource), "normal cold starts must reopen the active confirmed browser draft instead of generating an unrelated storage key");
  assert(/if \(projectDraftUsesBrowserStorage\(draftTarget, saveResult\.mode\)\)[\s\S]*rememberActiveBrowserProjectVibeDraftStorageKey\(draftTarget\.storageKey\)/.test(app), "confirmed browser stories must register their draft storage key for cold-start restore");
  assert(/bindCurrentDraftToLocalProjectSelection[\s\S]*if \(!prototypeProjectDraftTarget\.projectRoot\)[\s\S]*forgetActiveBrowserProjectVibeDraftStorageKey\(prototypeProjectDraftTarget\.storageKey\)/.test(app), "migrating an unsaved story to a local project must clear the browser-draft restore pointer");
  assert(/forgetProjectFileRoot[\s\S]*forgetActiveBrowserProjectVibeDraftStorageKey\(draftTargetToForget\.storageKey\)[\s\S]*forgetBrowserProjectVibeDraft\(draftTargetToForget\)/.test(app), "abandoning an unsaved story must clear both its active pointer and saved browser draft");
