...
function assertCreatorPanelContract() {
  ...
  assert(
    /migrateProjectAgentTimelineEntriesToProjectRoot[\s\S]*projectId:\s*currentProject\.manifest\.projectId[\s\S]*projectFactHash:\s*hashProjectVibeFacts\(currentProject\)[\s\S]*saveProjectAgentTimeline\(target,\s*migratedTimeline\)/.test(bindCurrentDraftToLocalProjectSelection),
    "save-location migration must explicitly fact-bind browser Agent timeline entries to the chosen local project",
  );
  assert(
    /migrateProjectAgentStagedPlanDraftToProjectRoot[\s\S]*sourceProjectRoot:\s*prototypeProjectDraftTarget\.projectRoot[\s\S]*targetProjectRoot:\s*selection\.projectRoot[\s\S]*saveProjectAgentStagedPlanDraft\(target,\s*migratedStagedPlan\.draft\)/.test(bindCurrentDraftToLocalProjectSelection),
    "save-location migration must validate and explicitly rebind an active staged plan before writing it locally",
  );
  assert(
    /Generation jobs are never rebound across roots[\s\S]*setRestoredAgentGenerationJobLedger\(undefined\)/.test(bindCurrentDraftToLocalProjectSelection),
    "save-location migration must not inherit browser-root generation jobs into a new local root",
  );
  ...
  assert(/if \(runtimeWriteOk\) \{[\s\S]*return \{[\s\S]*status: "written"[\s\S]*const writeResult = await writeProjectVibeSidecarText/.test(projectAgentTimelineSource), "an atomic runtime timeline write must return before the fallback writer");
  ...
  assert(/projectTargetMode:\s*agentNewVideoProjectTargetMode\(\{[\s\S]*localProjectReady:\s*folderReady[\s\S]*currentProjectShotCount:\s*runtimeState\.storyFlow\.shots\.length/.test(directorModeSource), "Agent-started drafts must reuse an explicitly connected empty local project without overwriting a project that already has shots");
  assert(/const\s+shouldUseSelectedProject\s*=\s*draft\.projectTargetMode !== "new_project";[\s\S]*if \(shouldUseSelectedProject && projectFileSelection\.status === "selected"\)/.test(app), "new-video confirmation must honor the Agent target mode when deciding whether to reuse the selected project");
  ...
  assert(/const\s+browserProjectDraftColdRestoreActive\s*=\s*Boolean\(browserProjectDraftRecoveryModeRef\.current\)[\s\S]*runtimeProjectBinding\.status !== "loading"[\s\S]*!runtimeBindingIsLocalProject[\s\S]*const\s+selectedProjectUsesBrowserDraftStorage\s*=\s*\(selectedProjectIsBrowserDraft \|\| browserProjectDraftColdRestoreActive\)[\s\S]*!\(runtimeBindingIsLocalProject && selectedProjectMatchesRuntimeBinding\)/.test(app), "runtime-bound browser-project projects must not use temporary draft storage while cold browser recovery remains available");
  ...
  assert(/async function openOrInitializeProjectDraft\(\)[\s\S]*openProjectVibeDraft\(prototypeProjectDraftTarget\)[\s\S]*if \(result\.ok && result\.project\)[\s\S]*openProjectAgentTimeline\(prototypeProjectDraftTarget[\s\S]*openProjectAgentGenerationJobLedger\([\s\S]*applyProjectVibeProjectState\(result\.project, prototypeProjectDraftTarget[\s\S]*setLoadedPrototypeProjectDraftTargetId\(prototypeProjectDraftTargetId\);\s*loadedPrototypeProjectDraftStorageModeRef\.current = "local"/.test(app), "local project restore must publish its loaded target only after Project.vibe, timeline, and generation ledger restoration finish");
  assert((app.match(/cancelled \|\| loadedPrototypeProjectDraftStorageModeRef\.current === "local"/g) || []).length >= 3, "late bootstrap runtime fixtures must not overwrite an authoritative local Project.vibe or clear its restored Agent sidecars");
  assert(/openProjectAgentTimeline\(runtimeDraftTarget,\s*\{[\s\S]*project:\s*projectForRestore[\s\S]*projectRoot:\s*runtimeProjectBinding\.projectRoot[\s\S]*openProjectAgentGenerationJobLedger\([\s\S]*const agentTimelineForRestore = interruptedExportRecovery\?\.timeline \|\| timelineOpen\.timeline[\s\S]*const generationLedgerForRestore = interruptedExportRecovery\?\.ledger \|\| generationLedgerOpen\.ledger[\s\S]*setRestoredAgentTimelineEntries\(timelineOpen\.ok \? agentTimelineForRestore\.entries : \[\]\)[\s\S]*setRestoredAgentGenerationJobLedger\(generationLedgerOpen\.ok \? generationLedgerForRestore : undefined\)/.test(app), "runtime-bound projects must restore timeline and generation ledger independently through project sidecars and interrupted-export recovery");
  ...
  assert(/restoreBrowserDraftStateAndAgentSidecars[\s\S]*openProjectAgentStagedPlanDraft\(prototypeProjectDraftTarget[\s\S]*openProjectAgentActionLog\(prototypeProjectDraftTarget[\s\S]*openProjectAgentTimeline\(prototypeProjectDraftTarget[\s\S]*setRestoredAgentTimelineEntries\(timelineOpen\.ok \? timelineOpen\.timeline\.entries : \[\]\)/.test(app), "browser draft Agent timeline must be restored from sidecar storage after the project draft is recovered");
  ...
  assert(/function initialBrowserProjectDraftSession\(\)[\s\S]*const explicitSession[\s\S]*if \(!explicitSession\)[\s\S]*readActiveBrowserProjectVibeDraftStorageKey\(\)[\s\S]*recoveryMode: "confirmed_project"[\s\S]*readPendingBrowserNewVideoIntakeStorageKey\(\)[\s\S]*recoveryMode: "pending_intake"/.test(appSource), "normal cold starts must prefer the active confirmed browser project, then recover a distinct pending intake session");
  assert(/function isNewVideoSurfaceAgentTimelineEntry\(entry: VibeAgentTimelineEntry\)[\s\S]*entry\.id\.startsWith\("draft_selection_context_"\)/.test(directorModeSource), "pending intake cold restore must retain structured selected-shot context when projecting the new-video timeline");
  assert(/browserProjectDraftRecoveryModeRef\.current === "pending_intake"[\s\S]*recoverPendingNewVideoIntake\(timelineOpen\.timeline\.entries\)[\s\S]*pendingIntakeRecovery\.status !== "restorable"[\s\S]*await clearPersistedPendingBrowserNewVideoIntakeStorageKey/.test(app), "missing or corrupt pending-intake sidecars must fail closed and durably clear only their recovery pointer");
  assert(/await clearPersistedPendingBrowserNewVideoIntakeStorageKey\(\);[\s\S]*if \(projectDraftUsesBrowserStorage\(draftTarget, saveResult\.mode\)\)[\s\S]*await persistActiveBrowserProjectVibeDraftStorageKey\(draftTarget\.storageKey\)/.test(app), "confirmed browser stories must durably replace their pending pointer with the active draft storage key");
  assert(/bindCurrentDraftToLocalProjectSelection[\s\S]*if \(!prototypeProjectDraftTarget\.projectRoot\)[\s\S]*await clearPersistedActiveBrowserProjectVibeDraftStorageKey\(prototypeProjectDraftTarget\.storageKey\)[\s\S]*await clearPersistedPendingBrowserNewVideoIntakeStorageKey/.test(app), "migrating an unsaved story to a local project must durably clear browser-draft recovery pointers");
  assert(/forgetProjectFileRoot[\s\S]*await clearPersistedActiveBrowserProjectVibeDraftStorageKey\(draftTargetToForget\.storageKey\)[\s\S]*await clearPersistedPendingBrowserNewVideoIntakeStorageKey[\s\S]*await forgetBrowserProjectVibeDraftPersisted\(draftTargetToForget\)/.test(app), "abandoning an unsaved story must durably clear both recovery pointers and its saved browser draft");
  ...
  assert(/const pendingVideoReviewFocusIdentity = useMemo\(\(\) => \{[\s\S]*item\.kind === "video_clip" && item\.returned && item\.reviewRequired[\s\S]*item\.sourceReceiptId[\s\S]*item\.outputHash[\s\S]*currentProjectPreviewProjection\.projectId[\s\S]*currentProjectPreviewProjection\.projectRoot/.test(appSource), "App must bind cold-start Review focus to an exact returned video identity instead of any passive review count");
  assert(/if \(!pendingVideoReviewFocusIdentity\) \{[\s\S]*pendingVideoReviewFocusIdentityRef\.current = ""[\s\S]*pendingVideoReviewFocusIdentityRef\.current === pendingVideoReviewFocusIdentity[\s\S]*pendingVideoReviewFocusIdentityRef\.current = pendingVideoReviewFocusIdentity[\s\S]*directorView !== "preview"[\s\S]*setDirectorView\("preview"\)/.test(appSource), "A newly recovered needs_review video must open Preview once, reset after review clears, and avoid trapping later navigation");
  ...
  assert(/const \[agentCurrentTaskProjection, setAgentCurrentTaskProjection\] = useState<AgentCurrentTaskProjection>\(\)[\s\S]*handleAgentCurrentTaskProjectionChange[\s\S]*onCurrentTaskProjectionChange=\{handleAgentCurrentTaskProjectionChange\}/.test(directorModeSource), "DirectorMode must receive the complete current-task projection from the right Agent");
  assert(/onCurrentTaskProjectionChange\?: \(projection: AgentCurrentTaskProjection \| undefined\) => void[\s\S]*onCurrentTaskProjectionChange\?\.\(agentCurrentTaskProjection\)/.test(agentPanelSource), "MinimalAgentPanel must publish its authoritative current-task projection to DirectorMode");
  assert(/const displayedPendingAgentConfirmationCopy = agentCurrentTaskProjection\?\.requiresConfirmation[\s\S]*\? agentCurrentTaskProjection\.label[\s\S]*const displayedPendingAgentConfirmationForStatus = displayedPendingAgentConfirmationCopy[\s\S]*projectStatusViewWithPendingAgentConfirmation/.test(directorModeSource), "DirectorMode must consume the projection-owned confirmation boundary without command or timeline fallback arbitration");
  assert(/function projectStatusViewWithCompletedExport[\s\S]*currentTask\?\.completion\?\.step !== "export"[\s\S]*currentTask\.completion\.executionMode !== "live"[\s\S]*stage:\s*"导出已完成"[\s\S]*agentCurrentTaskProjection,[\s\S]*directorView/.test(directorModeSource), "DirectorMode must let the structured live export completion override stale delivery-ready copy after restart");
  assert(/<MinimalExport[\s\S]*exportCompleted=\{agentCurrentTaskProjection\?\.completion\?\.step === "export"[\s\S]*agentCurrentTaskProjection\.completion\.executionMode === "live"\}/.test(directorModeSource), "DirectorMode must pass the projection-owned live completion into the delivery surface");
  assert(!/currentAgentCommandConfirmationLabel|latestPendingAgentConfirmation|pendingAgentConfirmationHasVisibleCard|staleProjectEditConfirmation/.test(directorModeSource), "DirectorMode must not independently parse visible command or confirmation copy after projection ownership is established");
  ...
  assert(!/videoSubmitCancelled/.test(agentPanelSource) && /const videoSubmissionBlocked = videoSendAction\?\.status === "blocked"/.test(agentPanelSource), "Agent Panel must use structured video status instead of cancellation display copy");
  assert(!/videoSubmitCancelled/.test(directorModeSource) && /command\.kind === "submit_video" && videoSendAction\?\.status === "blocked"/.test(directorModeSource), "Director shell must use structured blocked status instead of cancellation display copy");
