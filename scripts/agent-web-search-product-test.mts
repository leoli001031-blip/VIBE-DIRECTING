...
assert(enabledSuggestion.query.includes("分镜方法"), "suggestion should build a director research query");
assert(enabledSuggestion.query.includes("storyboard analysis"), "suggestion should keep English research terms for broader search recall");
...
try {
  ...
  const handled = await route.handleRuntimeApiAgentWebSearchRoute({
    ...
    body: {
      ...
      maxResults: 2,
      agentTaskEnvelope: {
        id: "agent_tool_task_web_search_test_001",
        inputHash: "agent_tool_input_web_search_test_001",
        policyBinding: "director_agent_tool_handoff",
        actionId: "agent_action_web_search_test_001",
        handoffId: "agent_tool_handoff_web_search_test_001",
        handler: "web_search",
        expectedReceipt: "web_search_review_card",
      },
    },
  ...
  const payload = mockResponse.writes[0].payload as {
    result?: {
      evidencePath?: string;
      resultCount?: number;
      networkCalled?: boolean;
      promotionAllowed?: boolean;
      agentTaskEnvelopeId?: string;
      agentTaskInputHash?: string;
    };
  };
  ...
  assert(payload.result.agentTaskEnvelopeId === "agent_tool_task_web_search_test_001", "web search result should preserve the Agent tool task id");
  assert(payload.result.agentTaskInputHash === "agent_tool_input_web_search_test_001", "web search result should preserve the Agent tool input hash");
  ...
  assert(evidence.includes("agent_tool_task_web_search_test_001"), "evidence should preserve the Agent tool task id");
  assert(evidence.includes("agent_tool_input_web_search_test_001"), "evidence should preserve the Agent tool input hash");
