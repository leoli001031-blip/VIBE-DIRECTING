...
checkMessage(requireWithin(minimalAgentPanelSource, /const preserveReviewRegenerationHandoff = agentDirectorReviewRegenerationConfirmationMatchesJob\([\s\S]*const refreshedToolHandoff = preserveReviewRegenerationHandoff[\s\S]*\? draft\.toolHandoff[\s\S]*: buildVibeAgentToolHandoff\(\{[\s\S]*action: draft\.action[\s\S]*userConfirmed: false[\s\S]*availability: currentAgentToolAvailability\(draft\.action\)[\s\S]*setAgentToolHandoff\(refreshedToolHandoff\)/, "Restored Agent confirmations must preserve exact independent-regeneration handoffs and refresh all other handoffs from action-specific live availability before showing blockers"));
