...
checkMessage(requireWithin(directorModeSource, /function agentTimelineEntryClosesPendingConfirmation\(entry: VibeAgentTimelineEntry\)[\s\S]*entry\.type !== "action_result" && entry\.type !== "tool_result"[\s\S]*entry\.status === "waiting" \|\| entry\.lifecycle === "running"[\s\S]*entry\.lifecycle === "cancelled"[\s\S]*function agentTimelineEntryMatchesPendingConfirmation[\s\S]*entry\.actionId && confirmation\.actionId[\s\S]*entry\.toolName && confirmation\.toolName[\s\S]*function latestPendingAgentConfirmation[\s\S]*const laterClosedConfirmations: VibeAgentTimelineEntry\[\] = \[\][\s\S]*agentTimelineEntryClosesPendingConfirmation\(entry\)[\s\S]*laterClosedConfirmations\.push\(entry\)[\s\S]*laterClosedConfirmations\.some\(\(result\) => agentTimelineEntryMatchesPendingConfirmation\(result,\s*entry\)\)[\s\S]*continue/, "DirectorMode must let later completed, blocked, failed, or cancelled Agent results close matching pending confirmations so the workbar does not keep waiting for a right-rail card that is no longer visible"));
checkMessage(requireWithin(directorModeSource, /function laterUserMessageReplacesPendingConfirmation\(entry: VibeAgentTimelineEntry\)[\s\S]*local_agent_user_[\s\S]*开始补参考\|准备参考计划[\s\S]*先别生成\|别生成\|不要生成了\|取消\|撤销\|继续说明[\s\S]*return false[\s\S]*function latestPendingAgentConfirmation[\s\S]*seenLaterUserMessage[\s\S]*entry\.type === "user_message"[\s\S]*laterUserMessageReplacesPendingConfirmation\(entry\)[\s\S]*seenLaterUserMessage = true[\s\S]*entry\.type === "confirmation_request"[\s\S]*return seenLaterUserMessage \? undefined : entry/, "DirectorMode must treat real later user revisions as replacing older right-rail confirmations without letting late local reference-plan records hide an active waiting confirmation"));
checkMessage(requireWithin(directorModeSource, /function pendingAgentConfirmationLabel\(entry\?: VibeAgentTimelineEntry\)[\s\S]*entry\.facts[\s\S]*Object\.values\(entry\.details \|\| \{\}\)[\s\S]*保存位置\|项目文件夹[\s\S]*return "选择保存位置"[\s\S]*方式\|update_shot_strategy[\s\S]*return "确认方式"[\s\S]*requestedStoryboardShotCountFromIntent\(copy\)[\s\S]*`确认重排为 \$\{requestedShotCount\} 个镜头`[\s\S]*修改\|revise_story_or_shot\|write_project[\s\S]*return "确认修改"/, "DirectorMode must label save-location confirmations before generic write-project edits while preserving concrete shot-count confirmations"));
checkMessage(requireWithin(directorModeSource, /function restoredAgentStagedPlanPendingConfirmationLabel\(draft\?: ProjectAgentStagedPlanDraft\)[\s\S]*draft\?\.status !== "active"[\s\S]*draft\.action\?\.status !== "staged"[\s\S]*!draft\.action\.requiresUserConfirmation[\s\S]*prepare_reference_generation\|image2_reference_generation\|generate_references[\s\S]*return "确认生成参考"/, "DirectorMode must treat an active restored staged sidecar as a pending confirmation even when later plan-only timeline turns hide the old confirmation entry"));
...
checkMessage(requireWithin(directorModeSource, /const pendingAgentConfirmationHasVisibleCard = Boolean\([\s\S]*agentVisiblePendingConfirmationLabel[\s\S]*restoredAgentStagedPlanConfirmationCopy[\s\S]*pendingAgentConfirmation[\s\S]*const staleProjectEditConfirmation = !pendingAgentConfirmationHasVisibleCard && \/确认方式\|确认修改\/\.test\(pendingAgentConfirmationCopy\)[\s\S]*displayedPendingAgentConfirmationCopy[\s\S]*currentCommandConfirmationCopy[\s\S]*pendingAgentConfirmationCopy[\s\S]*staleProjectEditConfirmation[\s\S]*\? currentCommandConfirmationCopy[\s\S]*: pendingAgentConfirmationCopy/, "Director workbar must not promote generic current Agent commands over a visible right-rail confirmation card"));
checkMessage(requireWithin(directorMode, /const surfaceAgentTimelineEntries = showNewVideoStart[\s\S]*newVideoSurfaceAgentTimelineEntries[\s\S]*restoredAgentTimelineEntries[\s\S]*const pendingAgentConfirmation = useMemo[\s\S]*latestPendingAgentConfirmation\(surfaceAgentTimelineEntries\)[\s\S]*pendingAgentConfirmationCopy[\s\S]*displayedPendingAgentConfirmationCopy[\s\S]*newVideoPlanningTakingFocus[\s\S]*localProjectTakingFocus[\s\S]*activeProjectStatusView\.stage === "需要本地项目"[\s\S]*activeProjectStatusView\.stage === "需要保存位置"[\s\S]*projectEditConfirmationCanOverrideLocalProjectSetup[\s\S]*确认修改\|确认方式\|确认重排为\\s\*\\d\+\\s\*个镜头[\s\S]*displayedPendingAgentConfirmationForStatus[\s\S]*newVideoPlanningTakingFocus[\s\S]*\? ""[\s\S]*localProjectTakingFocus && !projectEditConfirmationCanOverrideLocalProjectSetup[\s\S]*\? ""[\s\S]*agentPendingAction \? "确认当前消息" : displayedPendingAgentConfirmationCopy[\s\S]*projectStatusViewWithPendingAgentConfirmation\([\s\S]*displayedPendingAgentConfirmationForStatus/, "Director workbar must keep save-location setup primary except when a visible project-edit confirmation owns the right rail"));
checkMessage(requireWithin(directorMode, /const \[agentVisiblePendingConfirmationLabel,\s*setAgentVisiblePendingConfirmationLabel\] = useState\(""\)[\s\S]*handleAgentVisiblePendingConfirmationLabelChange[\s\S]*setAgentVisiblePendingConfirmationLabel[\s\S]*const restoredAgentStagedPlanConfirmationCopy = restoredAgentStagedPlanPendingConfirmationLabel\(restoredAgentStagedPlanDraft\)[\s\S]*const pendingAgentConfirmationCopy = agentVisiblePendingConfirmationLabel[\s\S]*restoredAgentStagedPlanConfirmationCopy[\s\S]*pendingAgentConfirmationLabel\(pendingAgentConfirmation\)/, "Director workbar and surface gates must prefer the right rail's visible confirmation label before stale or hidden timeline confirmation state"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const composerIntentNeedsLocalProject = Boolean\([\s\S]*visibleComposerInputText[\s\S]*projectStatusView\?\.stage === "需要本地项目"[\s\S]*projectStatusView\?\.stage === "需要保存位置"[\s\S]*intentNeedsLocalProjectBeforeTooling\(visibleComposerInputText\)[\s\S]*composerReadyDraftDeferredToolStatus[\s\S]*先确认故事[\s\S]*const footerStatusCopy = composerReadyDraftDeferredToolIntent[\s\S]*composerReadyDraftDeferredToolStatus[\s\S]*composerIntentNeedsLocalProject[\s\S]*需要保存位置：发送后先确认保存位置[\s\S]*composerReferenceGenerationFromReferencePlan[\s\S]*识别为：生成参考图（等待确认）[\s\S]*composerPermissionContract[\s\S]*识别为：更新工作方式[\s\S]*hasVisibleComposerInput[\s\S]*识别为：\$\{composerIntentRoute\.label\}[\s\S]*消息里等待你确认：/, "Bottom composer must show staged tool, save-location, reference-generation, and work-mode statuses before normal visible-input intent labels"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const primaryOperationConfirmationLabel = !hasComposerInput && primaryLabel !== "发送"[\s\S]*projectRequiredForWorkflow[\s\S]*Boolean\(footerDirectActionBoundaryNotice\)[\s\S]*Boolean\(workflowCanConfirm\(workflow\)\)[\s\S]*Boolean\(agentActionEnvelope\)[\s\S]*\? primaryLabel[\s\S]*const emptyComposerConfirmationLabel = footerNewVideoDraftConfirmationLabel[\s\S]*currentTimelineConfirmationLabel[\s\S]*primaryOperationConfirmationLabel[\s\S]*const emptyComposerPendingConfirmationLabel = !hasVisibleComposerInput && !canContinuePendingNewVideoDraft[\s\S]*const activeFooterConfirmationLabel = footerNewVideoDraftConfirmationLabel[\s\S]*currentTimelineConfirmationLabel[\s\S]*primaryOperationConfirmationLabel[\s\S]*const localProjectSetupRecoveryTakesFooterFocus = Boolean\([\s\S]*minimalAgentMessageIncompleteLocalProjectSetup\(localProjectSetupNotice\)[\s\S]*!preparedProjectEditCanConfirmWithoutLocalProject[\s\S]*const footerStatusCopy = composerReadyDraftDeferredToolIntent[\s\S]*localProjectSetupRecoveryTakesFooterFocus[\s\S]*activeFooterConfirmationLabel[\s\S]*emptyComposerPendingConfirmationLabel[\s\S]*localProjectSetupConfirmationContextActive[\s\S]*可点上方「选择保存位置」，也可以直接写要改哪里。[\s\S]*要修改就直接输入；确认在上方消息里。[\s\S]*消息里等待你确认：\$\{activeFooterConfirmationLabel\}/, "Pending confirmations must keep the footer pointed at real confirmation labels while save-location copy stays concrete, visibly empty composer state disables Send, and generic next-step suggestions stay conversational"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const userIntentIsPermissionControlOnly = isDirectorAgentPermissionControlOnlyIntent\(userIntent\)[\s\S]*const userIntentNeedsLocalProject = !userIntentIsPermissionControlOnly[\s\S]*!localProjectReadyForTools[\s\S]*projectRequiredForWorkflow[\s\S]*projectBlockedWithoutFooterResolver[\s\S]*projectStatusView\?\.stage === "需要本地项目"[\s\S]*projectStatusView\?\.stage === "需要保存位置"[\s\S]*intentNeedsLocalProjectBeforeTooling\(userIntent\)[\s\S]*if \(userIntentNeedsLocalProject\)[\s\S]*rememberLocalProjectBlockForIntent\(userIntent\)/, "Prepared Agent turns must use the visible save-location status before staging generation confirmations while pure work-mode commands bypass local-project blockers"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const preparedProjectEditCanConfirmWithoutLocalProject = Boolean\([\s\S]*agentActionIsProjectDraftEdit\(agentActionEnvelope\)[\s\S]*workflowCanConfirm\(workflow\)[\s\S]*if \(projectRequiredForWorkflow && !preparedProjectEditCanConfirmWithoutLocalProject\)/, "Prepared story or shot edits must not let save-location become the primary action"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function latestWaitingReferenceGenerationConfirmationActionId\(entries: VibeAgentTimelineEntry\[\]\)[\s\S]*prepare_reference_generation[\s\S]*return entry\.actionId[\s\S]*const projectStatusWaitingForReferenceConfirmation = projectStatusView\?\.stage === "等待你确认"[\s\S]*确认生成参考[\s\S]*const timelineReferenceGenerationActionId = projectStatusWaitingForReferenceConfirmation[\s\S]*latestWaitingReferenceGenerationConfirmationActionId\(agentTimelineEntries\)[\s\S]*const restoredReferenceGenerationActionId = restoredAgentStagedPlanDraft\?\.status === "active"[\s\S]*timelineReferenceGenerationActionId/, "Right Agent confirmation focus must fall back to the latest active reference confirmation when the main project status says it is still waiting"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /newVideoDraftTimelineCopy[\s\S]*visibleAgentTimelineEntries\.slice\(-6\)[\s\S]*newVideoDraftStatusCopy[\s\S]*newVideoDraftTimelineCopy/, "Right Agent composer must also use the focused Agent timeline as a ready-draft confirmation source"));
checkMessage(requireWithin(minimalAgentPanelSource, /const referencePlanningContextActive = referencePlanningFocusActive \|\| referencePlanningGenerationIntentActive[\s\S]*const storyReferenceDeferredFocusActive = Boolean\([\s\S]*projectStatusView\?\.stage === "故事已保存"[\s\S]*const storyReferencePlanOnlyIntentActive = Boolean\([\s\S]*!referencePlanningGenerationRequestText\(referencePlanningIntentText\)[\s\S]*const storyReferencePlanningIntentActive = Boolean\([\s\S]*projectStatusView\?\.stage === "故事已保存" \|\| storyReferencePlanOnlyIntentActive[\s\S]*const composerVideoIntentShouldConfirmReferencesFirst = Boolean\([\s\S]*intentRequestsVideoSubmitWork\(referencePlanningIntentText\)[\s\S]*const baseStoryLevelReferenceContextActive = referencePlanningContextActive[\s\S]*storyReferenceDeferredFocusActive[\s\S]*storyReferencePlanningIntentActive[\s\S]*composerVideoIntentShouldConfirmReferencesFirst[\s\S]*const storyLevelReferenceContextActive = baseStoryLevelReferenceContextActive \|\| pendingReferenceGenerationContextActive/, "Right Agent selection summary must keep plan-only reference focus, send-video reference preflights, and pending reference confirmations active as story-level context"));
checkMessage(requireWithin(minimalAgentPanelSource, /const savedStoryIdleReferenceFocusActive = Boolean\([\s\S]*referencePlanningFocusActive[\s\S]*savedStoryReferencePlanStatusActive[\s\S]*projectStatusView\?\.stage === "故事已保存"[\s\S]*storyShotCountRevisionPlaceholderActive = Boolean\([\s\S]*!newStoryComposerScopeActive[\s\S]*pendingStoryShotCountRevisionConfirmationCount[\s\S]*\^确认重排[\s\S]*const inputPlaceholder = newStoryComposerScopeActive[\s\S]*继续说这个新视频怎么拍[\s\S]*storyShotCountRevisionPlaceholderActive[\s\S]*继续说这次重排怎么改[\s\S]*localProjectSetupConfirmationContextActive[\s\S]*storyLevelReferenceContextActive[\s\S]*savedStoryIdleReferenceFocusActive[\s\S]*继续修改故事，或说“开始补参考”[\s\S]*继续说参考怎么安排/, "Right Agent empty composer placeholder must keep fresh new-story and pending-draft prompts ahead of shot-count revision placeholders, then let real shot-count confirmations override save-location copy and offer both story edits and reference entry after a saved story"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const localProjectSetupConfirmationContextActive = pendingTimelineConfirmationLabelForContext === "选择保存位置"[\s\S]*localProjectBusy[\s\S]*!localProjectReady[\s\S]*runtimeState\.storyFlow\.shots\.length > 0[\s\S]*projectStatusView\?\.stage === "需要保存位置"/, "Right Agent save-location busy state must stay scoped to the current story instead of falling back to the selected shot"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const statusReadyNewVideoDraftConfirmation = Boolean\([\s\S]*projectStatusView\?\.stage === "等待确认" \|\| projectStatusView\?\.stage === "草案待确认"[\s\S]*确认\|草案\|故事流\|写入故事流\|保存到项目\|加入项目计划/, "Footer must recognize ready new-video drafts from the visible project status summary"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const runExecution = async \(input:[\s\S]*runSharedAgentVideoExecution\(\{[\s\S]*executionMode:\s*input\.live \? "live" : "dry_run"[\s\S]*const productAdapter = buildMinimalAgentProductAdapter\(\{[\s\S]*availability:\s*currentAgentToolAvailability\(action\)[\s\S]*createReferences:\s*\(target\) => runExecution\(\{[\s\S]*executionAction:\s*"prepare_references"[\s\S]*submitVideo:\s*\(target\) => runExecution\(\{[\s\S]*executionAction:\s*"submit_video"[\s\S]*queryVideo:\s*\(target\) => runExecution\(\{[\s\S]*operation:\s*"query"[\s\S]*runExport:\s*\(target\) => runExecution\(\{[\s\S]*executionAction:\s*"export"[\s\S]*productAdapter\.runConfirmedAction\(\{[\s\S]*action,[\s\S]*userIntent,[\s\S]*preparedHandoff/, "MinimalAgentPanel confirmed execution must wrap all product callbacks through the shared Agent/footer execution adapter before delegating policy and handoff to the UI Agent product adapter"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /onVisiblePendingConfirmationLabelChange\?: \(label: string\) => void[\s\S]*const activeConfirmationActionLabel = activeConfirmationMessage[\s\S]*agentCurrentTaskProjection\.label[\s\S]*const visiblePendingConfirmationLabelForParent = hasVisibleComposerInput[\s\S]*composerEditingConfirmationLabel \|\| \(composerEditingPendingConfirmation \? agentCurrentTaskProjection\.label : ""\)[\s\S]*: activeConfirmationActionLabel[\s\S]*useEffect\(\(\) => \{[\s\S]*onVisiblePendingConfirmationLabelChange\?\.\(visiblePendingConfirmationLabelForParent\)[\s\S]*return \(\) => onVisiblePendingConfirmationLabelChange\?\.\(""\)/, "Right Agent must publish the projected confirmation label to the parent workbar while editing the confirmation"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const localProjectSetupComposerIntentActive = Boolean\([\s\S]*projectStatusView\?\.stage === "需要保存位置"[\s\S]*intentNeedsLocalProjectBeforeTooling\(text\)[\s\S]*const localProjectSetupConfirmationContextActive = pendingTimelineConfirmationLabelForContext === "选择保存位置"[\s\S]*localProjectSetupComposerIntentActive[\s\S]*const composerReferenceGenerationSelectionChips = composerReferenceGenerationFromReferencePlan && !composerIntentNeedsLocalProject/, "Unsaved confirmed-story tool requests must keep save-location context ahead of reference-generation selection chips"));
...
checkMessage(requireWithin(directorMode, /onEditingPendingConfirmationChange=\{handleAgentEditingPendingConfirmationChange\}[\s\S]*onVisiblePendingConfirmationLabelChange=\{handleAgentVisiblePendingConfirmationLabelChange\}/, "DirectorMode must receive visible confirmation-editing state and the active confirmation label from the right Agent composer"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /newVideoResetKey\?: number[\s\S]*newVideoResetKeyRef[\s\S]*useEffect\(\(\) => \{[\s\S]*newVideoResetKeyRef\.current === newVideoResetKey[\s\S]*setText\(""\)[\s\S]*setAgentTimelineEntries\(\[\]\)[\s\S]*setActiveComposerTurnIntent\(""\)[\s\S]*onPendingAgentActionChange\?\.\(false\)[\s\S]*onVisiblePendingConfirmationLabelChange\?\.\(""\)/, "MinimalAgentPanel reset must clear abandoned right-rail user turns, messages, status, pending action state, and visible confirmation labels"));
