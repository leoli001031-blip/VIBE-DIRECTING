...
checkMessage(requireWithin(minimalAgentPanelSource, /footerActionConfirmationMessage[\s\S]*agentThreadMessages = \[\.\.\.agentThreadMessages, footerActionConfirmationMessage\]/, "MinimalAgentPanel must keep the active Agent confirmation card visible after message compaction"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentConfirmationBoundary\(message: MinimalAgentMessage\)[\s\S]*这次只补[\s\S]*会生成参考图[\s\S]*结果保存到[\s\S]*确认前再核对/, "Agent confirmation boundary should read as one natural pre-confirmation sentence, not a debug fact list"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /latestConfirmationMessageId[\s\S]*minimalAgentConfirmationAction\(message,\s*primaryLabel\)[\s\S]*const confirmationIsNewVideoDraftAction = message\.id\.startsWith\("footer_action_new_video_draft"\) \|\| visibleNewVideoDraftConfirmation[\s\S]*const confirmationUsesPrimaryAction = !confirmationIsNewVideoDraftAction[\s\S]*confirmationMatchesPrimaryAction[\s\S]*confirmationIsFooterAction[\s\S]*const confirmationDisabled = confirmationIsNewVideoDraftAction[\s\S]*const confirmationButtonLabel = confirmationIsNewVideoDraftAction[\s\S]*confirmationAction\.label[\s\S]*confirmationUsesPrimaryAction \? confirmationAction\.label : "继续确认"[\s\S]*const confirmationButtonHint = confirmationIsNewVideoDraftAction[\s\S]*confirmationAction\.hint[\s\S]*confirmationUsesPrimaryAction[\s\S]*这条确认来自历史消息[\s\S]*const runConfirmationAction = \(\) => \{[\s\S]*confirmationIsNewVideoDraftAction[\s\S]*confirmDraftFromAgent[\s\S]*confirmationUsesPrimaryAction[\s\S]*handleNext\(\)[\s\S]*prepareChange\(agentMessageConfirmationIntent\(message,\s*confirmationAction\.label\),\s*currentComposerSelectionOverride\(\)\)[\s\S]*onClick=\{runConfirmationAction\}[\s\S]*disabled=\{confirmationDisabled\}[\s\S]*\{confirmationButtonLabel\}/, "Latest Agent confirmation message must expose its own concrete confirmation action without inheriting unrelated footer actions; draft confirmations use the draft bridge and recovered confirmations visibly re-enter the Agent as explicit confirmation intent"));
