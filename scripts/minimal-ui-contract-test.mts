...
checkMessage(requireWithin(creatorDeskPanelsSource, /browserDraftLabel = hasStoryDraftForProject \? projectRequirement\.label : "先写想法"[\s\S]*nextActionCopy = statusNextAction \|\| \(!localProjectReady[\s\S]*browserDraftLabel/, "CreatorDeskPanels should point confirmed browser stories to local-project setup instead of saying to write another idea"));
...
checkMessage(requireWithin(creatorDeskPanelsSource, /const projectInboxDefaultOpen = !videoTaskActive[\s\S]*projectInbox\.needsReviewCount > 0/, "CreatorDeskPanels must collapse the full material inbox while a video task is active"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const footerStatusCopy = composerInputUsesNextAction[\s\S]*识别为继续[\s\S]*识别为：\$\{composerIntentRoute\.label\}[\s\S]*建议动作：/, "Bottom composer must explain send input, typed continue, and the current next action"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const footerPrimaryLabel = hasComposerInput[\s\S]*footerPrimaryUsesAgentNext[\s\S]*\? "确认"[\s\S]*: primaryLabel/, "Bottom composer next-action button must stay generic: confirm in the button, concrete action in status copy"));
checkMessage(requireWithin(minimalAgentPanelSource, /footerPrimaryUsesAgentNext \? `确认：\$\{primaryAriaLabel\}` : sendAriaLabel/, "Bottom composer must keep the concrete next action in the accessible label"));
...
checkMessage(requireWithin(creatorDeskPanelsSource, /creator-status-details creator-agent-reasoning[\s\S]*AI 导演怎么判断[\s\S]*creator-preflight-strip[\s\S]*creator-desk-details/, "Creator desk must put process and full details behind one Agent reasoning disclosure"));
...
checkMessage(requireWithin(creatorDeskPanelsSource, /const projectInboxDefaultOpen = !videoTaskActive && projectInbox\.needsReviewCount > 0 && reviewShortcutItems\.length === 0[\s\S]*key=\{`project-inbox-\$\{projectInboxDefaultOpen \? "open" : "closed"\}`\}[\s\S]*open=\{projectInboxDefaultOpen\}/, "Creator desk should collapse the full material inbox when a shorter review shortcut is available or video is active"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const footerPrimaryUsesAgentNext = \(!hasComposerInput[\s\S]*composerInputUsesNextAction/, "MinimalAgentPanel must fold suggested actions and typed continue into one bottom primary button"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /onClick=\{hasComposerInput && !composerInputUsesNextAction \? handleSend : handleNext\}/, "MinimalAgentPanel primary action must either send text or continue the next action"));
...
checkMessage(requireWithin(minimalTopNav, /aria-label="预览"[\s\S]*>\s*预览\s*</, "Top content menu preview view must use product copy"));
