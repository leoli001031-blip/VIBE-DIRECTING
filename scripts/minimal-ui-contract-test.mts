...
checkMessage(requireWithin(newVideoStartSource, /confirmedFlowTitle[\s\S]*确认后只保存故事[\s\S]*确认后先补参考[\s\S]*确认后进入可提交视频/, "NewVideoStart organized draft must explain the post-confirm next step for each execution boundary"));
checkMessage(requireWithin(newVideoStartSource, /confirmedFlowDetail[\s\S]*补参考、发视频和导出都等你再说[\s\S]*不会发送视频[\s\S]*按消息确认提交视频/, "NewVideoStart organized draft must keep post-confirm copy aligned with the execution boundary"));
{
...
checkMessage(requireWithin(minimalAgentPanelSource, /minimalAgentAssetInboxSummaryFromTimelineEntry[\s\S]*handlingLabels[\s\S]*bindingPreviewLabels[\s\S]*foldedDetailLabels[\s\S]*assetInboxSummary[\s\S]*aria-label="素材识别摘要"[\s\S]*类型[\s\S]*待确认[\s\S]*边界[\s\S]*先确认再写入项目[\s\S]*处理方式[\s\S]*建议绑定[\s\S]*并入镜头/, "Agent material scanning must explain recognized materials, handling, confirmation boundary, and binding suggestions in the message flow"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /newVideoAgentSelectionContext\?: \{[\s\S]*title: string[\s\S]*chips: Array<\{ label: string; value: string \}>[\s\S]*newVideoAgentSelectionContext \|\| explicitAgentSelectionContextFromTimeline[\s\S]*mergeVibeAgentTimelineEntries\(agentTimelineEntries, restoredAgentTimelineEntries \|\| \[\]\)[\s\S]*shouldUseExplicitTimelineSelectionContext = Boolean\(explicitTimelineSelectionContext\)[\s\S]*visibleCompactSelectionHint[\s\S]*selectionContextTitle[\s\S]*displayedSelectionChips/, "Right Agent selection summary must prioritize explicit new-video selections and fall back to merged local/restored Agent timeline context"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /AI 导演权限[\s\S]*更改 AI 导演权限[\s\S]*选择权限[\s\S]*AI 导演权限：\$\{item\.label\}/, "Agent execution boundary controls must read as AI director permissions instead of workflow scope"));
