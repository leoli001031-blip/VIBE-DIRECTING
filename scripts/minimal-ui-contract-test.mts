...
checkMessage(requireWithin(directorModeSource, /const runtimeShotIds = new Set\(runtimeState\.storyFlow\.shots\.map[\s\S]*const scopedShots = directorView === "story" \|\| !activeSection[\s\S]*\? runtimeState\.storyFlow\.shots[\s\S]*runtimeState\.storyFlow\.shots\.filter[\s\S]*const\s+projectNavShotCount\s*=\s*showNewVideoStart \? \(newVideoStatus\?\.draftShotCount \|\| restoredNewVideoDraft\?\.shotCount \|\| 0\) : runtimeState\.storyFlow\.shots\.length[\s\S]*totalShots=\{projectNavShotCount\}/, "Director project rail, story scope, and Agent state must share the active draft/project shot count instead of mixing stale audit counts"));
