...
checkMessage(requireWithin(directorMode, /const pendingAgentConfirmation = useMemo[\s\S]*latestPendingAgentConfirmation\(restoredAgentTimelineEntries\)[\s\S]*pendingAgentConfirmationCopy[\s\S]*projectStatusViewWithPendingAgentConfirmation\([\s\S]*agentPendingAction \? "确认当前消息" : pendingAgentConfirmationCopy/, "Director workbar must sync restored timeline confirmations into the displayed project status"));
