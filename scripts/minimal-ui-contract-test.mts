...
const draftRevisionIntentPath = "src/core/draftRevisionIntent.ts";
...
const draftRevisionIntentSource = stripComments(readText(draftRevisionIntentPath));
const minimalAgentPanelBody = findFunctionBody(minimalAgentPanelSource, "MinimalAgentPanel");
const minimalAgentPanelCollapsedReturnIndex = minimalAgentPanelBody.indexOf("if (isComposerCollapsed)");
const minimalAgentPanelLastHookIndex = Math.max(
  ...["useState(", "useMemo(", "useEffect(", "useCallback(", "useRef("]
    .map((hook) => minimalAgentPanelBody.lastIndexOf(hook)),
);
assert(
  minimalAgentPanelCollapsedReturnIndex > minimalAgentPanelLastHookIndex,
  "Agent collapse must not return before all React hooks have run",
);
...
checkMessage(requireWithin(directorModeSource, /function\s+ProjectStatusSummary[\s\S]*aria-label="项目事实概览"[\s\S]*当前对象[\s\S]*project-status-summary-facts/, "Center workbar must present passive project facts instead of a second current-task narrator"));
check(!/function\s+ProjectStatusSummary[\s\S]*project-status-summary-next[\s\S]*currentTask\?\.label/.test(directorModeSource), "Center workbar must not repeat the right-rail current task or action");
...
checkMessage(requireWithin(directorModeSource, /aria-label=\{`故事，\$\{totalShots\} 镜头`\}[\s\S]*aria-label=\{`参考，\$\{referenceLabel\}`\}[\s\S]*aria-label=\{`视频，\$\{videoLabel\}`\}[\s\S]*aria-label="交付，展示包"/, "Collapsed project navigation must preserve accessible names for every icon-only destination"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const preserveReviewRegenerationHandoff = agentGenerationProjectIdentity\.projectRoot[\s\S]*agentDirectorReviewRegenerationConfirmationMatchesRestoredLedger\([\s\S]*const refreshedToolHandoff = preserveReviewRegenerationHandoff[\s\S]*\? draft\.toolHandoff[\s\S]*: buildVibeAgentToolHandoff\(\{[\s\S]*action: draft\.action[\s\S]*userConfirmed: false[\s\S]*availability: currentAgentToolAvailability\(draft\.action\)[\s\S]*setAgentToolHandoff\(refreshedToolHandoff\)/, "Restored Agent confirmations must preserve exact persisted-ledger regeneration handoffs and refresh all other handoffs from action-specific live availability before showing blockers"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const confirmationPointerHandledRef = useRef\(false\)[\s\S]*const handleConfirmationPointerDown = \(event: PointerEvent<HTMLButtonElement>\) => \{[\s\S]*confirmationPointerHandledRef\.current = true[\s\S]*event\.preventDefault\(\)[\s\S]*runConfirmationAction\(\)[\s\S]*const handleConfirmationMouseDown = \(event: MouseEvent<HTMLButtonElement>\) => \{[\s\S]*confirmationPointerHandledRef\.current[\s\S]*runConfirmationAction\(\)[\s\S]*const handleConfirmationClick = \(\) => \{[\s\S]*confirmationPointerHandledRef\.current = false[\s\S]*return;[\s\S]*runConfirmationAction\(\)[\s\S]*const handleConfirmationKeyDown = \(event: KeyboardEvent<HTMLButtonElement>\) => \{[\s\S]*event\.key !== "Enter"[\s\S]*event\.key !== " "[\s\S]*event\.preventDefault\(\)[\s\S]*runConfirmationAction\(\)[\s\S]*onPointerDown=\{handleConfirmationPointerDown\}[\s\S]*onMouseDown=\{handleConfirmationMouseDown\}[\s\S]*onClick=\{handleConfirmationClick\}[\s\S]*onKeyDown=\{handleConfirmationKeyDown\}/, "Agent confirmation buttons must execute from pointer, mouse, or keyboard activation for packaged Computer Use while deduplicating the following click"));
...
checkMessage(requireWithin(draftRevisionIntentSource, /function removalTargetsFromText[\s\S]*不要再提\|去掉\|移除\|删掉\|删除[\s\S]*\(\?!让\|使\|把\|将\|完整\|完全\|立刻\|立即\|马上\)[\s\S]*function itemChangeFact[\s\S]*item\.operation === "remove_content"[\s\S]*去掉\$\{item\.removalTargets\.join\("、"\)\}/, "Ready draft target-shot preview must identify removal-only phrases such as 结尾那镜不要月亮 without treating temporal or action-preservation feedback as deletion"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const stagedAgentTimelineEntries = stagedAgentPlan\?\.agentTimelineEntries\?\.length[\s\S]*stagedAgentPlan\.agentActionEnvelope\.status === nextAgentActionEnvelope\.status[\s\S]*const preparedTimelineEntries = stagedAgentActionMatchesChosen[\s\S]*agentTimelineEntriesForCurrentUserIntent\(stagedAgentTimelineEntries, userIntent\)[\s\S]*\? stagedAgentTimelineEntries[\s\S]*: buildLocalPreparedAgentTimelineEntries\(\{[\s\S]*userIntent,[\s\S]*action: nextAgentActionEnvelope[\s\S]*setAgentTimelineEntries\(activeDirectorClarificationTurn[\s\S]*mergeVibeAgentTimelineEntries\(agentTimelineEntries, preparedTimelineEntries\)/, "A new typed Agent plan must render a local current-turn message and preserve the clarification context that produced it"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const savedStoryIdleReferenceFocusActive = Boolean\([\s\S]*referencePlanningFocusActive[\s\S]*savedStoryReferencePlanStatusActive[\s\S]*storyShotCountRevisionPlaceholderActive = Boolean\([\s\S]*!newStoryComposerScopeActive[\s\S]*pendingStoryShotCountRevisionConfirmationCount[\s\S]*restoredAgentStagedPlanDraft\.action\?\.kind === "revise_story_or_shot"[\s\S]*const inputPlaceholder = effectiveReviewTarget\?\.status === "needs_review"[\s\S]*说明 \$\{effectiveReviewTarget\.label\}[\s\S]*newStoryComposerScopeActive[\s\S]*继续说这个新视频怎么拍[\s\S]*storyShotCountRevisionPlaceholderActive[\s\S]*继续说这次重排怎么改[\s\S]*localProjectSetupConfirmationContextActive[\s\S]*storyLevelReferenceContextActive[\s\S]*savedStoryIdleReferenceFocusActive[\s\S]*继续修改故事，或说“开始补参考”[\s\S]*继续说参考怎么安排/, "Right Agent empty composer placeholder must prioritize structured review context, then saved-story and shot-edit state before save-location and reference prompts"));
...
checkMessage(requireWithin(directorMode, /function startNewVideoFromAgent[\s\S]*setAgentIntakeCommand\(\{[\s\S]*projectTargetMode:\s*agentNewVideoProjectTargetMode\(\{[\s\S]*localProjectReady:\s*folderReady[\s\S]*currentProjectShotCount:\s*runtimeState\.storyFlow\.shots\.length/, "Right Agent fresh-video requests must reuse only an explicitly connected empty local project and preserve projects that already have shots"));
...
checkMessage(requireWithin(minimalExportSource, /exportCompleted\?:\s*boolean[\s\S]*&& !exportCompleted[\s\S]*exportPackageReady = exportCompleted[\s\S]*已写入 \$\{exportWorker\?\.exportRoot \|\| "exports"\}，可以复核交付内容/, "Export view must render a restored live completion as delivered instead of asking for another confirmation"));
...
checkMessage(requireWithin(directorModeSource, /function projectStatusViewWithCompletedExport[\s\S]*currentTask\?\.completion\?\.step !== "export"[\s\S]*currentTask\.completion\.executionMode !== "live"[\s\S]*stage:\s*"导出已完成"/, "DirectorMode must derive delivery status from the structured live export completion"));
checkMessage(requireWithin(directorMode, /<MinimalExport[\s\S]*exportCompleted=\{agentCurrentTaskProjection\?\.completion\?\.step === "export"/, "DirectorMode must pass the restored live export completion into the delivery page"));
...
checkMessage(requireWithin(minimalPreviewSource, /onActiveReviewTargetChange\?:[\s\S]*onActiveReviewTargetChange\?\.\(activeReviewTarget\)/, "Preview must publish its receipt-bound active review target to the right-side Director turn"));
check(!/onApprovePreviewItem|className="preview-review-action"/.test(minimalPreviewSource), "Preview artifact workspace must not expose a second approval shortcut");
...
checkMessage(requireWithin(appSource, /const stagedPlanRestore = agentStagedPlanRestoreResultForTimeline\(stagedPlanOpen,\s*timelineOpen\.timeline\.entries\)[\s\S]*if \(!stagedPlanRestore\.ok && stagedPlanRestore\.status === "cleared" && stagedPlanOpen\.draft\?\.status === "active"\)[\s\S]*clearProjectAgentStagedPlanDraft\(prototypeProjectDraftTarget[\s\S]*setRestoredAgentStagedPlanDraft\(projectAgentStagedPlanDraftForProjection\(stagedPlanRestore\)\)/, "App must persist only superseded active sidecars while retaining a real cleared marker for current-task projection"));
check((appSource.match(/agentStagedPlanRestoreResultForTimeline\(stagedPlanOpen,\s*(?:timelineOpen\.timeline|agentTimelineForRestore)\.entries\)/g) || []).length >= 3, "Browser-draft and local-project staged-plan restore paths must filter superseded Agent plans before and after interrupted-export recovery");
checkMessage(requireWithin(appSource, /const runtimeDraftTarget: ProjectVibeDraftTarget = \{[\s\S]*projectRoot: runtimeProjectBinding\.projectRoot[\s\S]*async function restoreRuntimeAgentTimeline\(\)[\s\S]*openProjectVibeDraft\(runtimeDraftTarget\)[\s\S]*applyProjectVibeProjectState\(projectOpen\.project,\s*runtimeDraftTarget\)[\s\S]*openProjectAgentTimeline\(runtimeDraftTarget[\s\S]*if \(timelineOpen\.ok\)[\s\S]*const stagedPlanOpen = await openProjectAgentStagedPlanDraft\(runtimeDraftTarget[\s\S]*const actionLogOpen = await openProjectAgentActionLog\(runtimeDraftTarget[\s\S]*const generationLedgerOpen = await openProjectAgentGenerationJobLedger\([\s\S]*const agentTimelineForRestore = interruptedExportRecovery\?\.timeline \|\| timelineOpen\.timeline[\s\S]*const generationLedgerForRestore = interruptedExportRecovery\?\.ledger \|\| generationLedgerOpen\.ledger[\s\S]*const stagedPlanRestore = agentStagedPlanRestoreResultForTimeline\(stagedPlanOpen,\s*agentTimelineForRestore\.entries\)[\s\S]*clearProjectAgentStagedPlanDraft\(runtimeDraftTarget[\s\S]*setRestoredAgentStagedPlanDraft\(projectAgentStagedPlanDraftForProjection\(stagedPlanRestore\)\)[\s\S]*setRestoredAgentTimelineEntries\(timelineOpen\.ok \? agentTimelineForRestore\.entries : \[\]\)[\s\S]*setRestoredAgentGenerationJobLedger\(generationLedgerOpen\.ok \? generationLedgerForRestore : undefined\)/, "runtime Agent recovery must apply Project.vibe first, then restore timeline, retained cleared marker, and generation ledger independently through interrupted-export recovery"));
...
for (const [term, pattern] of phase2123ForbiddenMainTerms) {
  const source = term === "hash" ? extractStringLiterals(phase2123DirectorSurface) : phase2123DirectorSurface;
...
checkMessage(requireWithin(minimalAgentPanelSource, /data-director-turn-phase=\{agentDirectorTurnProjection\.phase\}[\s\S]*aria-label="当前导演澄清"[\s\S]*本轮 · Clarify[\s\S]*conversation_only[\s\S]*导演意图选项[\s\S]*我再描述/, "The Agent rail must render the clarification turn as a structured conversation-only choice"));
checkMessage(requireWithin(minimalAgentPanelSource, /aria-label="当前导演提案"[\s\S]*本轮 · Proposal[\s\S]*staged_only[\s\S]*aria-label="提案身份"[\s\S]*aria-label="提案改动"[\s\S]*确认写入项目[\s\S]*继续调整/, "The Agent rail must render proposal identity, proposed changes, and explicit write-or-revise actions"));
checkMessage(requireWithin(minimalAgentPanelSource, /clarificationTurnVisible[\s\S]*proposalTurnVisible[\s\S]*paidConfirmationTurnVisible[\s\S]*runningTurnVisible[\s\S]*reviewTurnVisible[\s\S]*aria-describedby=\{!focusedTurnVisible && showComposerMaterialIntakeHint[\s\S]*clarificationTurnVisible \? <>[\s\S]*不会写项目[\s\S]*proposalTurnVisible \? <>[\s\S]*不会调用外部生成服务[\s\S]*paidConfirmationTurnVisible \? <>[\s\S]*不会执行当前任务[\s\S]*runningTurnVisible \? <>[\s\S]*不会重复提交或自动重试/, "Focused Director turns must own the composer footer and keep confirmation/running boundaries explicit"));
checkMessage(requireWithin(minimalAgentPanelSource, /aria-label="当前生成任务确认"[\s\S]*data-confirmation-id=\{paidConfirmationProjection\?\.confirmationId\}[\s\S]*本轮 · Confirmation[\s\S]*aria-label="确认任务身份"[\s\S]*aria-label="本次执行范围"[\s\S]*confirmPaidGenerationFromAgentTurn[\s\S]*revisePaidGenerationFromAgentTurn/, "The Agent rail must render exact paid-confirmation identity, facts, confirm-once, and revise actions"));
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentActionContractFacts[\s\S]*"模型"[\s\S]*"清晰度"[\s\S]*function minimalAgentConfirmationReadableBody[\s\S]*const modelFact = minimalAgentFactValue\(message, \["模型"\]\)[\s\S]*const resolutionFact = minimalAgentFactValue\(message, \["清晰度"\]\)[\s\S]*message\.executionMode === "live"[\s\S]*message\.actionKind === "prepare_video_submit"[\s\S]*提交 1 次 \$\{modelFact\} \$\{resolutionFact\} 外部视频任务/, "Live paid video confirmations must preserve the exact model and resolution in both facts and readable body"));
checkMessage(requireWithin(minimalAgentPanelSource, /aria-label="当前运行任务"[\s\S]*data-job-id=\{runningProjection\.jobId\}[\s\S]*本轮 · Running[\s\S]*aria-label="运行任务身份"[\s\S]*aria-label="执行进度"[\s\S]*backgroundCurrentAgentJob[\s\S]*inspectCurrentAgentJob/, "The Agent rail must render one existing job with observation-only Running actions"));
checkMessage(requireWithin(minimalAgentPanelSource, /minimalAgentReviewTargetFromGenerationJob[\s\S]*selectLatestAgentVideoGenerationReviewJob[\s\S]*minimalAgentReviewTargetsMatch[\s\S]*activeDirectorReviewJob[\s\S]*reviewJob: activeDirectorReviewJob/, "The Agent rail must restore and validate Review from one exact succeeded generation job"));
checkMessage(requireWithin(minimalAgentPanelSource, /aria-label="当前视频复核"[\s\S]*本轮 · Review[\s\S]*aria-label="复核对象身份"[\s\S]*通过预览[\s\S]*需要修改[\s\S]*只写入复核记录，不会晋级项目事实，不会导出/, "The Agent rail must render returned media as a bounded needs_review decision instead of auto-approving, promoting, or exporting it"));
checkMessage(requireWithin(minimalAgentPanelSource, /reviewRevisionTurnVisible[\s\S]*aria-label="当前视频修改意图"[\s\S]*原结果[\s\S]*保持不变[\s\S]*新任务[\s\S]*尚未创建/, "A Review revision must visibly remain conversation-only before Clarify forms a Proposal"));
checkMessage(requireWithin(minimalAgentPanelSource, /displayedProposalProjection = reviewRegenerationProposalProjection \|\| proposalProjection[\s\S]*aria-label="当前导演提案"[\s\S]*修改提案已形成[\s\S]*reviewRegenerationProposalProjection\.proposalId/, "The Agent rail must render the D6 regeneration Proposal with its own proposal identity"));
checkMessage(requireWithin(minimalAgentPanelSource, /const displayedCurrentTaskLabel = clarificationTurnVisible[\s\S]*确认导演意图[\s\S]*proposalTurnVisible[\s\S]*proposalConfirmAction\?\.label[\s\S]*reviewRevisionTurnVisible[\s\S]*说明修改方向[\s\S]*<strong>\{displayedCurrentTaskLabel\}<\/strong>/, "The focused D6 turn must own the visible current-task label instead of leaving the old Review task active"));
checkMessage(requireWithin(minimalAgentPanelSource, /const focusedTaskOwnsVisibleContext = clarificationTurnVisible[\s\S]*proposalTurnVisible[\s\S]*reviewRevisionTurnVisible[\s\S]*reviewRegenerationConfirmationTurnVisible[\s\S]*focusedCompactScopeLabel[\s\S]*focusedCompactSelectionHint[\s\S]*focusedSelectionChips[\s\S]*<strong>\{focusedCompactScopeLabel\}<\/strong>[\s\S]*<p>\{focusedCompactSelectionHint\}<\/p>/, "The visible Agent header and context facts must follow the D6 focused turn instead of the old returned-video summary"));
checkMessage(requireWithin(minimalAgentPanelSource, /field === "directorIntent"[\s\S]*修改方向[\s\S]*field === "versionPolicy"[\s\S]*版本处理[\s\S]*field === "reviewGate"[\s\S]*新版本复核/, "D6 Proposal changes must use creator-facing field labels rather than raw contract keys"));
checkMessage(requireWithin(minimalAgentPanelSource, /entry\.details\?\.executionMode === "dry_run"[\s\S]*if \(message\.executionMode\) return message\.executionMode[\s\S]*本地验证 · 不计费[\s\S]*本轮只验证本地执行合同，不调用付费外部生成服务/, "The D6 Confirmation must stay visibly dry-run even when live Provider capabilities are installed"));
checkMessage(requireWithin(stylesSource, /minimal-agent-confirmation-actions[\s\S]*minimal-agent-running-actions[\s\S]*minimal-agent-running-progress/, "Paid Confirmation and Running must use the selected focused-turn action and progress system"));
checkMessage(requireWithin(stylesSource, /minimal-agent-confirmation-risk[\s\S]*minimal-agent-running-policy/, "Paid Confirmation and Running must expose distinct bounded state notices"));
