...
checkMessage(requireWithin(minimalAgentPanelSource, /const shouldAppendFooterActionConfirmationMessage = Boolean\([\s\S]*footerActionConfirmationMessage\.id\.startsWith\("footer_action_new_video_draft"\)[\s\S]*message\.id === footerActionConfirmationMessage\.id[\s\S]*message\.role === "confirmation"[\s\S]*fullAgentThreadMessages\.push\(footerActionConfirmationMessage\)/, "Ready draft fallback confirmation must be appended as a clickable current-turn message card, while other footer confirmations still avoid duplicates"));
