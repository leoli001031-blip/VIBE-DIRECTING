...
checkMessage(requireWithin(creatorDeskPanelsSource, /const agentConfirmationTakingFocus = projectStatusStage === "等待你确认"[\s\S]*primaryFlowTakingFocus = agentConfirmationTakingFocus \|\| exportFlowTakingFocus \|\| videoFlowTakingFocus[\s\S]*先处理右侧消息里的确认/, "CreatorDeskPanels must stop competing with the right-side Agent confirmation while a confirmation is pending"));
