...
checkMessage(requireWithin(directorMode, /visibleAgentCommand[\s\S]*允许生成参考[\s\S]*允许发送视频/, "DirectorMode must adapt CreatorDesk Agent guidance to the visible execution boundary"));
...
for (const label of ["从新视频开始", "和 AI 导演说", "主角参考", "风格参考", "场景参考", "道具参考", "添加文件", "拖入图片、声音或脚本", "AI 拆分", "确认时选文件夹"]) {
...
checkMessage(requireWithin(creatorDeskPanelsSource, /const displayProjectInboxSummary = videoTaskActive[\s\S]*个素材已在项目中[\s\S]*const displayProjectInboxNextAction = videoTaskActive[\s\S]*视频处理中，不需要再确认用途/, "CreatorDeskPanels must soften the material inbox copy while video is queued or generating"));
checkMessage(requireWithin(creatorDeskPanelsSource, /const displayIntentLabel = exportFlowTakingFocus[\s\S]*交付复核[\s\S]*videoTaskActive[\s\S]*查询视频[\s\S]*等待视频[\s\S]*当前意图：\{displayIntentLabel\}/, "CreatorDeskPanels must show export or video query/wait intent while that task is primary"));
...
checkMessage(requireWithin(creatorDeskProjectionSource, /status === "recoverable"[\s\S]*点消息里的「查询结果」，不会重复发送/, "Creator desk projection must route resumable videos back to the Agent message task"));
...
checkMessage(requireWithin(creatorDeskPanelsSource, /videoCanResume[\s\S]*点消息里的「查询结果」，不会重复发送/, "CreatorDeskPanels must explain resumable Seedance jobs without exposing a second query button"));
checkMessage(requireWithin(creatorDeskPanelsSource, /需要取回结果时，点消息里的查询任务[\s\S]*需要发送视频时，点消息里的确认任务/, "CreatorDeskPanels must route video execution to Agent message tasks"));
...
checkMessage(requireWithin(creatorDeskPanelsSource, /点消息里的「查询结果」[\s\S]*点消息里的确认发送下一段/, "CreatorDeskPanels must point users to Agent message tasks for execution"));
...
checkMessage(requireWithin(stylesSource, /\.director-bottom-composer \.minimal-agent-head\s*\{[\s\S]*display:\s*grid[\s\S]*\.director-bottom-composer \.minimal-agent-selection-context\s*\{[\s\S]*display:\s*flex/, "Bottom Agent composer must keep the AI identity and current target visible instead of becoming a contextless input box"));
checkMessage(requireWithin(stylesSource, /--director-bottom-composer-space:[\s\S]*\.minimal-director\.has-bottom-composer,[\s\S]*padding-bottom:\s*var\(--director-bottom-composer-space\)/, "Director workspace must reserve enough bottom space for the compact fixed composer"));
checkMessage(requireWithin(stylesSource, /--director-bottom-composer-scroll-space:[\s\S]*\.creator-desk-panels,[\s\S]*\.review-tray-item,[\s\S]*scroll-margin-bottom:\s*var\(--director-bottom-composer-scroll-space\)/, "Review actions must scroll above the compact fixed composer instead of being covered"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const footerStatusCopy = hasComposerInput[\s\S]*识别为：\$\{composerIntentRoute\.label\}[\s\S]*消息里有下一步：/, "Bottom composer must send typed input into the Agent and point empty-input next actions back to Agent messages"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /resumeAgentAfterLocalProjectSetupRef[\s\S]*!resumeAgentAfterLocalProjectSetupRef\.current \|\| !localProjectReadyForTools[\s\S]*prepareChange\("继续"\)/, "Completed local-project setup must automatically resume the Agent continue turn once the folder is ready"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /message\.id === latestLocalProjectReadyMessageId[\s\S]*prepareChange\("继续"\)[\s\S]*继续检查项目/, "Completed local-project setup messages must expose a one-click Agent continue action"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageCompletedToolAction[\s\S]*action_result[\s\S]*generate_references[\s\S]*submit_video[\s\S]*export_project/, "Completed tool action results must be detected from persisted action_result entries"));
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageBlockedToolAction[\s\S]*action_result[\s\S]*blocked[\s\S]*generate_references[\s\S]*submit_video[\s\S]*query_video[\s\S]*export_project/, "Blocked tool action results must be detected from persisted action_result entries"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function completedToolActionView[\s\S]*generate_references[\s\S]*assets[\s\S]*submit_video[\s\S]*preview[\s\S]*export_project[\s\S]*export/, "Completed tool action results must route to the right result surface"));
checkMessage(requireWithin(minimalAgentPanelSource, /latestCompletedToolActionMessageId[\s\S]*completedToolActionView\(message\)[\s\S]*onOpenResultView\?\.\(resultView\.view\)[\s\S]*prepareChange\("继续"\)[\s\S]*继续下一步/, "Latest completed Agent tool result must expose result-view and continue actions in the message flow"));
checkMessage(requireWithin(minimalAgentPanelSource, /latestBlockedToolActionMessageId[\s\S]*minimalAgentMessageBlockedToolAction[\s\S]*message\.id === latestBlockedToolActionMessageId[\s\S]*retryConfirmedAgentTool[\s\S]*继续修改/, "Latest blocked Agent tool result must expose retry and revise actions in the message flow"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function visibleMinimalAgentMessages[\s\S]*latestUserIndex[\s\S]*hiddenCount/, "MinimalAgentPanel must keep long persisted Agent timelines readable by showing the latest user turn"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /latestConfirmationMessageId[\s\S]*minimalAgentConfirmationAction\(message,\s*primaryLabel\)[\s\S]*onClick=\{handleNext\}[\s\S]*disabled=\{primaryDisabled\}[\s\S]*\{confirmationAction\.label\}/, "Latest Agent confirmation message must expose its own concrete confirmation action without inheriting the empty composer send state"));
...
checkMessage(requireWithin(minimalStoryFlow, /aria-label="AI 选择的做法"[\s\S]*AI 选择的做法/, "Story Flow must describe internal strategy selection as the AI chosen approach"));
