...
checkMessage(requireWithin(directorModeSource, /function projectStatusViewWithCommittedDraft[\s\S]*stage:\s*"故事流已更新"[\s\S]*草案已写入故事流/, "Director workbar must collapse already-confirmed draft state into a clear story-flow-updated status"));
...
checkMessage(requireWithin(directorMode, /<CreatorDeskPanels[\s\S]*projectStatusView=\{projectStatusView\}/, "Director story view must pass the unified project status into CreatorDeskPanels"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /footerSelectionTargetCopy[\s\S]*输入文字会引用：\$\{displayedCompactScopeLabel\}；上方确认按行动卡范围执行。[\s\S]*minimal-agent-footer-target/, "MinimalAgentPanel input footer must distinguish typed selection feedback from the current confirmation card scope"));
...
checkMessage(requireWithin(directorModeSource, /const runtimeShotIds = new Set\(runtimeState\.storyFlow\.shots\.map[\s\S]*const scopedShots = directorView === "story" \|\| !activeSection[\s\S]*\? runtimeState\.storyFlow\.shots[\s\S]*runtimeState\.storyFlow\.shots\.filter[\s\S]*totalShots=\{runtimeState\.storyFlow\.shots\.length\}/, "Director project rail, story scope, and Agent state must share runtimeState shot counts instead of mixing stale audit counts"));
...
checkMessage(requireWithin(directorModeSource, /referenceLabel=\{projectRailReferenceLabel\}/, "Project rail must receive a dedicated reference status label"));
...
checkMessage(requireWithin(directorModeSource, /videoLabel=\{projectRailVideoLabel\}/, "Project rail must receive a dedicated video status label"));
...
checkMessage(requireWithin(newVideoStart, /planSummaryActionHint[\s\S]*storyboardPlanningStatus === "running"[\s\S]*草案出来后可确认[\s\S]*确认写入故事流[\s\S]*new-video-next-hint[\s\S]*planSummaryActionHint/, "NewVideoStart plan summary must point to the current composer state instead of exposing a premature confirmation action"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /if \(activeNewVideoDraftConfirmation \|\| visibleNewVideoDraftConfirmation\) \{[\s\S]*label:\s*NEW_VIDEO_DRAFT_CONFIRM_LABEL[\s\S]*statusLine:\s*disabled \? disabledReason : `下一步：\$\{NEW_VIDEO_DRAFT_CONFIRM_LABEL\}`[\s\S]*onStartNewVideoDraftFromAgent\?\.\(NEW_VIDEO_DRAFT_CONFIRM_LABEL\)/, "Active new-video ready drafts must expose a right-rail confirmation action instead of only hint text"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const visibleConfirmationActionAvailable = agentNextActionAvailable[\s\S]*visibleNewVideoDraftConfirmation[\s\S]*Boolean\(visibleTimelineConfirmationMessage && !hasComposerInput && !isPreparingPlan\)[\s\S]*message\.role === "confirmation"[\s\S]*visibleConfirmationActionAvailable[\s\S]*const confirmationIsFooterAction = message\.id\.startsWith\("footer_action_"\)[\s\S]*const confirmationMatchesPrimaryAction = Boolean\([\s\S]*message\.actionId === agentActionEnvelope\.actionId[\s\S]*const confirmationUsesPrimaryAction = visibleNewVideoDraftConfirmation[\s\S]*confirmationMatchesPrimaryAction[\s\S]*agentNextActionAvailable && confirmationIsFooterAction[\s\S]*const confirmationButtonLabel = confirmationUsesPrimaryAction \? confirmationAction\.label : "继续确认"[\s\S]*runConfirmationAction[\s\S]*handleNext\(\)[\s\S]*prepareChange\(agentMessageConfirmationIntent\(message,\s*confirmationAction\.label\),\s*currentComposerSelectionOverride\(\)\)/, "Visible timeline confirmation cards must remain clickable through the matching Agent next-action path, and recovered timeline confirmations must visibly re-enter the Agent as an explicit confirmation intent"));
checkMessage(requireWithin(minimalAgentPanelSource, /const statusReadyNewVideoDraftConfirmation = Boolean\([\s\S]*projectStatusView\?\.stage === "等待确认"[\s\S]*确认\|草案\|故事流\|写入故事流/, "Footer must recognize ready new-video drafts from the visible project status summary"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /id: "footer_action_new_video_draft"[\s\S]*title: "建议行动：写入故事流"[\s\S]*toolName: "write_project"[\s\S]*确认后进入项目故事流/, "Ready new-video drafts must appear as an Agent confirmation message instead of a footer workflow button"));
checkMessage(requireWithin(minimalAgentPanelSource, /if \(hasComposerInput \|\| projectRequiredForWorkflow\) return undefined[\s\S]*id: "footer_action_new_video_draft"/, "Ready new-video draft confirmation must still create a visible Agent card when an older timeline confirmation exists"));
checkMessage(requireWithin(minimalAgentPanelSource, /const shouldAppendFooterActionConfirmationMessage = Boolean\([\s\S]*footerActionConfirmationMessage\.id === "footer_action_new_video_draft"[\s\S]*message\.id === footerActionConfirmationMessage\.id[\s\S]*message\.role === "confirmation"[\s\S]*fullAgentThreadMessages\.push\(footerActionConfirmationMessage\)/, "Ready draft fallback confirmation must be appended as a clickable message card, while other footer confirmations still avoid duplicates"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /buildVibeAgentTimelineStatusView\(agentTimelineEntries\)[\s\S]*agentTimelineStatusLine[\s\S]*agentTimelineNextLine/, "MinimalAgentPanel bottom status must derive from the same Agent timeline status source"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentConfirmationAction[\s\S]*minimalAgentFactValue\(message,\s*\["外部提交",\s*"执行",\s*"调用",\s*"成本"\]\)[\s\S]*minimalAgentFactValue\(message,\s*\["目标",\s*"影响"\]\)[\s\S]*minimalAgentFactValue\(message,\s*\["写入"\]\)[\s\S]*预计保存到/, "Agent confirmation hints must use the same cost/submission/save facts shown in the message flow"));
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentConfirmationBoundary\(message: MinimalAgentMessage\)[\s\S]*\["目标",\s*"影响"\][\s\S]*\["成本",\s*"调用",\s*"执行"\][\s\S]*\["写入"\][\s\S]*\["外部提交"\][\s\S]*确认前/, "Agent confirmation cards must summarize target, cost, writes, and external submission from message facts"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentVisibleFacts\(message: MinimalAgentMessage\)[\s\S]*minimalAgentMessageRequestsActionConfirmation\(message\)[\s\S]*new Set\(\["成本",\s*"外部提交",\s*"写入"\]\)[\s\S]*hiddenConfirmationLabels\.has\(label\)/, "Agent confirmation fact chips must hide engineering-style cost/write/external labels and leave those boundaries in natural copy"));
checkMessage(requireWithin(minimalAgentPanelSource, /latestConfirmationMessageId[\s\S]*minimalAgentConfirmationAction\(message,\s*primaryLabel\)[\s\S]*const confirmationUsesPrimaryAction = visibleNewVideoDraftConfirmation[\s\S]*confirmationMatchesPrimaryAction[\s\S]*confirmationIsFooterAction[\s\S]*const confirmationDisabled = visibleNewVideoDraftConfirmation[\s\S]*const confirmationButtonLabel = confirmationUsesPrimaryAction \? confirmationAction\.label : "继续确认"[\s\S]*const confirmationButtonHint = confirmationUsesPrimaryAction[\s\S]*这条确认来自历史消息[\s\S]*const runConfirmationAction = \(\) => \{[\s\S]*confirmationUsesPrimaryAction[\s\S]*handleNext\(\)[\s\S]*prepareChange\(agentMessageConfirmationIntent\(message,\s*confirmationAction\.label\),\s*currentComposerSelectionOverride\(\)\)[\s\S]*onClick=\{runConfirmationAction\}[\s\S]*disabled=\{confirmationDisabled\}[\s\S]*\{confirmationButtonLabel\}/, "Latest Agent confirmation message must expose its own concrete confirmation action without inheriting unrelated footer actions; recovered confirmations visibly re-enter the Agent as explicit confirmation intent"));
