...
checkMessage(requireWithin(minimalAgentPanelSource, /const footerStatusCopy = composerInputUsesNextAction[\s\S]*识别为继续[\s\S]*按发送交给 AI 导演[\s\S]*建议动作：/, "Bottom composer must explain send input, typed continue, and the current next action"));
