...
checkMessage(requireWithin(directorMode, /displayedProjectStatusView[\s\S]*agentPendingAction[\s\S]*右侧有一条待确认动作[\s\S]*在右侧处理/, "Director workbar must let a pending right-rail Agent action override stale project next-step copy"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /minimal-agent-skill-groups[\s\S]*当前项目已加载[\s\S]*Agent 推荐[\s\S]*我的 Skills/, "Skills panel must keep the three creator-facing groups"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /minimalAgentAssetInboxSummaryFromTimelineEntry[\s\S]*assetInboxSummary[\s\S]*aria-label="素材识别摘要"[\s\S]*类型[\s\S]*待确认/, "Agent material scanning must explain recognized materials in the message flow"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageStageLabel[\s\S]*state_change[\s\S]*执行状态[\s\S]*message\.title === "执行边界"[\s\S]*执行边界/, "Agent timeline messages must label running state changes and execution boundaries in creator language"));
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageTitleLabel[\s\S]*minimalAgentMessageStatusLabel\(message\)[\s\S]*title === status[\s\S]*title === stage[\s\S]*title\.startsWith\(stage\)[\s\S]*remainder === status/, "Agent message headers must avoid repeating stage or status labels as titles"));
