...
checkMessage(requireWithin(minimalAgentPanelSource, /function readyDraftExplicitMultiTargetShotRevisionClauses\(value: string,\s*shotCount: number\)[\s\S]*matchAll\(markerPattern\)[\s\S]*new Set\(markers\.map\(\(marker\) => marker\.targetNumber\)\)\.size < 2[\s\S]*function readyDraftMultiTargetShotRevisionFromIntent\(value: string,\s*shotCount: number\)[\s\S]*readyDraftTargetShotRevisionFromIntent\(clause\.text,\s*shotCount\)[\s\S]*不会把后一个镜头的要求写进前一个镜头[\s\S]*const composerReadyDraftTargetShotRevision = !composerReadyDraftRequestedShotCount[\s\S]*readyDraftMultiTargetShotRevisionFromIntent\(composerReadyDraftInputText,\s*newVideoDraftShotCountForAgent\)[\s\S]*\|\| readyDraftTargetShotRevisionFromIntent/, "Ready draft feedback preview must isolate and aggregate multiple explicitly targeted shot edits before the single-shot fallback"));
checkMessage(requireWithin(minimalAgentPanelSource, /function readyDraftFeedbackExplicitlyRequestsSceneRevision[\s\S]*场景\|地点\|环境[\s\S]*const sceneMove = readyDraftFeedbackExplicitlyRequestsSceneRevision\(text\)[\s\S]*const removalOnly = Boolean\(removalTarget\)[\s\S]*&& !\/\(\?:改成\|改为\|换成\|调整成\|只保留\|保留\|改拍\|改用\)\//, "Ready draft feedback preview must not invent a scene move or collapse a mixed replacement request into removal-only copy"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function readyDraftRemovalTargetFromText\(value: string\)[\s\S]*只改场景\|不要改动作\|不要改变动作\|不改动作\|不改变动作\|保留动作\|动作不变\|动作保持不变[\s\S]*不要再提\|不要\|别\|不用\|去掉\|移除\|删掉\|删除[\s\S]*const removalTarget = readyDraftRemovalTargetFromText\(text\)[\s\S]*const removalOnly = Boolean\(removalTarget\)[\s\S]*const targetFact = selectedMatch[\s\S]*tailMatch && removalOnly[\s\S]*含\$\{removalTarget\}的镜头[\s\S]*去掉：\$\{removalTarget\}/, "Ready draft target-shot preview must identify removal-only phrases such as 结尾那镜不要月亮 without treating mixed replacement or action-preservation feedback as deletion"));
