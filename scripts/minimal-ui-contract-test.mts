...
checkMessage(requireWithin(minimalAgentPanelSource, /currentMessages\.length <= MAX_VISIBLE_AGENT_THREAD_MESSAGES[\s\S]*hiddenCount:\s*0[\s\S]*currentMessages\.length - bounded\.length/, "MinimalAgentPanel hidden-history notice must only count visible-message window truncation, not compacted internal process cards"));
