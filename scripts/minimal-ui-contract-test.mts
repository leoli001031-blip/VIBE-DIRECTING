...
checkMessage(requireWithin(projectStatusViewModelSource, /input\.projectReady && shotCount > 0[\s\S]*"临时项目"[\s\S]*"先写想法"/, "Confirmed browser drafts must show temporary-project status instead of asking for another idea"));
...
checkMessage(requireWithin(directorModeSource, /function latestPendingAgentConfirmation[\s\S]*confirmation_request[\s\S]*function pendingAgentConfirmationLabel[\s\S]*intakePhase === "planning_ready"[\s\S]*确认这版故事[\s\S]*negativeVideoCopy[\s\S]*negativeReferenceCopy[\s\S]*negativeExportCopy[\s\S]*generate_references[\s\S]*submit_video[\s\S]*prepare_export[\s\S]*!negativeReferenceCopy[\s\S]*!negativeVideoCopy[\s\S]*!negativeExportCopy/, "DirectorMode must derive creator-facing pending confirmation labels from structured Agent timeline state before scanning negative reference, video, or export copy"));
checkMessage(requireWithin(directorModeSource, /function projectStatusViewWithPendingAgentConfirmation[\s\S]*stage:\s*"等待你确认"[\s\S]*右侧消息里有[\s\S]*先确认当前卡/, "Director workbar must let a pending right-rail Agent confirmation override stale project next-step copy"));
checkMessage(requireWithin(directorMode, /const pendingAgentConfirmation = useMemo[\s\S]*latestPendingAgentConfirmation\(restoredAgentTimelineEntries\)[\s\S]*pendingAgentConfirmationCopy[\s\S]*displayedPendingAgentConfirmationCopy[\s\S]*projectStatusViewWithPendingAgentConfirmation\([\s\S]*agentPendingAction \? "确认当前消息" : displayedPendingAgentConfirmationCopy/, "Director workbar must sync restored timeline confirmations into the displayed project status"));
checkMessage(requireWithin(creatorDeskPanelsSource, /const agentConfirmationTakingFocus = projectStatusStage === "等待你确认"[\s\S]*primaryFlowTakingFocus = agentConfirmationTakingFocus \|\| exportFlowTakingFocus \|\| videoFlowTakingFocus[\s\S]*先处理右侧确认卡/, "CreatorDeskPanels must stop competing with the right-side Agent confirmation while a confirmation is pending"));
checkMessage(requireWithin(creatorDeskPanelsSource, /const displayCurrentTask = agentConfirmationTakingFocus[\s\S]*missing: projectStatusView\?\.doing[\s\S]*plan: projectStatusView\?\.nextAction[\s\S]*label: projectStatusView\?\.nextAction/, "CreatorDeskPanels must mirror the active Agent confirmation in the current-task copy"));
checkMessage(requireWithin(directorModeSource, /buildVibeAgentTimelineStatusView\(restoredAgentTimelineEntries\)[\s\S]*agentTimelineStatus:\s*agentTimelineStatusView/, "Director workbar status must share the restored Agent timeline status source"));
checkMessage(requireWithin(directorModeSource, /function projectStatusViewWithCommittedDraft[\s\S]*stage:\s*"项目故事已更新"[\s\S]*故事已保存到项目/, "Director workbar must collapse already-confirmed draft state into a clear story-updated status"));
...
checkMessage(requireWithin(directorMode, /<MinimalAgentPanel[\s\S]*agentCommand=\{visibleAgentCommand\}[\s\S]*projectObservation=\{projectNavReady \? creatorDesk\?\.projectObservation : undefined\}/, "DirectorMode must pass the unified CreatorDesk observation only when the current project context is active"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /projectObservation\?:\s*ProjectObservationProjection[\s\S]*if \(projectObservation\) return projectObservation;[\s\S]*buildProjectObservation\(/, "MinimalAgentPanel must prefer the unified project observation before falling back to local projection"));
...
for (const label of ["从新视频开始", "继续修改", "主角参考", "风格参考", "场景参考", "道具参考", "添加文件", "拖入图片、声音或脚本", "AI 拆分", "确认时选文件夹"]) {
...
checkMessage(requireWithin(readText("src/ui/director/directorSkillUi.ts"), /用角色、场景、道具和文字说明锁定画面[\s\S]*生成前仍会等待确认/, "Minimal story flow strategy copy must keep generation behind confirmation"));
...
checkMessage(requireWithin(newVideoStartSource, /new-video-start-guide[\s\S]*aria-label="开始方式"[\s\S]*在右侧和 AI 导演说/, "Empty new-project guide should point users to the right Agent rail"));
checkMessage(requireWithin(newVideoStartSource, /new-video-agent-entry-hint[\s\S]*右侧和 AI 导演说[\s\S]*确认前不会生成/, "Empty new-project workspace must show an Agent-entry hint instead of a second composer"));
...
checkMessage(requireWithin(newVideoStartSource, /confirmedFlowTitle[\s\S]*保存到项目，不会生成[\s\S]*保存到项目，再生成参考[\s\S]*保存到项目，参考通过后可发视频/, "NewVideoStart organized draft must explain the post-confirm next step for each execution boundary"));
...
checkMessage(requireWithin(creatorDeskPanelsSource, /reasoningDisclosureLabel = videoFlowTakingFocus \? "视频进度" : "AI 导演怎么判断"[\s\S]*reasoningDisclosureSummary = videoFlowTakingFocus \? activeVideoProgressSubject : displayPreflight\.modeSummary[\s\S]*reasoningDisclosureDetail = videoFlowTakingFocus/, "CreatorDeskPanels must switch the reasoning disclosure from reference strategy to video progress while video work is active"));
...
checkMessage(requireWithin(creatorDeskPanelsSource, /!localProjectReady[\s\S]*还没有本地项目文件夹[\s\S]*projectRequirement\.label[\s\S]*referenceGenerationNeedsPermission/, "CreatorDeskPanels must prioritize local-project setup before reference-generation permission copy"));
checkMessage(requireWithin(creatorDeskPanelsSource, /可以继续说想法；生成参考、视频或导出前再准备本地项目/, "CreatorDeskPanels browser-draft hint must say planning can continue"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /footerActionConfirmationMessage[\s\S]*!agentThreadMessages\.some\(minimalAgentMessageRequestsActionConfirmation\)[\s\S]*agentThreadMessages = \[\.\.\.agentThreadMessages, footerActionConfirmationMessage\]/, "MinimalAgentPanel must keep a footer confirmation visible after message compaction only when no user-turn confirmation card is already visible"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /minimalAgentAssetInboxSummaryFromTimelineEntry[\s\S]*bindingPreviewLabels[\s\S]*foldedDetailLabels[\s\S]*assetInboxSummary[\s\S]*aria-label="素材识别摘要"[\s\S]*类型[\s\S]*待确认[\s\S]*建议绑定[\s\S]*并入镜头/, "Agent material scanning must explain recognized materials and binding suggestions in the message flow"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const footerStatusCopy = hasComposerInput[\s\S]*识别为：\$\{composerIntentRoute\.label\}[\s\S]*消息里等待你确认：/, "Bottom composer must send typed input into the Agent and point empty-input confirmations back to Agent messages"));
checkMessage(requireWithin(minimalAgentPanelSource, /const footerHintCopy = composerHint\.trim\(\) === footerStatusCopy\.trim\(\) \? "" : composerHint[\s\S]*\{footerHintCopy && <small>\{footerHintCopy\}<\/small>\}/, "Bottom composer must suppress duplicate hint/action copy"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /prepareChange[\s\S]*!localProjectReadyForTools && intentNeedsLocalProjectBeforeTooling\(userIntent\)[\s\S]*rememberAgentTimelineEntries\(buildLocalBlockedAgentTimelineEntries/, "Typed continue or generation requests must enter the Agent timeline before showing the local-project blocker"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const agentNextActionAvailable = !hasComposerInput && primaryLabel !== "发送"/, "Agent next actions must be detected separately from the bottom send button"));
...
checkMessage(requireWithin(creatorDeskPanelsSource, /creator-status-details creator-agent-reasoning[\s\S]*reasoningDisclosureLabel[\s\S]*creator-preflight-strip[\s\S]*creator-desk-details/, "Creator desk must put process and full details behind one adaptive Agent disclosure"));
...
checkMessage(requireWithin(newVideoStart, /storyboardPlanningRunning \? "正在整理镜头" : "待确认草案"[\s\S]*storyboardPlanningRunning \? "AI 还在整理镜头，完成后再确认。" : "还没有保存到项目；确认后左侧故事数才会更新。"/, "NewVideoStart plan summary must not label a running storyboard plan as ready to confirm"));
checkMessage(requireWithin(minimalAgentPanelSource, /readyNewVideoDraftForAgent[\s\S]*newVideoDraftReadyForAgent[\s\S]*等待确认\|草案\|确认\.\*故事流/, "Right Agent composer must infer a ready new-video draft from the visible waiting-confirmation status"));
checkMessage(requireWithin(minimalAgentPanelSource, /newVideoDraftTimelineCopy[\s\S]*agentTimelineEntries\.slice\(-6\)[\s\S]*newVideoDraftStatusCopy[\s\S]*newVideoDraftTimelineCopy/, "Right Agent composer must also use the Agent timeline as a ready-draft confirmation source"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const currentProjectHasStoryContext = Boolean\([\s\S]*projectObservation\?\.story\.status === "selected"/, "Right Agent composer must know when the visible project already has a story"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const visibleNewVideoDraftConfirmation = Boolean\([\s\S]*currentTimelineConfirmationLabel[\s\S]*等待确认\|草案\|确认\|故事流[\s\S]*visibleTimelineConfirmationMessage\?\.body[\s\S]*visibleTimelineConfirmationMessage\?\.next/, "Right Agent visible confirmation copy must also become an executable ready-draft action"));
checkMessage(requireWithin(minimalAgentPanelSource, /if \(activeNewVideoDraftConfirmation \|\| visibleNewVideoDraftConfirmation\) \{[\s\S]*const confirmDraftFromAgent = onConfirmNewVideoDraftFromAgent \|\| \(\(\) => onStartNewVideoDraftFromAgent\?\.\(NEW_VIDEO_DRAFT_CONFIRM_LABEL\)\)[\s\S]*label:\s*NEW_VIDEO_DRAFT_CONFIRM_LABEL[\s\S]*statusLine:\s*disabled \? disabledReason : `下一步：\$\{NEW_VIDEO_DRAFT_CONFIRM_LABEL\}`[\s\S]*void confirmDraftFromAgent\(\)/, "Active new-video ready drafts must expose a right-rail confirmation action instead of only hint text"));
checkMessage(requireWithin(minimalAgentPanelSource, /if \(activeNewVideoDraftConfirmation \|\| visibleNewVideoDraftConfirmation\) \{[\s\S]*label:\s*NEW_VIDEO_DRAFT_CONFIRM_LABEL[\s\S]*if \(projectBlockedWithoutFooterResolver\)/, "Ready new-video draft confirmation must win over the no-local-project disabled-send fallback"));
checkMessage(requireWithin(minimalAgentPanelSource, /const visibleConfirmationActionAvailable = agentNextActionAvailable[\s\S]*visibleNewVideoDraftConfirmation[\s\S]*Boolean\(visibleTimelineConfirmationMessage && !hasComposerInput && !isPreparingPlan\)[\s\S]*message\.role === "confirmation"[\s\S]*visibleConfirmationActionAvailable[\s\S]*const confirmationIsFooterAction = message\.id\.startsWith\("footer_action_"\)[\s\S]*const confirmationIsNewVideoDraftAction = message\.id\.startsWith\("footer_action_new_video_draft"\) \|\| visibleNewVideoDraftConfirmation[\s\S]*const confirmationMatchesPrimaryAction = Boolean\([\s\S]*message\.actionId === agentActionEnvelope\.actionId[\s\S]*const confirmationUsesPrimaryAction = !confirmationIsNewVideoDraftAction[\s\S]*confirmationMatchesPrimaryAction[\s\S]*agentNextActionAvailable && confirmationIsFooterAction[\s\S]*const confirmationButtonLabel = confirmationAction\.label[\s\S]*runConfirmationAction[\s\S]*confirmationIsNewVideoDraftAction[\s\S]*confirmDraftFromAgent[\s\S]*handleNext\(\)[\s\S]*prepareChange\(agentMessageConfirmationIntent\(message,\s*confirmationAction\.label\),\s*currentComposerSelectionOverride\(\)\)/, "Visible timeline confirmation cards must route draft confirmations through the draft bridge with their own label, keep matching non-draft actions clickable, and visibly re-enter recovered confirmations as explicit Agent intent"));
checkMessage(requireWithin(minimalAgentPanelSource, /const statusReadyNewVideoDraftConfirmation = Boolean\([\s\S]*projectStatusView\?\.stage === "等待确认"[\s\S]*确认\|草案\|故事流\|写入故事流\|保存到项目/, "Footer must recognize ready new-video drafts from the visible project status summary"));
checkMessage(requireWithin(minimalAgentPanelSource, /const footerNewVideoDraftConfirmationReady = Boolean\([\s\S]*isNewVideoDraftConfirmationLabel\(currentTimelineConfirmationLabel\)[\s\S]*statusReadyNewVideoDraftConfirmation[\s\S]*const showFooterNextActionButton = \(agentNextActionAvailable && !hasComposerInput\) \|\| footerNewVideoDraftConfirmationReady/, "Footer must detect ready new-video drafts for message-flow confirmation even when the normal next action is not available"));
checkMessage(requireWithin(minimalAgentPanelSource, /const footerNewVideoDraftConfirmationId = `footer_action_new_video_draft_[\s\S]*id: footerNewVideoDraftConfirmationId[\s\S]*title: "建议行动：确认这版故事"[\s\S]*toolName: "write_project"[\s\S]*确认后保存到项目/, "Ready new-video drafts must appear as a current-turn Agent confirmation message instead of a footer workflow button"));
checkMessage(requireWithin(minimalAgentPanelSource, /if \(hasComposerInput\) return undefined[\s\S]*id: footerNewVideoDraftConfirmationId[\s\S]*if \(projectRequiredForWorkflow\) return undefined[\s\S]*if \(showAgentNote \|\| showAgentResultNote \|\| preparedContext\?\.userIntent\?\.trim\(\)\) return undefined/, "Ready new-video draft confirmation must create a visible Agent card before local-project workflow blocking or normal result replies can suppress it"));
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentFooterConfirmationHasExistingVisiblePeer[\s\S]*footerMessage\.id\.startsWith\("footer_action_"\)[\s\S]*minimalAgentConfirmationRequestsSameAction\(candidate,\s*footerMessage\)[\s\S]*const shouldAppendFooterActionConfirmationMessage = Boolean\([\s\S]*!minimalAgentFooterConfirmationHasExistingVisiblePeer\(fullAgentThreadMessages,\s*footerActionConfirmationMessage\)[\s\S]*footerActionConfirmationMessage\.id\.startsWith\("footer_action_new_video_draft"\)[\s\S]*message\.id === footerActionConfirmationMessage\.id[\s\S]*fullAgentThreadMessages\.push\(footerActionConfirmationMessage\)/, "Ready draft fallback confirmation must be appended only when an equivalent visible Agent confirmation is not already present"));
checkMessage(requireWithin(minimalAgentPanelSource, /id: "footer_action_export"[\s\S]*title: "建议行动：导出交付包"[\s\S]*确认前不会写入导出文件[\s\S]*toolName: "export_project"[\s\S]*actionKind: "prepare_export"/, "Ready export work must appear as an Agent confirmation message instead of only a topbar export button"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const agentNextActionAvailable = !hasComposerInput && primaryLabel !== "发送"/, "MinimalAgentPanel must detect empty-composer next actions without converting the send button into that action"));
...
checkMessage(requireWithin(appSource, /agentTimelineEntries:\s*vibeAgentTurn\.timeline\.entries/, "App stage result must return timeline entries to the composer"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /buildMinimalAgentProductAdapter\(\{[\s\S]*availability:\s*currentAgentToolAvailability\(\)[\s\S]*setAgentToolHandoff[\s\S]*createReferences:\s*onCreateP6RealSample[\s\S]*submitVideo:\s*onSendSeedanceVideo[\s\S]*queryVideo:\s*onSendSeedanceVideo[\s\S]*runExport:\s*onRunExport[\s\S]*productAdapter\.runConfirmedAction\(\{[\s\S]*action,[\s\S]*userIntent,[\s\S]*preparedHandoff/, "MinimalAgentPanel confirmed execution must delegate product action running through one UI Agent product adapter"));
...
checkMessage(requireWithin(`${directorModeSource}\n${minimalAgentPanelSource}`, /restoredAgentTimelineEntries\?:\s*VibeAgentTimelineEntry\[\][\s\S]*restoredAgentTimelineEntries=\{restoredAgentTimelineEntries\}/, "DirectorMode and MinimalAgentPanel must pass restored Agent timeline entries into the composer"));
checkMessage(requireWithin(`${directorModeSource}\n${newVideoStartSource}`, /<NewVideoStart[\s\S]*restoredAgentTimelineEntries=\{restoredAgentTimelineEntries\}[\s\S]*onRememberAgentTimelineEntries=\{onRememberAgentTimelineEntries\}[\s\S]*restoredAgentTimelineEntries\?:\s*VibeAgentTimelineEntry\[\]/, "DirectorMode must pass restored Agent timeline entries into the new-video intake composer"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /agentTimelineEntries\.map\(minimalAgentMessageFromTimelineEntry\)/, "MinimalAgentPanel message flow must prefer persisted Agent timeline entries over synthetic status text"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageStageLabel[\s\S]*action_result[\s\S]*执行结果/, "Agent timeline messages must label execution results in creator language"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageStageLabel[\s\S]*state_change[\s\S]*selection_context_[\s\S]*当前选择[\s\S]*执行状态[\s\S]*message\.title === "执行边界"[\s\S]*执行边界/, "Agent timeline messages must label running state changes, selected context, and execution boundaries in creator language"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageNeedsLocalProject[\s\S]*本地项目\|项目文件夹\|临时项目/, "Agent messages must identify local-project blockers from persisted timeline copy"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /选择项目文件夹[\s\S]*先继续改文字/, "Local-project blocker messages must offer a clear project setup action and a text-only fallback"));
checkMessage(requireWithin(minimalAgentPanelSource, /minimal-agent-action-hint[\s\S]*当前环境不能直接选文件夹，请用左上角打开或新建项目/, "Disabled local-project setup actions must explain the fallback visibly, not only in a hover title"));
checkMessage(requireWithin(minimalAgentPanelSource, /function buildLocalProjectSetupTimelineEntries[\s\S]*准备项目文件夹[\s\S]*项目文件夹已准备[\s\S]*没有选择项目文件夹/, "Local-project setup must produce real Agent timeline entries for started, completed, and cancelled states"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageCompletedLocalProjectSetup[\s\S]*write_project[\s\S]*项目文件夹已准备/, "Completed local-project setup messages must be detected from real Agent timeline entries"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /latestCompletedToolActionMessageId[\s\S]*completedToolActionView\(message\)[\s\S]*const nextIntent = agentMessageNextIntent\(message\)[\s\S]*const resultNextAction = availableFooterDirectAction[\s\S]*resultNextAction\.perform\(\)[\s\S]*prepareChange\(nextIntent,\s*currentComposerSelectionOverride\(\)\)[\s\S]*resultNextLabel/, "Latest completed Agent tool result must prefer the concrete next action while keeping a context-preserving Kernel-next fallback in the message flow"));
checkMessage(requireWithin(minimalAgentPanelSource, /function agentMessageRevisionIntent\(message: MinimalAgentMessage\)[\s\S]*message\.executionResult\?\.summary[\s\S]*agentMessageNextIntent\(message,[\s\S]*刚才这一步没通过[\s\S]*请按这个方向继续改/, "Blocked Agent result revision must carry the execution failure summary and next step back into the composer"));
checkMessage(requireWithin(minimalAgentPanelSource, /function reviseFromAgentMessage\(message: MinimalAgentMessage\)[\s\S]*agentMessageRevisionIntent\(message\)[\s\S]*setText\(revisionIntent\)[\s\S]*继续改这一步/, "Blocked Agent result revise action must restore a message-specific revision intent"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageIsCompletedProcessCard[\s\S]*request_user_confirmation[\s\S]*run_confirmed_action[\s\S]*function minimalAgentMessageHasResultCard[\s\S]*tool_result[\s\S]*action_result[\s\S]*function minimalAgentMessageIsSupersededProcessCard[\s\S]*execution_boundary_[\s\S]*function minimalAgentMessageIsSupersededObservationCard[\s\S]*inspect_project[\s\S]*plan_next_action[\s\S]*function minimalAgentMessageIsSupersededConfirmationPrepCard[\s\S]*minimalAgentMessageHasLaterConfirmationRequest[\s\S]*function minimalAgentMessageIsSupersededSelectionContextCard[\s\S]*selection_context_[\s\S]*function visibleMinimalAgentMessages[\s\S]*compactedMessages[\s\S]*latestUserIndex[\s\S]*hiddenCount/, "MinimalAgentPanel must keep long persisted Agent timelines readable by compacting completed internal process cards, superseded execution-process cards, superseded observation cards, pre-confirmation prep cards, older selection context cards, and duplicate internal tool returns"));
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageIsSupersededAssistantReplyCard\(messages: MinimalAgentMessage\[\], message: MinimalAgentMessage\)[\s\S]*message\.entryType !== "assistant_message" \|\| message\.title !== "AI 导演"[\s\S]*minimalAgentMessageRequestsActionConfirmation\(candidate\)[\s\S]*candidate\.actionId === message\.actionId[\s\S]*!minimalAgentMessageIsSupersededAssistantReplyCard\(filteredMessages,\s*message\)/, "Agent thread must keep the creator-facing confirmation card from competing with a generic same-action assistant reply"));
checkMessage(requireWithin(minimalAgentPanelSource, /function placeSelectionContextBeforeActiveConfirmation\(messages: MinimalAgentMessage\[\]\)[\s\S]*findIndex\(minimalAgentMessageIsWaitingConfirmation\)[\s\S]*selectionAfterConfirmation[\s\S]*minimalAgentMessageIsSelectionContext[\s\S]*withoutMovedSelection[\s\S]*return \[[\s\S]*selectionAfterConfirmation[\s\S]*withoutMovedSelection\.slice\(updatedConfirmationIndex\)[\s\S]*messages:\s*placeSelectionContextBeforeActiveConfirmation\(bounded\)/, "Agent thread must keep the active confirmation card as the final visible action while still showing what the current 'this' refers to"));
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentSelectionContextMessageIsOutsideActiveScope[\s\S]*selection_context_[\s\S]*!hasBoundSelection \|\| !activeSelectionKey[\s\S]*selectionContextMessageId\(activeSelectionKey\)[\s\S]*stateAwareAgentThreadMessages[\s\S]*minimalAgentSelectionContextMessageIsOutsideActiveScope\(message,\s*selectionFocusKey,\s*hasBoundSelection\)/, "Restored Agent timelines must hide stale selection-context cards when the current view has no active selected shot or asset"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentMessageTitleLabel\(message: MinimalAgentMessage\)[\s\S]*minimalAgentMessageRequestsActionConfirmation\(message\) && title === "请求确认"[\s\S]*minimalAgentConfirmationAction\(message,\s*"确认执行"\)\.label/, "Generic Agent confirmation titles must render as the concrete confirmation action"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentConfirmationAction[\s\S]*minimalAgentFactValue\(message,\s*\["外部提交",\s*"执行",\s*"调用",\s*"成本"\]\)[\s\S]*minimalAgentFactValue\(message,\s*\["目标",\s*"影响"\]\)[\s\S]*minimalAgentFactValue\(message,\s*\["写入",\s*"保存"\]\)[\s\S]*预计保存到/, "Agent confirmation hints must use the same cost/submission/save facts shown in the message flow"));
checkMessage(requireWithin(minimalAgentPanelSource, /function minimalAgentConfirmationBoundary\(message: MinimalAgentMessage\)[\s\S]*\["目标",\s*"影响"\][\s\S]*\["成本",\s*"调用",\s*"执行"\][\s\S]*\["写入",\s*"保存"\][\s\S]*\["外部提交"\][\s\S]*确认前/, "Agent confirmation cards must summarize target, cost, writes, and external submission from message facts"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /function cleanMinimalAgentMessageCopy\(value: string\)[\s\S]*会调用图片生成[\s\S]*会调用参考生成/, "Agent thread must clean old image-generation wording before rendering"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /latestConfirmationMessageId[\s\S]*minimalAgentConfirmationAction\(message,\s*primaryLabel\)[\s\S]*const confirmationIsNewVideoDraftAction = message\.id\.startsWith\("footer_action_new_video_draft"\) \|\| visibleNewVideoDraftConfirmation[\s\S]*const confirmationUsesPrimaryAction = !confirmationIsNewVideoDraftAction[\s\S]*confirmationMatchesPrimaryAction[\s\S]*confirmationIsFooterAction[\s\S]*const confirmationDisabled = confirmationIsNewVideoDraftAction[\s\S]*const confirmationButtonLabel = confirmationAction\.label[\s\S]*const confirmationButtonHint = confirmationIsNewVideoDraftAction[\s\S]*confirmationAction\.hint[\s\S]*confirmationUsesPrimaryAction[\s\S]*这条确认来自历史消息[\s\S]*const runConfirmationAction = \(\) => \{[\s\S]*confirmationIsNewVideoDraftAction[\s\S]*confirmDraftFromAgent[\s\S]*confirmationUsesPrimaryAction[\s\S]*handleNext\(\)[\s\S]*prepareChange\(agentMessageConfirmationIntent\(message,\s*confirmationAction\.label\),\s*currentComposerSelectionOverride\(\)\)[\s\S]*onClick=\{runConfirmationAction\}[\s\S]*disabled=\{confirmationDisabled\}[\s\S]*\{confirmationButtonLabel\}/, "Latest Agent confirmation message must expose its own concrete confirmation action without inheriting unrelated footer actions; draft confirmations use the draft bridge and recovered confirmations visibly re-enter the Agent as explicit confirmation intent"));
checkMessage(requireWithin(minimalAgentPanelSource, /latestConfirmationMessageId[\s\S]*const confirmationBoundary = minimalAgentConfirmationBoundary\(message\)[\s\S]*className="minimal-agent-confirmation-boundary"[\s\S]*\{confirmationBoundary\}/, "Latest Agent confirmation message must show the confirmation boundary before the action buttons"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /const displayedAgentBoundaryConfirmationLabel = currentTimelineConfirmationLabel[\s\S]*showFooterNextActionButton && footerDirectActionBoundaryNotice \? primaryLabel : ""[\s\S]*const displayedAgentBoundarySummaryLabel = displayedAgentBoundaryConfirmationLabel[\s\S]*\?\s*"等你确认"[\s\S]*:\s*agentBoundarySummaryLabel[\s\S]*const displayedAgentBoundaryDetail = displayedAgentBoundaryConfirmationLabel[\s\S]*当前等待你确认[\s\S]*displayedAgentBoundaryConfirmationLabel[\s\S]*agentBoundaryDetail[\s\S]*<strong>\{displayedAgentBoundarySummaryLabel\}<\/strong>[\s\S]*<small>\{displayedAgentBoundaryDetail\}<\/small>/, "When any Agent confirmation card or direct confirmation action is waiting, the visible execution boundary must show the pending confirmation instead of stale global plan-only mode"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /intentNeedsLocalProjectBeforeTooling[\s\S]*rememberAgentTimelineEntries\(buildLocalBlockedAgentTimelineEntries/, "MinimalAgentPanel must append and persist typed Agent messages when a temporary project blocks tool execution"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /executionContract:\s*actionExecutionPermissionContract[\s\S]*directorAgentExecutionContractFromCreatorBoundary/, "Local Agent action construction must omit the explicit execution contract for Agent-driven continue"));
checkMessage(requireWithin(minimalAgentPanelSource, /videoPermissionContract:\s*actionExecutionPermissionContract[\s\S]*availability:\s*currentAgentToolAvailability\(\)/, "Product Agent stage call must omit the explicit execution contract for Agent-driven continue"));
...
checkMessage(requireWithin(minimalAgentPanelSource, /className="minimal-agent-send-button"[\s\S]*onClick=\{handleSend\}[\s\S]*<Send size=\{15\} \/>/, "MinimalAgentPanel primary composer action must stay a send button; workflow confirmation belongs to Agent messages"));
...
checkMessage(requireWithin(minimalExportSource, /videoMissingCount[\s\S]*视频状态[\s\S]*视频结果/, "Export package summary must label missing video slots as status instead of finished results"));
checkMessage(requireWithin(minimalExportSource, /videoSummaryLabel = finalVideoPending \? "视频状态" : "视频结果"[\s\S]*\{videoSummaryLabel\}：\{videoSummary\}/, "Export visible video summary must call pending videos status instead of results"));
...
checkMessage(requireWithin(minimalTopNavSource, /projectStorageBadge[\s\S]*临时[\s\S]*本地[\s\S]*未连接/, "top nav project title must expose whether the current work is temporary, local, or not connected"));
...
checkMessage(requireWithin(appSource, /if \(runtimeProjectBinding\.status !== "bound" \|\| !runtimeProjectBinding\.projectRoot\) return;[\s\S]*if \(freshProjectSessionRequested && projectFileSelection\.status !== "selected"\) return;[\s\S]*setProjectFileSelection\(\{[\s\S]*projectRoot: runtimeProjectBinding\.projectRoot/, "fresh project sessions must not let a stale runtime binding repopulate the visible project before explicit selection"));
...
checkMessage(requireWithin(minimalAgentPanel, /agentThreadRef[\s\S]*thread\.scrollTop\s*=\s*thread\.scrollHeight/, "MinimalAgentPanel must keep the latest Agent reply visible after sending"));
