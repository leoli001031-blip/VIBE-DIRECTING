...
checkMessage(requireWithin(directorModeSource, /function latestPendingAgentConfirmation[\s\S]*confirmation_request[\s\S]*function pendingAgentConfirmationLabel[\s\S]*确认修改[\s\S]*确认生成参考/, "DirectorMode must derive creator-facing pending confirmation labels from the Agent timeline"));
