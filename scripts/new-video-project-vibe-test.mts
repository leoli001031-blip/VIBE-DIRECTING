...
const pollutedStoryboardRows = [{
  id: "storyboard_row_polluted",
  shotNo: "P-1",
  duration: "4",
  shotSize: "中景",
  camera: "固定镜头",
  visualDescription: "少女在旧书店窗边停下，书页被风吹开。",
  primaryAction: "待确认",
  actionTrigger: "待补",
  microReaction: "无",
  actionReactionQa: "待补充",
  executionMode: "single_continuous_shot",
  referenceStrategy: "omni_reference",
  visibleCutBudget: "待确认",
  subtitle: "-",
  sound: "无",
  title: "占位词清洗",
  characters: "待确认",
  ...
  rhythmReason: "待补",
  sourceFactId: "fact_storyboard_polluted",
}];
const stagedFromPollutedStoryboard = buildNewVideoProjectVibeStagedTransaction({
  project: createProject(),
  draft: {
    script: richDraft.script,
    style: richDraft.style,
  ...
  storyboardDraft: pollutedStoryboardRows,
  generatedAt,
});
assert(stagedFromPollutedStoryboard.blocked === false, `polluted storyboard should still stage after placeholder cleanup: ${stagedFromPollutedStoryboard.blockedReasons.join("; ")}`);
const pollutedShot = stagedFromPollutedStoryboard.planner.shots[0];
assert(pollutedShot, "polluted storyboard should produce one shot");
for (const placeholder of ["待确认", "待补", "待补充", "无"]) {
  assert(!JSON.stringify(pollutedShot).includes(placeholder), `formal Project.vibe shot should not persist placeholder value ${placeholder}`);
}
assert(pollutedShot.sceneGuidance?.includes("旧书店"), "non-placeholder scene guidance should survive cleanup");
assert(!pollutedShot.primaryAction && !pollutedShot.actionTrigger && !pollutedShot.microReaction, "placeholder action fields should be omitted");
assert(!pollutedShot.dialogueLines?.length, "dash subtitles should not become dialogue");
assert(!pollutedShot.sound && !pollutedShot.audioUsage, "empty sound and audio placeholders should be omitted");
