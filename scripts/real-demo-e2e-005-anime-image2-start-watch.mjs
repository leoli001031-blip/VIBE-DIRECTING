...
const timeoutMs = Number(process.env.REAL_DEMO_E2E_005_WATCH_TIMEOUT_MS || 30 * 60 * 1000);
const intervalMs = Number(process.env.REAL_DEMO_E2E_005_WATCH_INTERVAL_MS || 3000);
...
function collect() {
  ...
  for (const plan of manifest.shotPlans) {
    ...
    events.push(
      { ...base, eventId: `watcher_005_${plan.shotId}_file_observed`, sequence: events.length + 1, eventType: "file_observed", occurredAt: outputInfo.modifiedAt },
      { ...base, eventId: `watcher_005_${plan.shotId}_file_stable`, sequence: events.length + 2, eventType: "file_stable", occurredAt: outputInfo.modifiedAt },
      { ...base, eventId: `watcher_005_${plan.shotId}_hash_recorded`, sequence: events.length + 3, eventType: "hash_recorded", occurredAt: outputInfo.modifiedAt },
      { ...base, eventId: `watcher_005_${plan.shotId}_provider_paired`, sequence: events.length + 4, eventType: "sidecar_paired", sidecarKind: "provider_observation", sidecarPath: plan.providerObservationPath, occurredAt: providerInfo.modifiedAt },
      { ...base, eventId: `watcher_005_${plan.shotId}_qa_paired`, sequence: events.length + 5, eventType: "qa_paired", sidecarKind: "semantic_qa", sidecarPath: plan.semanticQaPath, occurredAt: qaInfo.modifiedAt },
    );
  }
...
while (result.blockers.length && Date.now() - startedAt < timeoutMs) {
  console.log(`Real Demo E2E 005 watcher waiting: ${result.blockers.slice(0, 6).join("; ")}`);
  await sleep(intervalMs);
  result = collect();
}

if (result.blockers.length) {
  console.error("Real Demo E2E 005 watcher timed out. No watcher event file was written.");
  for (const blocker of result.blockers) console.error(`- ${blocker}`);
  process.exit(1);
}

writeJson(absPath(manifest.runtimeTruthWatcherPath), {
  schemaVersion: "real_demo_e2e_005_runtime_truth_watcher_events_v1",
  generatedAt: new Date().toISOString(),
  runId: manifest.runId,
  sourceKind: "app_server_fs_changed",
  eventSource: "real-demo-e2e-005-anime-image2-start-watch-poller",
  events: result.events,
});

console.log("Real Demo E2E 005 watcher events written.");
