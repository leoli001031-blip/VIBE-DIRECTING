...
} from "../src/core/round5ArtifactIngest.ts";
...
function writeFixtureStart(runRoot, shotId) {
  ...
  const bytes = Buffer.from(`round5 fixture start ${shotId}`);
  fs.mkdirSync(path.join(runRoot, "shots", shotId), { recursive: true });
  fs.writeFileSync(path.join(runRoot, startPath), bytes);
  return {
    shotId,
...
const fixtureRoot = path.resolve("real-test-sandbox/generated-round5-artifact-ingest-fixture");
...
writeJson(reportPath, {
  ...
  endFrameStage: {
    ...
    blockers: round5EndFrameBlockers,
