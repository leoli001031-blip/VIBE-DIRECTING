...
async function loadCurrentProjectImage2BatchCore() {
  ...
  return {
    image2Batch: core.currentProjectImage2Batch,
    assetLibrary: core.assetLibraryCrud,
  };
}
...
const propPath = `${smallProjectFixture.projectRoot}/assets/props/compass/main.png`;
const {
  image2Batch: {
...
let libraryWithProp = lockedAssetLibrary();
libraryWithProp = addAsset(libraryWithProp, {
  id: "prop_locked",
  assetType: "prop",
  name: "Compass Prop",
  status: "locked",
...
const propProjection = buildCurrentProjectImage2ReferencesFromAssetLibrary(libraryWithProp);
assert(propProjection.blockers.length === 0, `locked prop should not add blockers: ${compact(propProjection.blockers)}`);
assert(propProjection.summary.eligibleCount === 4, "locked prop should be counted as an eligible future reference");
assert(propProjection.summary.byRole.prop === 1, "locked prop summary drifted");
const propReadyPlan = buildCurrentProjectImage2BatchPlan(baseInput({
  references: undefined,
  assetLibrary: libraryWithProp,
}));
assert(propReadyPlan.items.every((item) => item.referencePaths.includes(propPath)), "prop reference should enter Image2 batch item reference paths");
