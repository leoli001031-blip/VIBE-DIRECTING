...
import { buildVoiceSourceLibraryState, toRuntimeVoiceSources } from "../src/core/voiceSourceLibrary.ts";
import { buildVoiceAudioSettingsState as buildCoreVoiceAudioSettingsState } from "../src/core/voiceAudioSettings.ts";
import { buildImageKeyframeRuntimePlan } from "../src/core/imageKeyframeRuntime.ts";
import { buildProviderLiveGateState } from "../src/core/providerLiveGate.ts";
import { buildProviderExecutionPermissionGateState } from "../src/core/providerExecutionPermissionGate.ts";
import { buildProviderActionConfirmationReceiptState } from "../src/core/providerActionConfirmationReceipt.ts";
import { buildProviderExecutionHandoffState } from "../src/core/providerExecutionHandoff.ts";
import { buildLocalOrchestratorState } from "../src/core/localOrchestrator.ts";
import { buildRealProviderPilotState } from "../src/core/realProviderPilot.ts";
import { buildRealProviderExecutorState } from "../src/core/realProviderExecutor.ts";
import { buildRealProviderOneShotTestState } from "../src/core/realProviderOneShotTest.ts";
import {
...
const outPath = path.join(fixturesDir, "runtime-audit.json");
const stateOutPath = path.join(fixturesDir, "runtime-state.json");
...
const policy = {
  ...
  rules: [
    {
      slot: "image.generate",
      activeProvider: "openai-image2-agent-cli",
...
function buildToolDetectionReport(generatedAt) {
  ...
  const tools = [
    ...
    detectCommand({
      ...
      planned: true,
      notes: ["Phase 26 records the adapter placeholder only; the importer does not resolve or start Agent."],
    }, platform),
    detectCommand({
      id: "image2-runtime",
      command: "image2",
  ...
  return {
    ...
    slots: [
      ...
      },
      {
        ...
        forbiddenProviders: ["dreamina", "jimeng", "seedream", "seedance"],
        liveSubmitAllowed: false,
        notes: [`Image edit slot remains Image2-only; reference edits default to ${IMAGE2_REFERENCE_EDIT_MAX_CONCURRENCY} concurrent requests with no fallback to text-to-image.`],
      },
      {
        slot: "image.reference_asset",
        state: "active",
  ...
  return [
    {
      ...
      notes: [`Reference-image edit calls share the provider config and default to ${IMAGE2_REFERENCE_EDIT_MAX_CONCURRENCY} concurrent requests.`],
    },
    {
  ...
  return [
    {
      ...
      concurrencyPolicy: {
        ...
        notes: [
          "Reference-image edits use the measured stable default concurrency.",
          "Higher concurrency requires an explicit experimental override and retry/backoff evidence.",
        ],
      },
      source: "default",
      credential: {
...
function buildRuntimeEnvironment(generatedAt) {
  ...
  const config = {
    ...
    toolPaths: {
      agentCli: toolPath("agent", "Agent CLI", detectionReport, ["No agent session is started by import-runtime-test."]),
...
function buildAgentCliMockRunnerState(input) {
  ...
  return {
    ...
    notes: [
      ...
      "Import runtime records the boundary without spawning or resuming agent, submitting providers, executing shell, reading credentials, or mutating files.",
    ],
  };
}
...
function buildDefaultProviderRegistry(generatedAt) {
  const capabilities = [
    providerCapability({
      providerId: "openai-image2-agent-cli",
      providerName: "OpenAI Image2 via Agent CLI",
      slot: "image.generate",
      requiredMode: "text2image",
      executionState: "active",
      inputKinds: ["text"],
  ...
  return {
    ...
    defaultProviderBySlot: {
      "image.generate": "openai-image2-agent-cli",
...
function buildToolRuntimeHarnessState(input) {
  ...
  const checks = [
    toolRuntimeDetectedCheck({
      ...
      sourceRefs: ["adapterContracts:agentAdapters:agent-cli-agent"],
...
function buildKeyframePairDerivation(job, shot) {
  ...
  const startFrameReady = Boolean(shot.startFrame && !shot.issues.includes("missing_start_frame"));
  return {
    shotId: shot.id,
    ...
    validForI2vPair: endpointMode ? hasFrames && (shot.gates.pair === "PASS" || shot.gates.pair === "PARTIAL") : startFrameReady && shot.gates.pair !== "FAIL",
    exceptionReason: endpointMode
      ? hasFrames ? undefined : "Start or end keyframe is missing from runtime audit."
      : startFrameReady ? "First-frame video control does not require a separate end frame." : "Start keyframe is missing from runtime audit.",
...
function buildPreflight(job, shot, references, sourceIndex, expectedOutputs, keyframePairDerivation, generatedAt) {
  ...
  const requiresEndpointEndFrame = shot?.videoControlMode === "first_last_endpoint";
  if (job.slot === "video.i2v" && requiresEndpointEndFrame && !keyframePairDerivation?.validForI2vPair) {
    blockers.push({
      code: "invalid_keyframe_pair",
      messageForUser: "这个镜头选择了明确终点模式，需要有效的首尾帧。",
      technicalDetail: `${shot?.id || job.id} selected first_last_endpoint without a complete I2V pair.`,
      target: job.id,
    });
  }
...
function buildVideoReadinessGate(shot, task, capability, audioPlanning, issues) {
  ...
  const selectedFrameControlReady = requiresEndpointEndFrame ? derivation.validForI2vPair : startFramePresent && shot.gates.pair !== "FAIL";
  ...
  const checks = [
    ...
    videoReadinessCheck("end_frame_present", "endpoint end frame present", requiresEndpointEndFrame ? endFramePresent ? "pass" : "blocked" : "not_applicable", requiresEndpointEndFrame, requiresEndpointEndFrame ? endFramePresent ? "Shot has an endpoint end frame reference." : "Shot selected endpoint mode but is missing an end frame reference." : "First-frame video control does not require an endpoint end frame.", shot.endFrame || shot.id),
    videoReadinessCheck("keyframe_pair_derivation_valid", "selected frame control valid", selectedFrameControlReady ? "pass" : "blocked", true, selectedFrameControlReady ? requiresEndpointEndFrame ? "Start/end frame derivation is valid for endpoint video control." : "Start frame is ready for first-frame video control." : requiresEndpointEndFrame ? "Start/end frame derivation is missing or invalid for endpoint video control." : "Start frame is missing or blocked for first-frame video control.", shot.id),
    requiresEndpointEndFrame
      ? videoRequiredGateCheck("pair", shot.gates.pair, shot.id)
      : videoReadinessCheck("pair_gate_pass", "pair gate PASS", "not_applicable", false, "Pair gate is only required when explicit endpoint end-frame mode is selected.", shot.id),
...
function buildVideoTaskPlan(shot, task, gate, providerId, capability) {
  ...
  return {
    ...
    promptConstraints: ["no bgm", "first frame plus motion prompt by default", "endpoint end frame only when explicitly selected", "no text-to-video fallback", "no fast model", "no VIP channel", "preserve character identity, scene layout, and style capsule"],
    preflightFacts: {
...
if (!runtimeState.providerExecutionPermissionGate) {
  runtimeState.providerExecutionPermissionGate = buildProviderExecutionPermissionGateState({
    ...
    agentCliAdapterSpike: runtimeState.agentCliAdapterSpike,
  });
}
...
assert(runtimeState.localOrchestrator.queue.every((item) => item.canSpawnAgent === false), "localOrchestrator queue items must never spawn Agent");
