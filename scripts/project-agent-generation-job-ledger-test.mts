...
assert(/agentGenerationLedgerMatchesProject[\s\S]*restoredAgentGenerationJobLedger[\s\S]*runConfiguredAgentVideoExecution\(\{[\s\S]*onLedgerSnapshot:[\s\S]*input\.executionMode === "live" && !onRememberAgentGenerationJobLedger[\s\S]*throw new Error[\s\S]*await onRememberAgentGenerationJobLedger\?\.\(ledgerSnapshot\)[\s\S]*agentVideoExecutionLedgerRef\.current = ledgerSnapshot[\s\S]*setAgentVideoDryRunLedger\(ledgerSnapshot\)[\s\S]*onTimelineEntries:[\s\S]*rememberAgentTimelineEntries\(entries,\s*executionIdentity\)/.test(minimalAgentPanelSource), "MinimalAgentPanel must durably persist fact-bound live job transitions before publishing them in memory or timeline results");
assert(/const executionIdentity = \{[\s\S]*projectFactHash:\s*input\.projectFactHash[\s\S]*!agentGenerationLedgerMatchesProject\(executionLedger,\s*executionIdentity\)[\s\S]*createAgentVideoGenerationJobLedger\(\{[\s\S]*\.\.\.executionIdentity/.test(minimalAgentPanelSource), "post-write project facts must start execution from a matching generation ledger instead of appending to a stale ledger");
