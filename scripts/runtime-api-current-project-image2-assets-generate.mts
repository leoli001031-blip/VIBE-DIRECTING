...
function agentTaskEnvelopeBlockers(envelope) {
  ...
  return uniqueStrings([
    ...
    envelope.handler === "image2_reference_generation" ? "" : "Agent 任务不是补参考动作。",
    envelope.expectedReceipt === "image_reference_receipt" ? "" : "Agent 补参考任务回执类型不正确。",
    envelope.preflight?.projectWriteReceiptRequired ? "" : "Agent 补参考任务缺少项目写入回执要求。",
    envelope.preflight?.ruleQaRequired ? "Agent 补参考任务不应要求视频规则 QA。" : "",
    envelope.preflight?.textQaRequired ? "Agent 补参考任务不应要求视频文本 QA。" : "",
    envelope.preflight?.noBgmGuardRequired ? "Agent 补参考任务不应携带 no BGM 视频保护。" : "",
    envelope.preflight?.providerSubmitAfterPreflightOnly ? "Agent 补参考任务不应携带视频提交前置门。" : "",
  ]);
}
...
export function createRuntimeApiCurrentProjectImage2AssetGenerate(deps) {
  ...
  async function currentProjectImage2AssetGenerateResponse(input, extra = {}, source) {
    ...
    const blockers = uniqueStrings([
      ...
      workbenchFacts?.visualMemory?.readable === true ? "" : "项目资产文件读不到。先保存草案，或重新打开项目文件夹。",
