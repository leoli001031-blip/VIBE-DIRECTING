...
import { buildLocalOrchestratorState, localOrchestratorHardLocks } from "../src/core/localOrchestrator.ts";
...
const stalledState = buildLocalOrchestratorState({
  ...
  taskRuns: [
    taskRun("image_task_plan_07", {
      ...
      retryCount: 2,
      agentSessionId: "agent_stalled_session",
      lastEventAt: "2026-04-30T00:00:00.000Z",
...
assert(stalledItem.agentActivity.stalled === true, "stalled task must expose stalled=true");
assert(stalledItem.agentActivity.retryBudget === 2, "retry budget must be recorded");
assert(stalledItem.agentActivity.retriesRemaining === 0, "exhausted retry budget must be recorded");
assert(stalledItem.agentActivity.manualReviewRequired === true, "stalled task must require manual review");
assert(stalledItem.agentActivity.stallDetectedBy.includes("timeout"), "stalled task should include timeout evidence");
...
for (const state of [twentyTaskState, autoContinueState, stalledState, selfReportState, qaMissingState, coverageGapState]) {
  ...
  assert(state.hardLocks.noSpawnAgent === true, "hard lock noSpawnAgent must be true");
...
assert(schema.$defs.hardLocks.properties.noSpawnAgent.const === true, "schema must pin noSpawnAgent true");
...
assert(packageJson.scripts["orchestrator:test"] === "tsx scripts/local-orchestrator-test.mts", "package.json must expose orchestrator:test");
