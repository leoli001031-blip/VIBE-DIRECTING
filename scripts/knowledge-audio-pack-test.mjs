...
function moduleUrl(filePath, output) {
  return `data:text/javascript;base64,${Buffer.from(`${output}\n//# sourceURL=${pathToFileURL(filePath).href}`).toString("base64")}`;
}

async function importKnowledgeRouter() {
  const manifestUrl = moduleUrl("src/core/knowledgeManifest.ts", transpile("src/core/knowledgeManifest.ts"));
  const contextBudgetOutput = transpile("src/core/knowledgeContextBudget.ts").replace(
    /from "\.\/knowledgeManifest";/g,
    `from "${manifestUrl}";`,
  );
  const contextBudgetUrl = moduleUrl("src/core/knowledgeContextBudget.ts", contextBudgetOutput);
  const routerOutput = transpile("src/core/knowledgeRouter.ts")
    .replace(/from "\.\/knowledgeManifest";/g, `from "${manifestUrl}";`)
    .replace(/from "\.\/knowledgeContextBudget";/g, `from "${contextBudgetUrl}";`);

  return import(moduleUrl("src/core/knowledgeRouter.ts", routerOutput));
}
...
const { routeKnowledge } = await importKnowledgeRouter();
