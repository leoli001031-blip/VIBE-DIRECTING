...
const fractionalDurationPlan = buildJimengImage2VideoPlan({
  imagePath: "/tmp/start.png",
  outputDir: "/tmp/video",
  prompt: "camera push in",
  durationSeconds: 5.7,
});
assert(fractionalDurationPlan.durationSeconds === 6, "Jimeng duration should round fractional planning seconds before CLI submission");
assert(
  fractionalDurationPlan.args[fractionalDurationPlan.args.indexOf("--duration") + 1] === "6",
  "Jimeng CLI args should receive integer duration seconds",
);
assert(
