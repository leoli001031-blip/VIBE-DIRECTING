...
} from "../src/core/betaAcceptance.ts";
...
function workerRuntimeGate(overrides = {}) {
  return {
    hardLocks: {
      noSpawnAgent: true,
      noShellExecution: true,
...
function runtimeState(overrides = {}) {
  return {
    ...
    workerRuntimeGate: workerRuntimeGate(),
...
assert(projectRuntimeSchema.$id === "https://vibecore.local/schemas/project_runtime_state.schema.json", "ProjectRuntimeState schema $id drifted");

const betaSchema = readJson("schemas/beta_acceptance.schema.json");
assert(betaSchema.required.includes("schemaVersion"), "beta acceptance schema must require schemaVersion");
