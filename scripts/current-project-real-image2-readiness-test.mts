...
try {
  ...
  for (const shotId of shotIds) {
    ...
    assert(prepare.payload.transportPlan?.mode === "agent_app_server", `${shotId} should prepare app-server handoff mode`);
    assert(prepare.payload.transportPlan?.target === "agent_app_server", `${shotId} app-server target missing`);
    assert(prepare.payload.transportPlan?.endpoint === "/api/agent/app-server/image2/one-shot", `${shotId} app-server endpoint drifted`);
    ...
    assert(confirm.payload.transportPlan?.mode === "agent_app_server", `${shotId} confirm should preserve app-server transport mode`);
    assert(confirm.payload.handoffPacket?.transportPlan?.mode === "agent_app_server", `${shotId} handoff packet should preserve app-server transport mode`);
    ...
    assert(reloaded.payload.transportPlan?.mode === "agent_app_server", `${shotId} persisted status should preserve app-server transport mode`);
    assert(reloaded.payload.handoffPacket?.transportPlan?.mode === "agent_app_server", `${shotId} persisted handoff should preserve app-server transport mode`);
