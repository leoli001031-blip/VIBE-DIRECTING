...
async function loadModule(sourcePath, exportPath) {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vibe-image-reference-delivery-"));
...
const {
  ...
  imageReferenceDeliveryReceiptSchemaVersion,
} = await loadModule("src/core/imageReferenceDeliveryReceipt.ts", "imageReferenceDeliveryReceipt.mjs");
...
function delivery(overrides = {}) {
  return {
    ...
    protocol: {
      ...
      actionId: "codex_app_server_image_action",
    },
    toolSchemaHash: "sha256:tool-schema",
    generatedSchemaVersion: "codex-app-server-schema-2026-05-07",
    ...overrides,
  };
}
...
assert(ready.delivery.protocol.generatedSchemaVersion === "codex-app-server-schema-2026-05-07", "generated schema version should be copied to protocol binding");
...
assert(schema.$id === "https://vibecore.local/schemas/image_reference_delivery_receipt.schema.json", "schema id drifted");
assert(schema.properties.transportPolicy.$ref === "#/$defs/transportPolicy", "transport policy schema missing");
...
assert(
  packageJson.scripts["image-reference-delivery-receipt:test"] === "node scripts/image-reference-delivery-receipt-test.mjs",
