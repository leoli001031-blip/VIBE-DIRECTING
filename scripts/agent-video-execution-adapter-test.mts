...
assert(/agentVideoExecutionInFlightRef[\s\S]*existingExecution[\s\S]*return existingExecution[\s\S]*\.set\(inFlightKey, execution\)[\s\S]*\.delete\(inFlightKey\)/.test(minimalAgentPanelSource), "the shared UI entry must collapse concurrent calls for the same project action and operation");
