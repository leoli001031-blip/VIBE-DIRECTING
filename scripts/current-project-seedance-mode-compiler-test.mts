...
const agentTaskEnvelope = {
  ...
  inputHash: "sha256:agent-tool-seedance-submit-test",
  policyBinding: "director_agent_tool_handoff",
  actionId: "agent_action_seedance_submit_test_001",
  handoffId: "agent_handoff_seedance_submit_test_001",
  handler: "seedance_video_submit",
  expectedReceipt: "video_submit_receipt",
  preflight: {
    projectWriteReceiptRequired: true,
    ruleQaRequired: true,
    textQaRequired: true,
    noBgmGuardRequired: true,
    providerSubmitAfterPreflightOnly: true,
  },
};
const unconfirmedResponse = await route.currentProjectSeedanceSubmitResponse({
  confirmation: {
    confirmed: false,
    phrase: "submit-seedance-video",
    receiptId: "receipt_unconfirmed",
...
assert(unconfirmedResponse.ok === false, "unconfirmed submit must be blocked");
assert(unconfirmedResponse.relayQueue?.storyboardConfirmed === false, "unconfirmed submit must not expose a confirmed relay queue");
assert(unconfirmedResponse.relayQueue?.autoSubmitAllowed === false, "unconfirmed submit must not expose an auto-submittable relay queue");
assert(String(unconfirmedResponse.message || "").includes("需要确认"), "unconfirmed submit should explain that confirmation is required");
...
assert(response.agentTaskEnvelopeId === agentTaskEnvelope.id, "Seedance submit should preserve the Agent tool task id");
assert(response.agentTaskInputHash === agentTaskEnvelope.inputHash, "Seedance submit should preserve the Agent tool input hash");
assert(response.agentTaskEnvelope?.preflight?.textQaRequired === true, "Seedance submit should preserve the Agent text QA preflight contract");
assert(response.agentTaskEnvelope?.preflight?.noBgmGuardRequired === true, "Seedance submit should preserve the Agent no-BGM preflight contract");
...
const groupedResumeResponse = await route.currentProjectSeedanceResumeResponse({
  pollSeconds: 30,
...
assert(groupedResumeResponse.relayQueue?.counts?.completed === 1, "resume query should mark the recovered segment complete");
assert(groupedResumeResponse.relayQueue?.autoSubmitAllowed === true, "resume query should unlock the next ready serial segment");
assert(groupedResumeResponse.relayQueue?.nextReadyItemId === "seedance_segment_2", "resume query should point to the next ready segment");
const resumedPreviewPlan = JSON.parse(readFileSync(groupedSource.previewPlanPath, "utf8"));
...
const genericSceneRunRootRelativePath = ".vibe-runtime/test-current-project-seedance-mode-compiler-generic-scene";
const genericSceneRunRootPath = path.resolve(repoRoot, genericSceneRunRootRelativePath);
rmSync(genericSceneRunRootPath, { recursive: true, force: true });
mkdirSync(genericSceneRunRootPath, { recursive: true });
writeFile(path.join(genericSceneRunRootPath, "assets/blue-salt-archive.png"), tinyPng);
writeFile(path.join(genericSceneRunRootPath, "assets/girl.png"), tinyPng);
const genericSceneSource = {
  ...source,
  runRootPath: genericSceneRunRootPath,
  runRootRelativePath: genericSceneRunRootRelativePath,
  projectVibePath: path.join(genericSceneRunRootPath, "project/project.vibe"),
  projectVibeRelativePath: `${genericSceneRunRootRelativePath}/project/project.vibe`,
  previewPlanPath: path.join(genericSceneRunRootPath, "reports/preview_plan.json"),
  previewPlanRelativePath: `${genericSceneRunRootRelativePath}/reports/preview_plan.json`,
};
projectFacts = {
  projectVibe: {
    shots: [
      {
        id: "U01",
        storyboardGroupId: "blue_salt_archive_sequence",
        title: "蓝盐档案厅门口",
        durationSeconds: 4,
        executionMode: "relationship_wide",
        referenceStrategy: "storyboard_narrative",
        rhythmProfile: "anime_emotion",
        intent: "少女推开蓝盐档案厅的厚门，冷色灰尘从门缝里浮出来。",
        camera: "低机位轻微推进门缝。",
        primaryAction: "她推开档案厅门。",
        sceneGuidance: ["蓝盐档案厅"],
        characterGuidance: ["短发少女"],
      },
      {
        id: "U02",
        storyboardGroupId: "blue_salt_archive_sequence",
        ...
        referenceStrategy: "storyboard_narrative",
        rhythmProfile: "anime_emotion",
        intent: "同一个蓝盐档案厅里，她从发冷的书架上抽出一张透明卡片。",
        camera: "俯拍手部后轻微上摇。",
        primaryAction: "她抽出透明卡片。",
        sceneGuidance: ["蓝盐档案厅"],
        characterGuidance: ["短发少女"],
      },
    ],
...
workbenchFacts.visualMemory.assets = [
  {
    type: "scene",
    id: "blue_salt_archive",
    name: "蓝盐档案厅",
    path: `${genericSceneRunRootRelativePath}/assets/blue-salt-archive.png`,
    usedByShotIds: ["U01", "U02"],
  },
  {
    type: "character",
    name: "短发少女",
    path: `${genericSceneRunRootRelativePath}/assets/girl.png`,
    usedByShotIds: ["U01", "U02"],
  },
];
const genericSceneResponse = await route.currentProjectSeedanceSubmitResponse({
  confirmation: {
    confirmed: true,
    phrase: "submit-seedance-video",
    receiptId: "receipt_test_generic_scene",
    confirmedAt: "2026-05-23T00:00:00.000Z",
  },
  selectedShotIds: ["U02"],
  modelVersion: "seedance2.0",
...
assert(genericSceneResponse.ok === true, `generic bound scene submit should pass: ${JSON.stringify(genericSceneResponse)}`);
const genericSceneManifestPath = path.join(path.dirname(path.resolve(repoRoot, genericSceneResponse.promptPath)), "input-manifest.json");
const genericSceneManifest = JSON.parse(readFileSync(genericSceneManifestPath, "utf8"));
assert(genericSceneManifest.segmentPlan.length === 1, "shared bound scene asset should group unknown scene names without relying on keyword lists");
