...
check(
  /targetShotRevisionIndexForRows\(feedbackText,\s*rows,\s*selectedRowId\)/.test(applyTargetedShotRevisionRows)
    ...
    && /const withoutSafetyClauses = stripDraftRevisionPromptPrefix\(text\)[\s\S]*replace\(safetyClausePattern,\s*" "\)[\s\S]*stripDirectorAgentPermissionControlPhrases\(withoutSafetyClauses\)/.test(cleanTargetShotRevisionText)
