...
import { buildSubagentRuntimeGateReceipt, subagentRuntimeGateHardLocks, validateSubagentRuntimeGateHardLocks } from "../src/core/subagentRuntimeGate.ts";
...
function subagentEnvelope(id = "subagent_video_A1_01", parentTaskId = "task_video_A1_01") {
  ...
  const nonOverridableGateHashes = task.nonOverridableGateHashes || buildNonOverridableGateHashes({ ...task, policyBinding });
  const base = {
    schemaVersion: envelopeSchemaVersion,
    id,
    parentTaskId,
  ...
  const inputHash = buildSubagentInputHash(base);
