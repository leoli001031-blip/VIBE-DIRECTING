...
import { buildDefaultProviderRegistry } from "../src/core/providerCapabilities.ts";
...
function promptPlan(overrides = {}) {
  ...
  return {
    ...
    shotId: "S01",
    providerId: isEnd ? "openai-image2-api" : "openai-image2-agent-cli",
...
function baseInput(overrides = {}) {
  return {
    ...
    assets,
    jobs: [
      job("job_S01_start", "image.generate", "text2image", "openai-image2-agent-cli", "outputs/keyframes/S01_start.png"),
...
assert(startPlan.adapterRequestPreview.adapterId === "openai-image2-agent-cli-dry-run", "start frame adapter preview must use the registry-selected provider adapter id");
...
assert(schema.$schema === "https://json-schema.org/draft/2020-12/schema", "schema $schema missing");
...
assert(packageJson.scripts["image-keyframe:test"] === "tsx scripts/image-keyframe-runtime-test.mts", "package script image-keyframe:test missing");
