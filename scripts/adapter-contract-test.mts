...
const agent = byId.get("agent-cli");
assert(agent.kind === "agent", "agent-cli-agent must be an agent contract");
...
const appServerAgent = byId.get("agent-app-server");
assert(appServerAgent.kind === "agent", "agent-app-server-agent must be an agent contract");
assert(appServerAgent.runtimeKind === "agent_app_server", "agent-app-server-agent must declare app-server runtime kind");
assert(appServerAgent.state === "planned", "agent-app-server-agent must remain planned");
assert(appServerAgent.dryRunOnly === true, "agent-app-server-agent must be dry-run only");
assert(appServerAgent.readOnly === true, "agent-app-server-agent must be read-only");
assert(appServerAgent.liveSubmitAllowed === false, "agent-app-server-agent must not allow live submit");
assert(appServerAgent.credentialStorage === false, "agent-app-server-agent must not store credentials");
assert(appServerAgent.uiBinding === false, "agent-app-server-agent cannot bind UI");
...
assert(appServerAgent.notes.some((note) => note.includes("legacy agent exec --json fallback")), "app-server agent must preserve legacy exec fallback note");
...
assert(image2.providerIds.includes("openai-image2-agent-cli"), "Image2 must cover agent CLI Image2 provider id");
...
assert(schema.$schema === "https://json-schema.org/draft/2020-12/schema", "adapter contract schema $schema missing");

const projectSchema = readJson("schemas/project_runtime_state.schema.json");
assert(projectSchema.$schema === "https://json-schema.org/draft/2020-12/schema", "project runtime schema $schema missing");
