...
async function loadModule(sourcePath, exportPath) {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vibe-image-reference-transport-"));
...
const {
  buildImageReferenceTransport,
  imageReferenceTransportSchemaVersion,
} = await loadModule("src/core/imageReferenceTransport.ts", "imageReferenceTransport.mjs");
...
function explicitImageAction(overrides = {}) {
  return {
    actionId: "codex_image2_explicit_local_input",
...
assert(schema.$id === "https://vibecore.local/schemas/image_reference_transport.schema.json", "schema id drifted");
...
assert(
  packageJson.scripts["image-reference-transport:test"] === "node scripts/image-reference-transport-test.mjs",
