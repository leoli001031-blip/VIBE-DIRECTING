...
} from "../src/core/providerClosedLoopShell.ts";
...
function seedanceVideoPlan(overrides = {}) {
  return {
    ...
    promptConstraints: ["no bgm", "first frame plus motion prompt by default", "endpoint end frame only when explicitly selected", "no text-to-video fallback", "no fast model", "no VIP channel"],
...
assert(schema.$schema === "https://json-schema.org/draft/2020-12/schema", "provider closed loop shell schema $schema missing");

const projectRuntimeSchema = readJson("schemas/project_runtime_state.schema.json");
assert(projectRuntimeSchema.$schema === "https://json-schema.org/draft/2020-12/schema", "project runtime schema $schema missing");
...
const projectVibeSource = fs.readFileSync("src/project/projectVibe.ts", "utf8");

const packageJson = readJson("package.json");
assert(
  packageJson.scripts["provider-closed-loop-shell:test"] === "tsx scripts/provider-closed-loop-shell-test.mts",
  "package script provider-closed-loop-shell:test missing",
);
assert(
  packageJson.scripts["provider-qa-promotion-gate:test"] === "tsx scripts/provider-qa-promotion-gate-test.mts",
  "provider qa promotion gate script should run the focused Project.vibe review/promotion contract test",
);
assert(
  projectVibeSource.includes("buildProjectVibeReviewPromotionTransaction") &&
    projectVibeSource.includes("missing_output_cannot_promote") &&
    projectVibeSource.includes("late_output_cannot_promote_directly") &&
    projectVibeSource.includes("retry_shot_id_required"),
  "Project.vibe must own first-pass review/promotion contract for missing, late, and per-shot retry",
);
