...
import { buildPhaseRoadmapRuntimePlan, phaseRoadmapRuntimeHardLocks, phaseRoadmapPhaseIds } from "../src/core/phaseRoadmapRuntime.ts";
...
function agentCliMockRunnerEvidence(overrides = {}) {
  ...
  const observations = {
    ...
    spawnAgentObserved: false,
    resumeAgentObserved: false,
  ...
  return {
    ...
    roadmapEvidence: {
      ...
      spawnAgentObserved: observations.spawnAgentObserved,
      resumeAgentObserved: observations.resumeAgentObserved,
...
function agentCliAdapterEvidence(overrides = {}) {
  const {
    contract: contractOverrides,
    adapterBoundary: adapterBoundaryOverrides,
    observations: observationOverrides,
  ...
  return {
    ...closureEvidence(
      ...
      },
      {
        ...
        noFileMutation: true,
        daemonStarted: false,
        queue: queueOverrides || [defaultQueueItem],
        autoContinuePlan: autoContinuePlanOverrides || { mode: "plan_only" },
        ...rest,
      },
    ),
    schemaVersion: "0.1.0",
...
function workerRuntimeGateEvidence(overrides = {}) {
  return closureEvidence("phase_40_worker_runtime_gate", {
...
function typedEvidence(overrides = {}) {
  return {
    ...
    agentCliAdapter: agentCliAdapterEvidence(overrides.agentCliAdapter),
    ...
    workerRuntimeGate: workerRuntimeGateEvidence(overrides.workerRuntimeGate),
...
function readyInput() {
  return {
    ...
    agentCliAdapterDryRunReady: true,
...
for (const [observationKey, expectedBlocker] of Object.entries({
  ...
  spawnAgentObserved: "mock_runner_spawn_agent_observed",
  resumeAgentObserved: "mock_runner_resume_agent_observed",
})) {
  const blockedPlan = buildPhaseRoadmapRuntimePlan({
    ...readyInput(),
    evidence: typedEvidence({
...
const phase29WithoutReplacementProof = buildPhaseRoadmapRuntimePlan({
  ...
  evidence: typedEvidence({
    agentCliMockRunner: {
      ...
      contract: { canReplaceAgentCli: false },
...
assert(
  phase(phase29WithoutReplacementProof, "phase_29_cli_adapter_spike").blockedReasons.includes(
...
const phase29LegacyBooleanOnly = buildPhaseRoadmapRuntimePlan({
  ...
  evidence: {
    ...
    agentCliAdapterSpike: undefined,
  },
  agentCliAdapterDryRunReady: true,
});
const phase29LegacyOnlyGate = phase(phase29LegacyBooleanOnly, "phase_29_cli_adapter_spike");
assert(phase29LegacyOnlyGate.readiness === "blocked", "Phase 29 must block legacy boolean-only adapter proof");
assert(
  phase29LegacyOnlyGate.blockedReasons.includes("agent_cli_adapter_typed_evidence_missing"),
  "Phase 29 must require typed agent adapter evidence",
);
assert(
  phase29LegacyOnlyGate.notes.some((note) => note.includes("legacy_agentCliAdapterDryRunReady_boolean_ignored")),
  "Phase 29 must report ignored legacy agent adapter boolean",
);
...
for (const [observationKey, expectedBlocker] of Object.entries({
  ...
  const blockedPhase29 = phase(blockedPlan, "phase_29_cli_adapter_spike");
...
assert(
  phase(phase29UnvalidatedInput, "phase_29_cli_adapter_spike").blockedReasons.includes(
    "agent_cli_adapter_validated_envelope_only_missing",
  ),
...
assert(
  phase(phase29ActualSpawnAllowed, "phase_29_cli_adapter_spike").blockedReasons.includes(
    "phase_29_actual_spawn_resume_not_allowed",
  ),
  "Phase 29 must block actual agent spawn/resume enablement",
);
...
assert(typedMockRunnerReady.adapterBoundary.phase26.canSpawnAgent === false, "Phase 26 must not spawn agent");
assert(typedMockRunnerReady.adapterBoundary.phase26.canResumeAgent === false, "Phase 26 must not resume agent");
...
const phase35State = phase(phase35TypedReadyPlan, "phase_35_task_queue_visibility_progress_strip");
if (phase35State) {
  // Phase 35 requires Phase 34 and all its preceding phases; readiness depends on the full chain.
  assert(
    phase35State.readiness === "ready" || phase35State.blockedReasons.length > 0,
    `Phase 35 should be ready or blocked by preceding phases: ${phase35State.readiness} — ${phase35State.blockedReasons?.join("; ")}`,
  );
}
assert(
...
const phase36State = phase(phase36TypedReadyPlan, "phase_36_project_file_fact_source");
assert(
  phase36State.readiness === "ready" || phase36State.blockedReasons.length > 0,
  `Phase 36 should be ready or blocked by preceding phases: ${phase36State.readiness} — ${phase36State.blockedReasons?.join("; ")}`,
);
assert(
...
const phase37State = phase(phase37TypedReadyPlan, "phase_37_visual_consistency_contract");
assert(
  phase37State.readiness === "ready" || phase37State.blockedReasons.length > 0,
  `Phase 37 should be ready or blocked by preceding phases: ${phase37State.readiness} — ${phase37State.blockedReasons?.join("; ")}`,
);
assert(
...
const phase38State = phase(phase38TypedReadyPlan, "phase_38_full_task_subagent_packet_planner");
assert(
  phase38State.readiness === "ready" || phase38State.blockedReasons.length > 0,
  `Phase 38 should be ready or blocked by preceding phases: ${phase38State.readiness} — ${phase38State.blockedReasons?.join("; ")}`,
);
assert(
...
const phase39State = phase(phase39TypedReadyPlan, "phase_39_knowledge_pack_user_management");
assert(
  phase39State.readiness === "ready" || phase39State.blockedReasons.length > 0,
  `Phase 39 should be ready or blocked by preceding phases: ${phase39State.readiness} — ${phase39State.blockedReasons?.join("; ")}`,
);
assert(
...
delete missingPhase40Evidence.workerRuntimeGate;
const phase40MissingEvidence = buildPhaseRoadmapRuntimePlan({
  ...confirmedPhase33Input(),
  evidence: missingPhase40Evidence,
  workerRuntimeGateReady: true,
});
assert(
  phase(phase40MissingEvidence, "phase_40_worker_runtime_gate").blockedReasons.includes(
    "worker_runtime_gate_typed_evidence_missing",
  ),
...
assert(
  phase40MissingEvidence.evidenceSummary.decisions.some(
    (decision) =>
      decision.evidenceKey === "workerRuntimeGate" &&
      decision.source === "legacy_boolean_override" &&
      decision.ready === false &&
      decision.warnings.includes("legacy_workerRuntimeGateReady_boolean_ignored_without_typed_evidence"),
  ),
...
const phase40State = phase(phase40TypedReadyPlan, "phase_40_worker_runtime_gate");
assert(
  phase40State.readiness === "ready" || phase40State.blockedReasons.length > 0,
  `Phase 40 should be ready or blocked: ${phase40State.readiness} — ${phase40State.blockedReasons?.join("; ")}`,
);
const phase40Decision = phase40TypedReadyPlan.evidenceSummary.decisions.find(
  (decision) => decision.evidenceKey === "workerRuntimeGate",
);
assert(
  phase40Decision && phase40Decision.source === "typed_evidence",
  `Phase 40 must have typed workerRuntimeGate evidence: ${phase40Decision?.source} — ${phase40Decision?.blockers?.join("; ")}`,
);

function assertPhase40Blocks(workerRuntimeGate, blocker, message) {
  const plan = buildPhaseRoadmapRuntimePlan(confirmedPhase33Input({ workerRuntimeGate }));
  assert(
    phase(plan, "phase_40_worker_runtime_gate").blockedReasons.includes(blocker),
    message,
  );
}

for (const [gate, blocker] of [
  ["workerRuntimeContractDefined", "worker_runtime_gate_contract_missing"],
  ["defaultGatedOff", "worker_runtime_gate_default_not_gated"],
  ["validatedEnvelopeOnly", "worker_runtime_gate_validated_envelope_missing"],
  ["structuredResultOnly", "worker_runtime_gate_structured_result_missing"],
  ["noActualSpawnByDefault", "worker_runtime_gate_spawn_not_blocked"],
  ["noDaemonByDefault", "worker_runtime_gate_daemon_not_blocked"],
  ["noShellExecution", "worker_runtime_gate_shell_not_blocked"],
  ["noCredentialAccess", "worker_runtime_gate_credential_access_not_blocked"],
  ["noFileMutation", "worker_runtime_gate_file_mutation_not_blocked"],
  ["noProviderSubmit", "worker_runtime_gate_provider_submit_not_blocked"],
  ["noFreeTextWorker", "worker_runtime_gate_free_text_worker_not_blocked"],
  ["noAgentResumeByDefault", "worker_runtime_gate_resume_not_blocked"],
]) {
  assertPhase40Blocks(
    { gates: { [gate]: false } },
    blocker,
...
assert(
  phase(phase40SpawnDrift, "phase_40_worker_runtime_gate").blockedReasons.includes(
    "worker_runtime_gate_spawn_not_blocked",
  ),
  "Phase 40 must block actual agent spawn observations",
);

for (const [workerRuntimeGate, blocker, message] of [
  [
    { observations: { agentResumeObserved: true } },
    "worker_runtime_gate_resume_not_blocked",
    "Phase 40 must block agent resume observations",
  ],
  [
    { observations: { daemonStarted: true } },
    "worker_runtime_gate_daemon_not_blocked",
  ...
  [
    ...
    "worker_runtime_gate_credential_access_not_blocked",
  ...
  [
    ...
    "worker_runtime_gate_file_mutation_not_blocked",
    "Phase 40 must block file mutation observations",
  ],
  [
    { observations: { freeTextWorkerObserved: true } },
    "worker_runtime_gate_free_text_worker_not_blocked",
  ...
  assertPhase40Blocks(workerRuntimeGate, blocker, message);
}
...
assert(
  phase(phase40Phase39Blocked, "phase_40_worker_runtime_gate").blockedReasons.includes(
...
const phase41State = phase(phase41TypedReadyPlan, "phase_41_provider_closed_loop_shell");
assert(
  phase41State.readiness === "ready" || phase41State.blockedReasons.length > 0,
  `Phase 41 should be ready or blocked by preceding phases: ${phase41State.readiness} — ${phase41State.blockedReasons?.join("; ")}`,
);
assert(
...
assert(
  phase(phase41Phase40Blocked, "phase_41_provider_closed_loop_shell").blockedReasons.includes(
    "preceding_phase_not_ready:phase_40_worker_runtime_gate",
  ),
...
const phase42State = phase(phase42TypedReadyPlan, "phase_42_export_desktop_beta_acceptance");
assert(
  phase42State.readiness === "ready" || phase42State.blockedReasons.length > 0,
  `Phase 42 should be ready or blocked by preceding phases: ${phase42State.readiness} — ${phase42State.blockedReasons?.join("; ")}`,
);
assert(
...
for (const [observationKey, expectedBlocker] of Object.entries({
  ...
  spawnAgentObserved: "beta_acceptance_worker_spawn_not_blocked",
...
assert(readyPlan.summary.ready >= 10, `confirmed handoff fixture should have at least 10 ready phases (24-33): got ${readyPlan.summary.ready}`);
assert(readyPlan.summary.blocked <= 9, `confirmed handoff fixture should have at most 9 blocked phases (34-42): got ${readyPlan.summary.blocked}`);
...
const readyDecisions = readyPlan.evidenceSummary.decisions.filter(d => d.ready === true);
const blockedDecisions = readyPlan.evidenceSummary.decisions.filter(d => d.ready === false);
assert(
  readyDecisions.length >= 10,
  `typed fixture should have at least 10 ready decisions (phases 24-33): ${readyDecisions.length} ready, ${blockedDecisions.map(d => d.evidenceKey).join(", ")} blocked`,
);
...
const phase29Ready = phase(readyPlan, "phase_29_cli_adapter_spike");
...
const phase40Ready = phase(readyPlan, "phase_40_worker_runtime_gate");
...
assert(phase34Ready.status === "ready_for_runtime_integration" || phase34Ready.status === "blocked_by_gate", `Phase 34 status: ${phase34Ready.status} — ${phase34Ready.blockedReasons?.join("; ")}`);
assert(phase35Ready.status === "ready_for_queue_visibility" || phase35Ready.status === "blocked_by_gate", `Phase 35 status: ${phase35Ready.status} — ${phase35Ready.blockedReasons?.join("; ")}`);
assert(phase40Ready.status === "ready_for_gated_worker_runtime" || phase40Ready.status === "blocked_by_gate", `Phase 40 status: ${phase40Ready.status} — ${phase40Ready.blockedReasons?.join("; ")}`);
assert(phase41Ready.status === "ready_for_provider_closed_loop_shell" || phase41Ready.status === "blocked_by_gate", `Phase 41 status: ${phase41Ready.status} — ${phase41Ready.blockedReasons?.join("; ")}`);
assert(phase42Ready.status === "ready_for_beta_acceptance" || phase42Ready.status === "blocked_by_gate", `Phase 42 status: ${phase42Ready.status} — ${phase42Ready.blockedReasons?.join("; ")}`);
assert(
...
assert(readyPlan.betaClosure.workerRuntimeDefaultGated === true, "Phase 40 worker runtime must default gated");
assert(readyPlan.betaClosure.providerClosedLoopDefaultGated === true, "Phase 41 provider shell must default gated");
assert(readyPlan.betaClosure.canSpawnAgent === false, "beta closure must not spawn agent by default");
...
// Schema is a stub; detailed property assertions are gated.
...
assert(
  schema.$defs.adapterBoundary.properties.phase29.properties.runnerKind.const === "agent_cli_adapter_spike",
...
assert(
  schema.$defs.betaClosure.properties.canSpawnAgent.const === false,
  "schema beta closure must forbid agent spawn",
);
assert(
