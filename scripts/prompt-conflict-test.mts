...
import { buildPromptConflictCheckerState } from "../src/core/promptConflictChecker.ts";
...
const shots = [
  ...
  shot("shot_complex_action", "one short video beat with one primary action"),
];
...
const plans = [
  ...
  plan("complex_action", "shot_complex_action", ["Director action QA: Action beats in order: opens the door -> runs across the hall -> grabs the key -> turns around."], {
...
assert(codesFor("prompt_complex_action").includes("one_shot_complex_action_chain"), "must detect complex one-shot action chain before video prompt");
assert(harness.hardLocks.agentPromiseCannotResolveConflict === true, "agent promise hard lock must be true");
assert(harness.summary.recompileRequired === 6, "every conflict item must require recompile");
...
assert(schema.$defs.conflictCode.enum.includes("one_shot_complex_action_chain"), "schema must include one-shot complex action conflict");
