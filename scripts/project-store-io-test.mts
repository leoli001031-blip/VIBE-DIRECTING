...
} from "../src/core/projectStore.ts";
import {
  buildProjectStoreIoGate,
  executeProjectStoreIoGate,
} from "../src/core/projectStoreIo.ts";
...
const createGate = buildProjectStoreIoGate({
  mode: "create",
  snapshot,
  generatedAt,
  runtimeState: {
...
const createResult = await executeProjectStoreIoGate(createGate, adapter);
...
const openResult = await executeProjectStoreIoGate(openGate, adapter);
...
const tamperedWriteResult = await executeProjectStoreIoGate(tamperedWriteGate, tamperedWriteAdapter);
...
const tamperedMkdirResult = await executeProjectStoreIoGate(tamperedMkdirGate, tamperedMkdirAdapter);
...
const contentHashDriftResult = await executeProjectStoreIoGate(contentHashDriftGate, new MemoryProjectAdapter());
...
const blockedRuntimeGate = buildProjectStoreIoGate({
  mode: "create",
  snapshot,
  generatedAt,
  runtimeState: {
...
const blockedRuntimeResult = await executeProjectStoreIoGate(blockedRuntimeGate, new MemoryProjectAdapter());
...
const secretFactGate = buildProjectStoreIoGate({
  mode: "save",
...
const directInputAuthorityGate = buildProjectStoreIoGate({
  mode: "save",
...
const tamperedRuntimeResult = await executeProjectStoreIoGate(tamperedRuntimeGate, new MemoryProjectAdapter());
...
const malformedOpenGate = buildProjectStoreIoGate({
  mode: "open",
...
const malformedOpenResult = await executeProjectStoreIoGate(openGate, malformedOpenAdapter);
...
const invalidOpenResult = await executeProjectStoreIoGate(openGate, invalidProjectVibeAdapter);
...
const sidecarMismatchResult = await executeProjectStoreIoGate(openGate, sidecarMismatchAdapter);
...
const corruptSidecarResult = await executeProjectStoreIoGate(openGate, corruptSidecarAdapter);
...
const saveGate = buildProjectStoreIoGate({
  mode: "save",
  snapshot: applyProjectStorePatch(snapshot, {
    id: "phase19_patch",
...
const saveResult = await executeProjectStoreIoGate(saveGate, adapter);
...
const blockedGate = buildProjectStoreIoGate({
  mode: "save",
...
const blockedResult = await executeProjectStoreIoGate(blockedGate, adapter);
...
assert(schema.$schema === "https://json-schema.org/draft/2020-12/schema", "project store io schema $schema missing");
...
assert(packageJson.scripts["project-store-io:test"] === "tsx scripts/project-store-io-test.mts", "project-store-io:test script missing");
