...
function transpile(sourcePath, outPath, replacements = []) {
...
async function loadRuntimeModules() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vibe-runtime-ingest-"));
  const outDir = path.join(tmpDir, "src/core");
  transpile("src/core/taskRunLedger.ts", path.join(outDir, "taskRunLedger.mjs"));
  transpile("src/core/artifactTransactionGate.ts", path.join(outDir, "artifactTransactionGate.mjs"), [
    ['from "./taskRunLedger";', 'from "./taskRunLedger.mjs";'],
  ]);
  transpile("src/core/runtimeIngestShell.ts", path.join(outDir, "runtimeIngestShell.mjs"), [
    ['from "./artifactTransactionGate";', 'from "./artifactTransactionGate.mjs";'],
    ['from "./taskRunLedger";', 'from "./taskRunLedger.mjs";'],
  ]);

  return {
    taskRunLedger: await import(pathToFileURL(path.join(outDir, "taskRunLedger.mjs")).href),
    runtimeIngest: await import(pathToFileURL(path.join(outDir, "runtimeIngestShell.mjs")).href),
  };
}
...
const {
  ...
  runtimeIngest: {
    evaluateRuntimeArtifactFacts,
    ingestAppServerEvent,
    ingestQueueDecision,
    ingestWatcherFileEvent,
    ingestWorkerLease,
    runtimeIngestHardLocks,
  },
...
assert(schema.title === "RuntimeIngestShell", "runtime ingest schema title drifted");
for (const field of ["leaseId", "workerId", "threadId", "turnId", "retryBudget", "stallTimeoutSeconds", "resumePolicy"]) {
  assert(JSON.stringify(schema).includes(field), `runtime ingest schema must include ${field}`);
}
for (const eventType of ["created", "changed", "settled", "hash_observed", "sidecar_paired"]) {
  assert(schema.$defs.fileEventType.enum.includes(eventType), `runtime ingest schema must include watcher event ${eventType}`);
}
for (const appEventKind of ["thread", "turn", "toolCall", "fsChanged", "approval", "disconnect", "reconnect"]) {
  assert(schema.$defs.appServerEventKind.enum.includes(appEventKind), `runtime ingest schema must include app-server event ${appEventKind}`);
}
assert(JSON.stringify(schema).includes("verify_scan"), "runtime ingest schema must include verify_scan source");
assert(JSON.stringify(schema).includes("liveTransportEnabled"), "runtime ingest schema must include app-server hard locks");
