...
} from "../src/core/projectRealChainStatus.ts";
...
let recentChoicesErrorLogged = false;
console.error = (...args: unknown[]) => {
  if (String(args[0] || "").includes("loadCurrentProjectChoices")) recentChoicesErrorLogged = true;
  originalConsoleError(...args);
};
let closedChoices: Awaited<ReturnType<typeof loadCurrentProjectChoices>> = [];
try {
  closedChoices = await loadCurrentProjectChoices();
} finally {
  console.error = originalConsoleError;
}
assert(Array.isArray(closedChoices) && closedChoices.length === 0, "choices helper must fail closed to an empty list");
assert(!recentChoicesErrorLogged, "missing recent-project choices should fail closed without noisy console errors");
