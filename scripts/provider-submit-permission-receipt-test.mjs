...
async function loadModule(sourcePath, exportPath) {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vibe-provider-submit-permission-receipt-"));
...
} = await loadModule("src/core/providerSubmitPermissionReceipt.ts", "providerSubmitPermissionReceipt.mjs");
...
assert(schema.title === "ProviderSubmitPermissionReceiptState", "schema title drifted");
assert(schema.properties.schemaVersion.const === "0.1.0", "schema version const drifted");
assert(schema.$defs.status.enum.includes("pending_action_time_confirmation"), "schema must include pending status");
assert(schema.$defs.status.enum.includes("blocked"), "schema must include blocked status");
assert(schema.properties.maxProviderCallsPerReceipt.const === 1, "schema must pin maxProviderCallsPerReceipt=1");
assert(schema.$defs.submitIntent.properties.maxProviderCallsPerReceipt.const === 1, "schema must pin submitIntent max calls");
assert(schema.$defs.submitIntent.properties.providerSubmitAllowed.const === 0, "schema must pin submitIntent providerSubmitAllowed=0");
assert(schema.$defs.credential.properties.secretMaterialPresent.const === false, "schema must pin secretMaterialPresent=false");
assert(schema.$defs.credential.properties.credentialMaterialRead.const === false, "schema must pin credentialMaterialRead=false");
assert(schema.$defs.credential.properties.credentialMaterialStored.const === false, "schema must pin credentialMaterialStored=false");
assert(schema.$defs.hardLocks.properties.providerSubmitAllowed.const === 0, "schema must pin hard lock providerSubmitAllowed=0");
assert(schema.$defs.hardLocks.properties.externalNetworkIoAllowed.const === false, "schema must pin external network lock");
...
assert(schema.$defs.hardLocks.properties.noWorkerSpawn.const === true, "schema must pin noWorkerSpawn=true");
assert(schema.$defs.hardLocks.properties.noFileMutation.const === true, "schema must pin noFileMutation=true");
assert(schema.$defs.hardLocks.properties.maxConcurrency.const === 1, "schema must pin maxConcurrency=1");
assert(schema.$defs.hardLocks.properties.maxAutoRetries.const === 0, "schema must pin maxAutoRetries=0");
