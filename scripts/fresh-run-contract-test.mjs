...
async function loadModule(sourcePath, exportPath) {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vibe-fresh-run-contract-"));
...
const {
  ...
  freshRunContractSchemaVersion,
} = await loadModule("src/core/freshRunContract.ts", "freshRunContract.mjs");
