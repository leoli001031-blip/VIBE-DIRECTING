...
function assertPreviewClosedLoopAppContract() {
  ...
  assert(/buildCurrentProjectPreviewProjection\(\{[\s\S]*reviewReceipts:\s*prototypeProjectVibe\.receipts\?\.reviewReceipts/.test(appSource), "App must apply persisted human review receipts to current Preview state");
  assert(/returnedVideoPreviewCount[\s\S]*pendingVideoReviewCount[\s\S]*returnedCount:\s*returnedVideoPreviewCount[\s\S]*reviewCount:\s*pendingVideoReviewCount/.test(appSource), "App must pass structured returned and review counts into the Agent video state");
  ...
  assert(/buildMissingPreviewQueueFromShots/.test(appSource), "App must build a local story placeholder queue for visible projects without returned media");
  assert(/const\s+directorPreviewQueue\s*=\s*useMemo\(\s*\(\)\s*=>[\s\S]*currentProjectPreviewQueue\.length\s*\?\s*currentProjectPreviewQueue\s*:\s*localStoryPreviewQueue/.test(appSource), "App must prefer current project returned media and otherwise show local story placeholders");
  assert(/currentProjectPreviewItems=\{directorPreviewQueue\}/.test(appSource), "App must pass the visible director preview queue into the Preview view");
