...
async function loadModule(sourcePath, exportPath) {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vibe-task-run-ledger-"));
...
const {
  ...
  validatePortableTaskRunLedgerPaths,
} = await loadModule("src/core/taskRunLedger.ts", "taskRunLedger.mjs");
...
assert(schema.title === "TaskRunLedger", "schema title drifted");
for (const status of [
  "prepared",
  "queued",
  "leased",
...
for (const eventType of [
  ...
  assert(schema.$defs.eventType.enum.includes(eventType), `schema must include event ${eventType}`);
}
assert(JSON.stringify(schema).includes("reviewedOutputHash"), "schema must require reviewed output hash for QA");
assert(JSON.stringify(schema).includes("sidecarHash"), "schema must represent provider/QA sidecar hashes");
