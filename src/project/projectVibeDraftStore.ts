...
export interface ProjectVibeSidecarTextResult {
  ok: boolean;
  status: "read" | "written" | "missing" | "unavailable" | "error";
  mode?: ProjectVibeDraftStorageMode;
  targetId: string;
  path: string;
  content?: string;
...
export async function readProjectVibeSidecarText(
  ...
  const adapter = createProjectVibeDraftStorageAdapter(target);
  const targetId = projectVibeDraftTargetId(target);
  const sidecarPath = projectPathForTarget({ projectPath: path });
  if (!adapter) {
    return {
      ok: false,
      status: "unavailable",
      targetId,
      path: sidecarPath,
      errors: ["Project.vibe sidecar storage is unavailable."],
    };
  }
  try {
    if (adapter.adapter.existsFile && !(await adapter.adapter.existsFile(sidecarPath))) {
      return {
        ok: false,
        status: "missing",
        mode: adapter.mode,
        targetId,
        path: sidecarPath,
        errors: [`Project.vibe sidecar not found: ${sidecarPath}`],
      };
    }
    const content = await adapter.adapter.readFile(sidecarPath);
    return {
      ok: true,
      status: "read",
      mode: adapter.mode,
...
export async function writeProjectVibeSidecarText(
  target: ProjectVibeDraftTarget = {},
  path: string,
  content: string,
): Promise<ProjectVibeSidecarTextResult> {
