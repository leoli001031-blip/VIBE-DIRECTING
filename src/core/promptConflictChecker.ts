...
function buildConflicts(plan: ShotPromptPlan, report: PromptConflictReport | undefined, shot: ShotRecord | undefined, assets: AssetRecord[]): PromptConflictCheckerConflict[] {
  ...
  const refs = sourceRefs(plan, report, shot, lockedRefs);
  ...
  if (hasOutfitConflict(lockedText, text)) {
    conflicts.push(
      makeConflict({
  ...
  if ((garagePattern.test(lockedText) && frontDoorPattern.test(text)) || /\b(scene|location)\s*:\s*locked\b/i.test(lockedText) && /\bnew location|different scene\b/i.test(text)) {
    conflicts.push(
      makeConflict({
  ...
  if (hasStyleConflict(lockedText, text)) {
    conflicts.push(
      makeConflict({
