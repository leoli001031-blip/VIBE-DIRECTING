...
function isDraftPlaceholder(value: string): boolean {
  return /^(?:待确认|待补|待补充|待复核|待定|未定|unknown|pending|tbd|无|没有|none|n\/a|not applicable|-|—)$/iu.test(value);
}

function cleanProjectFactValue(value: unknown): string {
  const text = clean(value);
  return isDraftPlaceholder(text) ? "" : text;
}
...
function storyboardDraftUsableRows(
  ...
  return (rows || []).filter((row) => clean([
    ...
    row.camera,
    row.shotNo,
  ].map((item) => cleanProjectFactValue(item)).join(" ")));
}
...
function storyboardDraftQaBlockers(rows: NewVideoProjectVibeStoryboardDraftShotLike[]): ScriptPlannerResult["qaBlockers"] {
  ...
  const actionRows = rows.filter((row) => clean([
    ...
    row.visualDescription,
    row.camera,
  ].map((item) => cleanProjectFactValue(item)).join(" ")).length >= 8);
...
function storyboardVisibleClips(row: NewVideoProjectVibeStoryboardDraftShotLike, actionBeats: string[]): number {
  ...
  const budget = cleanProjectFactValue(row.visibleCutBudget);
...
function storyboardPanelCount(
  ...
  if (strategy === "storyboard_rapid_cut") return Math.max(2, Math.min(12, Math.max(visibleClips, actionBeats.length || 2)));
  return 0;
}
...
function splitStoryboardList(value: unknown): string[] {
  return unique(clean(value)
    .split(/[、,，/|]/u)
    .map((item) => cleanProjectFactValue(item))
    .filter(Boolean));
}

function storyboardShotTitle(row: NewVideoProjectVibeStoryboardDraftShotLike, index: number): string {
  return shortNote(cleanProjectFactValue(row.title) || cleanProjectFactValue(row.visualDescription) || `镜头 ${index + 1}`, 36);
}

function storyboardShotIntent(row: NewVideoProjectVibeStoryboardDraftShotLike, index: number): string {
  const subtitle = cleanProjectFactValue(row.subtitle);
  return [
    clean(row.shotNo) ? `镜号：${clean(row.shotNo)}` : `镜头 ${index + 1}`,
    clean(row.duration) ? `时长：${clean(row.duration)} 秒` : "",
    cleanProjectFactValue(row.shotSize) ? `景别：${cleanProjectFactValue(row.shotSize)}` : "",
    cleanProjectFactValue(row.camera) ? `镜头：${cleanProjectFactValue(row.camera)}` : "",
    cleanProjectFactValue(row.visualDescription) ? `画面：${cleanProjectFactValue(row.visualDescription)}` : "",
    cleanProjectFactValue(row.primaryAction) ? `主动作：${cleanProjectFactValue(row.primaryAction)}` : "",
    cleanProjectFactValue(row.actionTrigger) ? `触发：${cleanProjectFactValue(row.actionTrigger)}` : "",
    cleanProjectFactValue(row.microReaction) ? `微反应：${cleanProjectFactValue(row.microReaction)}` : "",
    cleanProjectFactValue(row.actionReactionQa) ? `行动反应：${cleanProjectFactValue(row.actionReactionQa)}` : "",
    cleanProjectFactValue(row.visibleCutBudget) ? `剪辑：${cleanProjectFactValue(row.visibleCutBudget)}` : "",
    subtitle ? `字幕：${subtitle}` : "",
    cleanProjectFactValue(row.sound) ? `声音：${cleanProjectFactValue(row.sound)}` : "",
    cleanProjectFactValue(row.characters) ? `角色：${cleanProjectFactValue(row.characters)}` : "",
    cleanProjectFactValue(row.scene) ? `场景：${cleanProjectFactValue(row.scene)}` : "",
    cleanProjectFactValue(row.props) ? `道具：${cleanProjectFactValue(row.props)}` : "",
  ].filter(Boolean).join("。");
}
...
function applyStoryboardDraftToPlanner(input: {
  ...
  rows.forEach((row, index) => {
    ...
    const visualAnchor = shortNote([
      ...
      cleanProjectFactValue(row.rhythmProfile),
      cleanProjectFactValue(row.executionMode),
    ...
    shots.push({
      ...
      camera: cleanProjectFactValue(row.camera) || undefined,
      videoControlMode: "reference_driven",
      executionMode: cleanProjectFactValue(row.executionMode) || undefined,
      rhythmProfile: cleanProjectFactValue(row.rhythmProfile) || undefined,
      splitPolicy: cleanProjectFactValue(row.visibleCutBudget) || undefined,
      visibleClips,
      storyboardPanels,
      actionBeats,
      primaryAction: cleanProjectFactValue(row.primaryAction) || undefined,
      actionTrigger: cleanProjectFactValue(row.actionTrigger) || undefined,
      microReaction: cleanProjectFactValue(row.microReaction) || undefined,
      seedanceDirection: shortNote(cleanProjectFactValue(row.actionReactionQa) || cleanProjectFactValue(row.rhythmReason), 180) || undefined,
      subtitle: cleanProjectFactValue(row.subtitle) || undefined,
      sound: cleanProjectFactValue(row.sound) || undefined,
      audioUsage: cleanProjectFactValue(row.audioUsage) || undefined,
      dialogueLines: cleanProjectFactValue(row.subtitle) ? [cleanProjectFactValue(row.subtitle)] : [],
      referenceStrategy,
      directorFeedbackDirectives: unique([
        cleanProjectFactValue(row.rhythmReason) ? `节奏判断：${cleanProjectFactValue(row.rhythmReason)}` : "",
        cleanProjectFactValue(row.audioUsage) ? `音频用途：${cleanProjectFactValue(row.audioUsage)}` : "",
      ]),
      ...
      sourceRefs,
      plannerNotes: {
        visibleAction: shortNote(cleanProjectFactValue(row.primaryAction) || cleanProjectFactValue(row.visualDescription) || title, 120),
        emotionalIntent: shortNote(cleanProjectFactValue(row.microReaction) || cleanProjectFactValue(row.rhythmReason) || cleanProjectFactValue(row.actionReactionQa), 120),
        visualAnchor,
      },
    });
  });
