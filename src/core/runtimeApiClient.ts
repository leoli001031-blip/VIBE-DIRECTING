...
export class RuntimeHttpError extends Error {
  ...
  constructor(url: string, status: number, detail?: string, payload?: unknown) {
    super(detail ? `${url} returned ${status}: ${detail}` : `${url} returned ${status}`);
...
function runtimePayloadErrorDetail(payload: unknown): string {
  if (!isRecord(payload)) return "";
  return firstString(payload.message)
    || firstString(payload.blockers)
...
async function runtimeHttpError(url: string, response: RuntimeFetchResponse) {
  ...
  const detail = runtimePayloadErrorDetail(payload) || text.trim().slice(0, 240);
  return new RuntimeHttpError(url, response.status, detail || undefined, payload);
}

export function runtimeErrorMessage(error: unknown, fallback: string) {
  if (error instanceof RuntimeHttpError && error.detail) return error.detail;
