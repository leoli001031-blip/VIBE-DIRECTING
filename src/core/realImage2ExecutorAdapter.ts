...
export function buildRealImage2ExecutorAdapterContract(input: RealImage2ExecutorAdapterInput): RealImage2ExecutorAdapterContract {
  ...
  const checks: RealImage2ExecutorAdapterCheck[] = [
    ...
    check("transport_mode_codex_app_server", transportPlan.mode === "codex_app_server", "Executor can only consume codex_app_server handoff transport."),
    check("transport_target_codex_app_server", transportPlan.target === "codex_app_server", "Executor can only consume codex_app_server target handoffs."),
    check("transport_endpoint_codex_app_server", transportPlan.endpoint === "/api/codex/app-server/image2/one-shot", "Codex app-server handoff endpoint drifted."),
    check("transport_prepared_only", transportPlan.externalCallPreparedOnly === true, "Codex app-server handoff must remain prepared-only before executor consumption."),
    check("app_server_contract_mode", appServerContract.mode === "codex_app_server_handoff_only", "App-server contract mode must remain handoff-only."),
