...
function inferProtagonist(scriptText: string): string | undefined {
  const match = scriptText.match(/(?:一个|一位|这位|那个)?([\p{Script=Han}A-Za-z0-9]{0,12}(?:女孩|男孩|女人|男人|老人|孩子|母亲|父亲|女儿|儿子|摄影师|画家|学生|店员|导演|护士|医生|程序员|少女|少年|骑手|车手|外卖员|放映员))/u);
...
function inferCharacterCandidates(scriptText: string): string[] {
  const candidates: string[] = [];
  const animalCandidates: Array<[RegExp, string]> = [
    [/黑猫/u, "黑猫"],
    [/白猫/u, "白猫"],
    [/橘猫/u, "橘猫"],
    [/狸花猫/u, "狸花猫"],
  ...
  for (const [pattern, label] of animalCandidates) {
    if (!pattern.test(scriptText)) continue;
    if (label === "猫" && candidates.some((candidate) => /猫$/u.test(candidate))) continue;
    if (label === "狗" && candidates.some((candidate) => /狗$/u.test(candidate))) continue;
    candidates.push(label);
  }

  const rolePattern = /(?:一个|一位|这位|那个)?([\p{Script=Han}A-Za-z0-9]{0,12}(?:女孩|男孩|女人|男人|老人|孩子|母亲|父亲|女儿|儿子|摄影师|画家|学生|店员|导演|护士|医生|程序员|少女|少年|骑手|车手|外卖员|放映员))/gu;
  for (const match of scriptText.matchAll(rolePattern)) {
    const label = normalizeCharacterCandidate(match[1]);
    if (label) candidates.push(label);
  }

  const protagonist = inferProtagonist(scriptText);
  if (protagonist) candidates.push(protagonist);
  return Array.from(new Set(candidates.map(normalizeCharacterCandidate).filter(Boolean))).slice(0, 5);
}
...
function stagedFactsFor(input: BuildDirectorSessionFromIntakeInput, sourceTurnId: string): DirectorStagedFact[] {
  ...
  if (scriptText) {
    ...
    inferCharacterCandidates(scriptText).forEach((character, index) => {
      facts.push(makeStagedFact({
        id: factId(input.draft.draftId, "character_candidate", character, index),
