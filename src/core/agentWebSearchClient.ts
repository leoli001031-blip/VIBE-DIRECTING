...
export type AgentWebSearchToolTrace = DirectorAgentToolTrace;
...
export function buildDirectorResearchQuery(userIntent: string): string {
  const intent = userIntent.trim().replace(/\s+/g, " ").slice(0, 180);
  if (!intent) return "";
  const hasChinese = /[\u4e00-\u9fff]/.test(intent);
  const suffix = hasChinese
    ? " 分镜方法 镜头语言 动作拆分 动画制作资料 production notes storyboard analysis cinematography reference"
    : " storyboard analysis cinematography reference production notes animation direction";
  return `${intent}${suffix}`.replace(/\s+/g, " ").trim().slice(0, 260);
}
...
export function buildAgentWebResearchSuggestion(userIntent: string, settings: AgentWebSearchSettings): AgentWebResearchSuggestion {
  ...
  const query = buildDirectorResearchQuery(intent);
  return {
...
export async function runAgentWebSearch(input: {
  ...
  agentToolTrace?: AgentWebSearchToolTrace;
  ...
  const payload = await fetchRuntimeJson(agentWebSearchEndpoint, {
    ...
    body: JSON.stringify({
      ...
      agentTaskEnvelope: input.agentToolTrace,
    }),
  });
