...
export function buildDirectorTextQaPrompt(input: DirectorTextQaInput): string {
  ...
  return [
    ...
    "5. 故事板组是否合理：同一个故事板组只能承载同一空间/天气/光线基准；不同场景应拆成不同故事板或视频任务；同一场景不要重复生成多个场景参考。",
    "6. Seedance prompt 是否存在矛盾：故事板数量、最终可见剪辑数、总时长、no BGM、禁止渲染箭头/数字/面板框/文字等。",
    "7. 风格和用户意图是否被尊重；比如用户要 1990 年代日本 TV 动画，就不要写成真人写实、3D 或商业广告质感。",
