...
function isPlanningMetadataLine(line: string): boolean {
  ...
  return (/^(?:标题|片名|故事名|项目名|作品名|Title)\s*[:：]/iu.test(trimmed) && trimmed.length <= 120)
    ...
    || /^先(?:整理|拆|规划)(?:故事|镜头|分镜|脚本|节奏|规划|计划)/u.test(trimmed);
}

function isDirectiveOnlyLine(line: string): boolean {
  const trimmed = clean(line);
  return isDirectorAgentPermissionControlOnlyIntent(trimmed)
    || /^(?:风格|画风|节奏|参考|偏好|要求|限制|请先|先查一下|查一下|先做规划|先做计划|先做分镜|先整理|先拆|不要|不用|先不要|先不|先只|只规划|只做计划|只做规划)\s*[:：]?/iu.test(trimmed);
}
...
function isCompactStyleDirectiveClause(clause: string): boolean {
  const text = clean(clause).replace(/[，,；;。.!?！？]+$/gu, "");
  return /^(?:(?:气氛|氛围|感觉|整体)(?:要|保持|偏向)?\s*)?(?:安静|克制|沉静|冷静|温柔|诗意|写实)(?:[、，,和与\s]+(?:安静|克制|沉静|冷静|温柔|诗意|写实)){0,3}$/u.test(text);
}
  ...
  const storyLines = cleanMultiline(value, maxLength)
    ...
    .flatMap((line) => {
      ...
      const withoutInlineDirectives = prefixed.line.replace(
        /(^|[，,；;。.!?！？\s])(((?:我)?(?:想要|希望|偏向|请先|先查一下|查一下|先做|先看|先整理|先拆|先规划|参考|类似|像|不要|不用|先不要|先不|先只|只要|只做|只看|只拆|只整理|不生图|不生成|不提交|不跑|不走生图|跑到视频前|风格|画风|节奏)|(?:你来|由你|请你)决定|(?:镜头|分镜)(?:数量|个数)?(?:由你|你来|请你)(?:决定|安排|规划)|(?:镜头|分镜)(?:怎么|如何)(?:拆|分|安排|规划)(?:你(?:来)?|由你|请你)(?:安排|决定|规划)?|(?:(?:拆成|分成|分为|切成|规划成|做成|整理成|整理为|重排成|重排为)\s*(?:(?:[0-9０-９]{1,3}|一|二|两|俩|三|四|五|六|七|八|九|十|十[一二两俩三四五六七八九]|[一二两俩三四五六七八九]十[一二两俩三四五六七八九]?)\s*(?:个|条|段)?|(?:最少|尽量少|最精简)(?:的)?)\s*(?:镜头|分镜|视频段|视频|短片|片段|段落|shots?|clips?|cuts?)))[^。.!?！？\n]*)/gu,
      ...
      const withoutTrailingStyleDirective = withoutInlineDirectives.replace(
        /(^|[。.!?！？])\s*([^。.!?！？\n]{1,24})[。.!?！？]*\s*$/u,
        (match, separator: string, clause: string) => {
          if (!isCompactStyleDirectiveClause(clause)) return match;
          directives.push(clean(clause));
          return separator;
        },
      );
      const withoutPermissionControls = stripDirectorAgentPermissionControlPhrases(withoutTrailingStyleDirective)
