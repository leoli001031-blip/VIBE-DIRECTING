...
export interface AgentWebSearchToolResult {
  ...
  warnings: string[];
  agentTaskEnvelope?: Record<string, unknown>;
...
export function createAgentWebSearchTool(config: AgentWebSearchToolConfig = {}): ToolDefinition<AgentWebSearchToolInput, AgentWebSearchToolResult> {
  ...
  return {
    ...
    async execute(rawInput, context): Promise<AgentWebSearchToolResult> {
      ...
      const taskEnvelopeRecord = typeof context.taskEnvelope === "object" && context.taskEnvelope !== null
        ? context.taskEnvelope as Record<string, unknown>
        : {};
      const agentTaskEnvelopeId = typeof taskEnvelopeRecord.id === "string" ? taskEnvelopeRecord.id : "";
      const agentTaskInputHash = typeof taskEnvelopeRecord.inputHash === "string" ? taskEnvelopeRecord.inputHash : "";
      const result: AgentWebSearchToolResult = {
        ...
        promotionAllowed: false,
        warnings,
        ...(agentTaskEnvelopeId && agentTaskInputHash
          ? {
              agentTaskEnvelope: taskEnvelopeRecord,
              agentTaskEnvelopeId,
              agentTaskInputHash,
            }
          : {}),
