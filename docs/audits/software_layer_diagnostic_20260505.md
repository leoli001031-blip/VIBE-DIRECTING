...
- Real Demo E2E 002 final report is `blocked` with declaration `readiness_harness_only`.
- Planned real outputs: 6/6.
- Provider observation sidecars: 6/6.
- Semantic QA sidecars: 6/6.
- Semantic QA P0/P1/P2: 0/0/0.

## Issues Found

| Severity | ID | Issue | Recommendation |
|---|---|---|---|
| P1 | real_demo_002_final_report_not_ready | Real Demo E2E 002 final report is not ready | Treat this as expected until output return, semantic QA, and preview update facts are all present; diagnostic should report it instead of crashing. |
| P1 | semantic_qa_not_completed | Semantic QA sidecars are present but not completed | Require an actual image-review pass to replace template_pending_image_review sidecars before claiming real-chain readiness. |
