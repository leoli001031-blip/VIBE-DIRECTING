...
把 demo 主界面从“半自动工作流面板”收成“Agent 主导的视频项目工作台”。用户只需要在右侧和 Agent 说目标或拖素材；Agent 负责说明它理解了什么、缺什么、准备怎么做，以及现在是否需要用户确认。
...
- 导出：保持可见，但项目不完整时禁用。
- 发送：右侧 Agent 输入框的主操作，不能只依赖快捷键。
- 浏览器草稿或缺少本地项目时，右侧主按钮仍保持“发送”；项目要求放进状态说明或确认卡，避免用户以为不能继续沟通。
...
- 修改建议和待确认修改：Agent-first 模式放进右侧消息流；非 Agent-first 的旧入口才允许折叠显示。
...
- `NewVideoStart`：右侧 Agent 输入框是主入口；中间只显示结果，镜头编辑和草案细节默认收起。
