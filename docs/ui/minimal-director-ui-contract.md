...
- Story / section tabs：故事段落、镜头列表、当前选择；顶部 tab 只显示短名，完整 section label/id 放入 title / aria。
- Preview：粗剪/预览状态和 selected shot 的轻量反馈。
- Selected / Scope：自然语言变更的作用对象和影响范围。
- Diagnostics 入口：用户需要时能进入工程事实层。
- Creator status：主界面只保留一个极短状态点/文案，不常驻详细进度计数。
...
- 在 Story Flow 显示 queue / gate / provider / task 细节。
- 在 Agent 面板展示 prompt、复杂 task details 或“准备修改/确认修改”式工程流程。
- 在 Asset Library 或 Preview 里混入 provider unlock、credential、live submit 或 bypass 控件。
- 让添加资产表单占据 Asset Library 首屏；它应折叠成轻入口。
...
- DirectorProgressStrip 的完整计数、queue/task 细节和阻断明细。
...
- Story / section tabs 使用短 label 且 CSS ellipsis，不让长 id 压坏 Preview 顶部导航。
- Director 三个主页面不挂载详细 DirectorProgressStrip，只保留 compact status dot；详细计数进入 Diagnostics。
- Asset Library 添加表单折叠，阻断显示短状态，角色/场景/道具/风格均图像优先。
- Story Flow 不展示 queue/gate/provider 工程细节。
